...
- Added optional, repeatable historical-claim entry after a Vanuatu rapid current observation. An RA can record one structure event, worship-function event or state, denomination or affiliation state, leadership state, shared-use state, or other historical claim at a time. Each claim retains the source wording or a confirmed dictated account, supported partial date bounds or an explanation of unresolved dates, confidence and its basis, source details, and sensitivity. The dedicated `historical_claim_v1` contract is authenticated, author-bound, user-idempotent, rate-limited, and restricted to an active Vanuatu rapid observation. It does not infer dates, fill target-year states, alter the current observation, or write to the master or public map. Reviewers can inspect each claim separately, and the export files now preserve `historical_claims.jsonl`. Files: `convex/{historicalClaims.ts,model.ts,schema.ts,exports.ts}`, `convex/lib/{historicalClaims.ts,rateLimits.ts,validators.ts}`, `apps/regions/nz/{verification.html,js/historical-claim-contract.js,js/verification-map.js,js/review-portal.js,js/convex-task-client.js}`, tests, export materialiser, FAQ, and API/portal documentation.
