...
- PR-E of the ruled temporal plan — census-year states derived from the periods entry on assigned tasks (`docs/development/assigned-task-periods-brief-2026-09-02.md`, JB rulings R-E1–R-E4 of 2026-09-02; revised on the PR #72 review). In the guided form the period cards replace the hand-set target-year grid as the temporal entry; the grid survives collapsed under "Set census-year statuses by hand" with a required reason, saved as `target_year_entry_reason` and shown to the reviewer, and the action prefills it only while open while the current-observation defaults still follow the action. An ordinary submission needs at least one period (a duplicate claim, the hand grid with its reason, or the unresolved-note route stand in). The cards are part of the draft: persisted on the device per country, user, and task, saved with every draft as `pending_occupancy_cards` (cleared by the server once recorded), restored on reopen, and discarded with the form on task switch, skip, or sign-out. A preview strip states the derived census-year states with the rule in words and flags conflicts, on a verbatim client port of the presence rules held equal to the server by `convex/lib/occupancyDerivationMirror.node-test.mjs`, which also asserts the exact 2013/2018/2023 outputs of every worked case. After the first complete card the form asks once whether there was a spell with no worship here; "Yes" adds the later period, "Not sure" takes bounds (a latest-only stop is refused rather than forged) and never an invented date. The same prompt and preview render in the post-submission periods pane. A still-in-use card is anchored to the source date, not the day the page opened, and follows it when it changes. The periods rest on the evidence's own source and assessment by default, with the numeric assessment confidence mapped to the occupancy vocabulary and a blank refused; on submit they follow the parent evidence, and a failed periods call leaves the parent standing with the cards reopened, unsaved, in the pane. Reopening a task (a revision in particular) loads the RA's active periods into the cards, and the server supersedes the author's set on the task's earlier parents when the revision's set is recorded. Vocabulary: the start basis gains `reopening_stated` with rule 2b `before_stated_reopening` (absent), so a stated reopening never has to be recorded as a founding; reviewer text, `Confirm all eligible`, the client mirror, and the collection schema move together. Guide step-2 capture recaptured. Files: `convex/{model,schema,occupancies}.ts`, `convex/lib/{occupancies,limits}.ts`, `convex/lib/{occupancies,occupancyDerivationMirror}.node-test.mjs`, `apps/regions/nz/js/{verification-map,occupancy-contract,occupancy-review,review-portal}.js` (stamps `20260902n`, `i`, `f`, `f`), `apps/regions/nz/js/occupancy-contract.test.cjs`, `apps/regions/nz/{verification,review}.html`, `apps/guides/ra.html`, `tools/guide-captures/{capture.mjs,manifest.json}`, `schemas/collection.v1.schema.json`, `docs/development/{assigned-task-periods,occupancy-build}-brief-2026-09-02.md`, `docs/api/convex-functions.md`.
