...
- Moving an existing Google member to the new sign-in needs proof that the person holds that Google account, not only its mailbox (R-C18, JB 2026-09-24). A new mutation `users:beginIdentityMigration`, callable only by the Google sign-in that is a member's current identifier while the move is open, issues a single-use grant for that member, valid ten minutes; only its SHA-256 is stored (new table `identity_migration_grants`). `claimInvite` takes the grant as a new optional argument and spends it in the same transaction as the re-keying; without it, or with an expired, used or other member's grant, it refuses, so a verified email alone never moves an existing account. Issue and use are recorded in `role_events`, without the secret. On the portals, an existing member whose sign-in needs the proof sees "Confirm your existing account for the new sign-in" with Google's own button, and the card folds "Used Google sign-in here before?" for doing it first; the grant is held in the page's memory only, never in storage. It is presented only by the session it was issued in, or, when confirmed before sign-in, by the next sign-in started with that page's own form; it goes with any sign-out, any change or end of an established session, or a session that arrives from another tab, and a grant answer that arrives after that is dropped. `beginIdentityMigration` resolves its caller through the shared `resolveUser` and refuses a linked, earlier identifier.
