...
- Review round 2: every sign-out bumps a session epoch on both portals, and a task list, queue, evidence read or submission response asked for by the ended session (or for another user) is dropped rather than repainting the page. Drafts kept on the device without an owner (written before this change) are granted to nobody: they are removed once at load, and the next person to sign in is told an unsaved entry was removed, without seeing it; nothing ownerless is written any more, and a deliberate sign-out deletes only its own user's snapshots. Clerk resolves a sign-out whose request never reached its server, so the client now confirms revocation by reloading Clerk's client and checking the session is no longer live; an unconfirmed or offline confirmation counts as failed and keeps the retry. Stamps `?v=20260924-clerk3`.
