...
- The public-map rebuild retires a revised record's occupancy set (handover 2026-09-09: "a revision and its source both draw until supersession is written"). A revision task made through *Move the pin* or *Revise this place* names its record in `issue_report.source_task_id`. `scripts/build_occupancy_dated_places.py` now drops every accepted occupancy row on that source task once a reviewer has accepted the revision for export and the revision holds at least one accepted occupancy row; a revision that recorded nothing leaves the source on the map, and a chain of revisions retires every earlier link. Replacement is set-level, as `supersedeEarlierOccupancySets` in `convex/occupancies.ts` retires an author's earlier set within one task; the builder never matches rows across tasks. The summary reports `superseded_by_revision_dropped` per country. The ruling fixture now has the revision re-record both periods; three new tests cover supersession, a recording-free revision, and a two-step chain. Files: `scripts/build_occupancy_dated_places.py`, `scripts/test_build_occupancy_dated_places.py`, `docs/portal-location-and-occupancy-plan.md`.
