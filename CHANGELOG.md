...
- Repaired the personal-details quarantine contract of the internal agent research pipeline (orchestration step S1; design agreed 2026-09-24). A review bundle previously fixed `item_count` to 0 and admitted no items, although `lib.redact_quarantine` writes a positive count with hashed values, so a correctly redacted dossier with any quarantine hit could not be packaged. A bundle now admits a redacted block (`redacted: true`) whose items carry only `kind` and `context_claim_id`, with `item_count` equal to the number of items. No value and no hash of a value enters a bundle, the review prompt or Convex; the hashes that let a later run recognise a recurring detail stay in the private dossier copy the runner writes to its run directory. The Python, TypeScript and Rust validators refuse an unredacted block, an item with a value or a hash, a count that differs from the items, and a reference to an unknown claim; eleven shared regressions cover these cases. `lib.redact_quarantine` refuses an item with neither a value nor a valid SHA-256, a new `lib.bundle_quarantine` reduces a redacted block to the bundle form, and the standalone `agent-dossier.v1` schema now requires `value_sha256` to be a SHA-256 hex digest. Because the block no longer stops a run with a hit, redaction and screening now cover the whole dossier: `lib.quarantine_dossier` walks every string of the dossier the bundle schema leaves unconstrained, the walk the first-pass receipts already screened with (now shared as `screenedStrings` in `convex/lib/agentIntake.ts` and `screened_strings` in `lib.py` and `pow-cli`), and all three validators refuse a phone number, email address or honorific-led name surviving anywhere in the dossier. Before this, a name in a claim's source title or in the run notes passed all three validators. A privacy review then widened the screen: every string value and undeclared key of the whole outgoing bundle, the reviewer's text and both run manifests included, is screened whatever its schema says (a place slug such as `new:021-123-4567` is no longer exempt), and a value shaped as a hex hash is refused outside the designated hash fields listed in the new shared `schemas/screen-policy.v1.json`. The runner withholds every recurrence of a quarantined value, normalised for case, width and spacing and with a caught name also withheld without its honorific, and refuses before transport any reviewer output that carries a personal detail, a quarantined value or its hash, keeping the original privately as `review.refused.json` and recording on `run-result.json` each finding's path, detector and code-point span without its text; refusal is the interim behaviour until a flag-and-hold review exists. `intake.py submit` now asks the new `internalAgentIntake:findReceiptByHash` query with the bundle's hash alone and compares the returned digest of the stored bytes locally, so a locally refused bundle never leaves the machine. The Rust honorific and email checks now use the same patterns as Python and TypeScript, so they also catch forms such as "Rev'd" and an address followed by a full stop. A quarantine item's `context_claim_id` takes claim ids up to the same 8,000 characters as the claims. The shared bundle fixture's quarantine block and the two first-pass fixtures' embedded dossiers change only from `redacted: false` to `redacted: true`; the shared regressions gain twenty-two more cases for the whole-bundle screen, designated hash fields and long claim ids. Files: `scripts/agent_research/{lib,intake,internal_runner,test_agent_research,test_internal_runner,test_intake}.py`, `scripts/agent_research/{first_pass,test_first_pass}.py`, both schemas and `schemas/screen-policy.v1.json`, `scripts/agent_research/fixtures/{internal-review-bundle,intake-regressions,first-pass-researched,first-pass-all-fields}.json`, `convex/lib/{agentIntake,firstPass}.ts`, `convex/internalAgentIntake.ts`, `convex/{firstPassReceipts,internalAgentIntake}.node-test.mjs`, `crates/pow-cli/src/main.rs`, `docs/development/internal-agent-review.md`, `docs/development/agent-research-pipeline-brief-2026-09-10.md`.

### 2026-09-24 (lean freeze, PR L1)

- Export batches are now budgeted and a country is frozen by a scheduled chain (lean-storage brief PR L1; JB rulings 1, 2, 3 and 12 of 2026-09-24, D20 step two-b). `createExportBatch` measures every task it takes (the freeze recheck and bundle reads, through a read meter) and refuses a batch whose estimate breaks the 6 MiB plain-bundle budget or the read budget (10 MiB, 16,000 documents, 2,048 index ranges), naming the task; an explicit list is never truncated. This replaces PR #112's interim gates (the 100-task automatic-selection cap and the capture-time byte refusal). New `exports:composeExportBatches` cuts a country's `pi_accepted` tasks into budgeted draft batches in bounded scheduled steps and captures the run's membership in `export_run_members`, so a task accepted later waits for the next run; `exports:freezeCountryBatches` freezes the run's batches one per invocation under a per-country lease, each invocation scheduling the next, stopping on a failed batch (never skipping it) for a curator re-run to resume; `exports:getExportRun` reports progress. A composition that replaces an earlier run archives its unfrozen drafts (status `archived`). Stored files are now gzip-compressed with the pinned pure-JS codec `fflate` 0.8.3 at level 6 with a deterministic header, and every `frozen_files` entry records the encoding, the stored SHA-256 and length, and the codec record beside the plain SHA-256 and length; freezing and retrieval verify all four values, and batches frozen before this change are still served and verified as plain bytes. `pending_freeze.stored_objects` records each object as it is stored, so a dead attempt leaves a trail that a later attempt's completion or failure cleans up. Tasks record `last_export_batch_id` and `last_exported_at`; the `exported` transition stays at freeze completion. All schema changes are additive (two new tables, optional fields, one new index). The `pow-export-bundle.v1` contract and its fixture are unchanged. Review repairs before merge (gpt-6-astra, a Convex reviewer, Greptile): the read meter now charges each document as it streams, to the task's budget and to a per-transaction ceiling, so an oversized history is refused by name within one row; candidate reads (training-excluded rows included) are charged to each composition step, taken one bounded `paginate` page per step with the cursor stored, and automatic selection streams the index, so tasks sharing a creation time are never skipped; a replaced run is marked at once and its drafts archived in bounded steps; `completeFreeze` records run progress and checks the stored files against the captured bundle and the attempt's trail; a two-minute hand-off lease closes the gap between chain invocations; a run's batch is frozen only by its chain; a batch row budget (256 KiB), a capture tolerance (1 MiB), `stalled` on `getExportRun`, and de-duplicated task lists; in a second round, the cutting steps stream and meter member rows and stop at a batch boundary, and starting or resuming a chain takes the run's lease in the same transaction. Measured on an isolated local backend with 201 realistic synthetic tasks: five batches of up to 42 tasks, `prepareFreeze` reading 9.6 MiB (60% of the transaction cap) and returning 5.7 MiB, each batch freezing in under a second; the figures are in `docs/development/frozen-exports.md`.

### 2026-09-24 (first-pass receipts)

- Archived first-pass research records can now be submitted to the shared backend for receipts, and the judgments they carry are recorded in `agent_judgments`. `first_pass.py submit HASH --store PATH --deployment dev|local` verifies a record's parent graph, screens the whole history, and sends each record, parents first, to the internal mutation `firstPassReceipts:ingestFirstPass`, behind the existing `POW_INTERNAL_AGENT_INGEST_ENABLED` gate. The backend accepts only the archive's exact version-1 wire format (Python's sorted, compact, ASCII JSON, re-encoded and compared in `convex/lib/wireJson.ts` against Python-generated cross-language vectors), re-applies the first-pass schema and semantic rules, refuses any string in the record or its dossier, other than fixed-shape values, that carries a phone number, email address, or honorific-led name, applies Python's integer and float distinction to the schema check so every accepted record restores, requires each parent to hold a receipt for the same place, and resolves any portal `context` to one task in the record's country and about its place. It writes one `agent_first_pass_receipts` row (`first-pass:<sha256>`, storage tier `convex_only`, bytes kept until an independent copy verifies). A record with a dossier also yields the researcher's `status_assessment` of the place, attributed from the dossier's run and shared by passes carrying the same dossier, and the author's `annotation` judgment per claim annotation, attributed from the record. An identical retry returns the existing receipt and writes nothing, before any validation, so a later rule cannot block it. `first_pass.py restore` rebuilds a record's history in a clean local archive from the receipts and verifies every hash. Reviewers, curators, administrators, and the PI can read receipts with `getFirstPassReceipt` and `listFirstPassReceipts`. The receipt contract (`object-receipt.v1`, `convex/lib/objectReceipts.ts`) is general so that a later inspection import can reuse it. The judgment writer's parent lookup now reads a new `by_lineage` index on `agent_judgments` and takes at most ten rows instead of collecting every judgment on the subject. The backend change is additive: one new table, one new index on an existing table, one new module, and no change to any existing field, argument, or return type. The receipt grants nothing: it creates no task, draft, version, or review decision, and every record stays provisional. A synthetic researched fixture, `scripts/agent_research/fixtures/first-pass-researched.json`, joins the tests.

### 2026-09-24 (agent provenance repairs)

- Repaired three provenance gaps in the internal agent research pipeline, each first reproduced against `main` (orchestration ruling R-O4, JB 2026-09-24). A claim whose source host lies outside the pinned allowlist's domains is now a validation failure in the Python, TypeScript and Rust validators, which read the same `fixtures/allowlist-nz-v1.json` and apply one host policy: the host is read as written, non-ASCII hosts, userinfo, empty or invalid ports and empty labels are refused, the host is lower-cased and exactly one trailing root dot is stripped. A listed domain covers its subdomains but not a lookalike suffix, and an unknown allowlist version is refused. The runner checks the dossier before paying for the review and records `allowlist_violations` and the offending hosts on `run-result.json`, including when a later step fails. A null or empty `model_id_reported` on the research run, the review run or the dossier manifest is now refused, as is a dossier manifest naming a different model from its research run; the bundle schema's run `model_id_reported` is a non-empty string. Claude cost is read per billing model from `provider_usage.modelUsage` into `usage.per_model`, because the flat usage block omits helper models the client chooses; the dossier's `cost_usd_reported` is that total at list price, and an incomplete per-model block leaves the cost null with basis `unknown`. Intake returns the existing receipt for an exact retry of already-receipted bytes before re-validating them, and `intake.py submit` reaches that receipt through the new read-only `internalAgentIntake:findReceiptForBytes` query when local validation fails, without sending the bundle for ingestion. The shared fixture's two example.org locators now use an allowlisted host and its two null run model ids now carry reported ids; the shared intake regressions gain cases for allowlist hosts, host normalisation and reported model ids. Consequences: a parish's own website is refused until an allowlist version names it, since the church-website rule cannot be checked by machine, and a Codex run whose trace names no model fails intake until the runner reads a model id from the provider. Files: `scripts/agent_research/{intake,internal_runner,test_intake,test_internal_runner}.py`, `scripts/agent_research/schemas/agent-review-bundle.v1.json`, `scripts/agent_research/fixtures/{internal-review-bundle,intake-regressions}.json`, `convex/lib/agentIntake.ts`, `convex/internalAgentIntake.ts` and both node tests, `crates/pow-cli/src/main.rs`, `docs/development/internal-agent-review.md`, `docs/development/agent-research-pipeline-brief-2026-09-10.md`.
...
- Version 0.1.6 of the [operational definition](docs/operational-definition.md), snapshot at `docs/development/place-of-worship-definition-2026-09-22-v0.1.6.md` (JB rulings, 2026-09-22). A new section, Purpose and entity, states why the project counts places of worship and defines the entity by six kinds of attribute (spatial, temporal, worship, community, functional, relational), each carrying uncertainty and vagueness, with the identifier tracking identity through time. Identity is now continuity of the place of worship at its site, with an end condition: a conversion that continues worship under a new community is a dated attribute change (Hagia Sophia is one place); a place ends when worship ceases and the site is destroyed, repurposed or abandoned with no succession, and a later recurrence there is a new place linked by location (the Templo Mayor and the Metropolitan Cathedral are two); a move to a new site is a new place linked to its predecessor (Christ Church Cathedral and the Transitional Cathedral are two); a building moved within its site keeps its identifier. Ten worked examples recovered from JB's counting discussion of 2026-09-22 sit under Identity over time, and Measurement over time states the two governing commitments: the project records evidence for attributes consistently, and investigators decide which frequency and confidence combinations count. JB revised the canonical text by hand before adoption, adding the limits of the definition, the vagueness of "move", the evidential status of the OpenStreetMap-sourced map, and a comment on geography, whose rule for one plotted point is one place per distinct worship space unless the spaces form one coherent worship complex (a cathedral's nave and side chapel are one; a hospital's separate chapel and prayer room are two; a chaplaincy suite run as one centre is one), on the stated rationale that sharing and separating are different social activities. Cemeteries and homes have a stated default: candidates until evidence shows recurring worship by or for a community; a household is not a community, so a house church counts and a family altar does not; a cemetery with calendrical community rites counts at intermittent use, and a churchyard is a component of its complex. After the Fustel de Coulanges test, a corporate kin group at a shared site (lineage hall, gens tomb, urupā) is a community and counts, the household exclusion reaches only the domestic unit worshipping for itself, and documented domestic worship is recorded as an attribute of a population, not as sites; the rationale is written into the definition. Criterion 2 gains the decade rule (a period says whether it covers its years throughout or places worship somewhere within them; only the first establishes each year), Measurement over time restores the study statement for any group's studies and adds the two angles on temporal bounds: uncertainty (bounds and coverage) and vagueness (a criterion: a place begins when recurring worship begins at the site), with the Cathedral of Mexico as the example. Amendment R-F1′, the country index and the shared-point rule are folded into the canonical text; a shared room is one place with several community attributes and a hospital's separate chapel and prayer room are two. The v0.1.5 snapshot carries a supersession notice, and the FAQ and the counting discussion point at the new version. Documentation only: the occupancy validator, the occupancy plan and the review-decision vocabulary still encode the earlier relocation rule, listed in the revision note as follow-up work.
...
### 2026-09-23 (entry follows the pin)

- The contributor signs in at a country and the entry moves between countries freely (JB ruling, 2026-09-23: "sign in at a country, but allow entry to move effortlessly between them; a PoW is a place on a map in time, and countries can be recovered from places and time"; raised by a Sweden page whose `Drop pin at my location` landed in New Zealand under the Sweden banner). The entry's country is now derived from the pin, or the quick photo's fix, through the shared resolver `apps/shared/region-resolve.js`, loaded on the portal: the page manifest's exact outlines (`region-bboxes.json`, 795 KB) and the world outlines (84 KB) load once, off the critical path, the first time an entry could need them; until they arrive the registry's boxes place the point (smallest containing box), and an outline arriving mid-entry re-reads the open pin. The derived country's code goes to `submitCurrentObservation`, `createIssueTask` (a revision) and `createSource`, so the task lands in that country's nomination or issue batch with its census years; the server's intake box keeps the last word, and a point no source can place keeps the page's country. A moved entry carries a warning and never a block: "This pin is in New Zealand; this page opened for Sweden. The entry is recorded as New Zealand." under the entry's title, on the quick photo card, on the recorded screen, and in the header as "Sweden evidence · entry in New Zealand" while the entry is open. A neighbour's dot on the map now revises here, recorded as its own country, where its popup used to send the contributor to the other portal. The contributor's own work follows the user, not the page: `listMyTasks` is called without a country and the list keeps the assignment batch and any country's nomination or issue batch. The derived country is the present-day index the review queue and the master data use, never a historical claim; the operational definition now says so under Measurement over time. No backend change: the deployed `listMyTasks` already takes an optional country and the world intake registry has served every country since PR-H1. Tests `entry-country.test.cjs` (five checks, real registry and resolver) in CI, a Sweden-fix check in `quick-photo.test.cjs`, and the foreign-dot case in `country-config.test.cjs` updated to the new ruling. Style guide, RA guide and operational definition updated; stamp `verification-map.js?v=20260923a`. Not built: a handoff link to the other portal (no longer needed) and auto-switching the page (rejected: it would drop the open entry).
- `Quick photo` wears the entry teal (`--entry`), filled, the colour the contributor's own nominations already carry, so it stands out under `Add / Revise` (JB, 2026-09-23: "make the QUICK PHOTO tab easy to see"). Convex wiring checked on the live deployment for Guy's use: attachment storage answers enabled, and every function the flow calls (`submitCurrentObservation` with `flagForDiscussion` and a candidate, `requestAttachmentUpload`, `confirmAttachmentUpload`, `listTaskAttachments`, `listMyTasks`) is deployed with the arguments the portal sends.
...
- A `Quick photo` button under `Add / Revise` on the contributor portal (JB, 2026-09-22: "a quick photo entry button, where a user snaps a shot of a possible PoW for further (potentially AI) review"). One press opens the phone's camera (`capture="environment"` on a hidden file input; a computer offers its file picker) and asks for the position at the same time. The photo shows on a card with the fix ("about 8 m", the blue ring on the map, the map moved to the spot), an optional place name and an optional note for the reviewer (Greptile P1 on #139 was valid: one "name or note" field would have stored commentary as the task's name; the name alone becomes `candidate.name`, the note travels only in the discussion text); `Send for review` lands it on the rapid lane as a flagged partial entry (`flagForDiscussion`, `could_not_determine`, basis `other`, privacy `needs_review`, today's date), with an approximate-area location assertion whose radius is the fix's accuracy clamped to the contract's 25 m to 100 km and basis `local_investigator_account`, confidence moderate to 50 m and low beyond. The task lands as `unresolved_note` in the review queue with the note "Quick photo capture: ... details not entered." and the photo uploads against it on the recorded screen (caption `Quick photo capture`), through the existing pending-files path. The nearby check at send reads both sources this device has, the task features the portal holds and the rendered dot nearest the fix on the map's tiles (Greptile's second P1 was valid: the task features alone miss reviewed and exported places); a recorded place within 150 m or the fix's radius is named on the card and the first press holds, the second sends anyway, and the note carries the count and the names (or "nothing recorded within 150 m on this device's map"), so the reviewer knows what was checked. `placement_zoom` is the map's zoom after it moves to the spot. No position (refused, unavailable, timed out): `Send` stays closed and `Drop the pin instead` carries the photo into the ordinary add flow, counted on the pin form's picker and uploaded with whatever the contributor saves there; leaving that flow drops it. A failed send keeps the card and reuses its submission id, so a retry cannot land twice. `Add / Revise` reads `Cancel` while the card is open and asks before discarding the photo; Escape does the same. The button shows only signed in, in `Add / Revise`, with attachment storage wired (one probe per sign-in) and no entry open. Nothing in the backend changed: the Claude batch-review lane does not yet read attachments, so "AI review" of the photo is a follow-up on that lane; the note and the caption mark the captures for it. Test `quick-photo.test.cjs` in CI (six checks); style guide and RA guide updated; stamp `verification-map.js?v=20260922c`. Unverified on a device: the camera-first input and the geolocation prompt ordering on iOS Safari.

### 2026-09-22 (a held touch on the pin)

- A held touch on the pending pin (a right click on a computer) opens the pin's own menu (JB, 2026-09-22: "once a pin is dropped, say by accident, how can we remove it"; the card's `Cancel placement` can sit below the fold on a tall entry pane). Before the location is confirmed: `Remove pin` lifts the pin and keeps the entry armed for another drop (the map takes the screen again), `Cancel placement` leaves the entry as the card's button does. Once confirmed: `Discard this entry`, which asks first. The undo path's removal shares the new `liftPendingPin`. Check 14 in `add-revise-control.test.cjs`; style guide and RA guide updated; stamp `verification-map.js?v=20260922b`.

### 2026-09-22 (Sweden crash on the phone; tap to revise, hold to add)

- iOS Safari killed the contributor portal on Sweden at the first pinch ("A problem repeatedly occurred", JB's screenshot, 2026-09-22). Cause: the unreviewed-places layer drew every dot as an SVG DOM path (leaflet.vectorgrid's default renderer), and the places-overview tiles a phone's viewport requests at country zoom carry some 317,000 places across Europe (Germany 104,000, Poland 85,000, Sweden 7,500; one zoom-5 tile is 748 KB of protobuf), re-rendered at each zoom step while Leaflet retained the outgoing tiles. `unvalidated-places.js` now renders both tiers on canvas tiles (`L.canvas.tile`, zero DOM nodes per dot; the nearest-dot hit test reads the same per-tile symbol table) and takes an `overviewKeep(props)` predicate: the RA portal keeps only its own country's places (and any the tiles leave untagged) at zooms 5 to 7, while the full tier from zoom 8 keeps every place, a neighbour's dot with its own portal named in the popup as before. The review portal shares the module and gains the canvas renderer. Measured in desktop Chrome at the Sweden opening view: 401,000 SVG paths and a 296 MB heap before, 9 paths after; on a phone-sized viewport the overview tiles now hold 33,000 canvas symbols (7,500 Swedish, 25,500 untagged) in place of 317,000 DOM paths, and a tap still resolves to the named church at both tiers. Follow-up for the tile server, outside this repo: the overview tier should be thinned at source.
- Tap to revise, hold to add (JB, 2026-09-22: "if you hit a dot on the map and press you automatically get edit, and if you press longer on the map you automatically get add (double click on desktop)"). Signed in, a tap on a recorded dot opens the revise entry on that record with no popup between; the popup remains for the signed-out reader (its sign-in entry), for an armed pin (revise or add here) and where the rapid lane cannot take the record (the flag-only card beside it). A held touch on the map (Leaflet's `tapHold` on mobile Safari, the native long press on Android), a double click or a right click on a computer, opens the add entry with the pin already on that spot; on a dot the press belongs to the dot; with a pin armed it lands the pin; with the pin down the taps and the drag already move it. `doubleClickZoom` is off. The hint under `Add / Revise` names both gestures. Tests: checks 12 and 13 in `add-revise-control.test.cjs`, the renderer and keeper checks in `unvalidated-places.test.cjs` (now in CI). Style guide and RA guide updated. Stamps `unvalidated-places.js?v=20260922a` (both portals), `verification-map.js?v=20260922a`.
...
- Photos and documents are one button, `Take photo or add files`, in front of the hidden native file input, so a phone offers its camera and the count of files chosen reads beside the button; the help line is one sentence. Both the in-form block and the Step 2 block use it.
- The post-sign-in chooser no longer renders: the portal lands in `Add / Revise` on every screen with the task list, filters and counts absent (JB: the task list is specialist assignment work, most of it for agents or the review portal). Test `add-revise-control.test.cjs` covers the label, the hint, the dispatch and the cancel; the RA guide describes the control.
- Screenshot round (JB, 2026-09-20 evening). The header's batch line (`nz-temporal-ra-workpack-001 | sign in to load assigned tasks`) is gone from the portal; `Exit` and `Guide` are outlined buttons at body size on a 44 px target. `Add / Revise`, `Cancel` and a new `Assigned tasks (n)` button share one size (56 px, `--fs-lg`); `Assigned tasks` shows under the control only while the batch holds tasks open to the contributor or work of theirs in progress, and leaves while an entry is open; the add-mode bar with `Assigned tasks →` is gone, the assigned sheet keeps its bar with `← Add / Revise`. The pin card heads with `Add a place` at heading size and three full-width options, `Drop pin at my location`, `Drop pin on map` (the map takes the screen; tap the building) and `Search and drop` (folds open the address search and the coordinate boxes); on a revise or period pin the same buttons read `Move pin`. Typed coordinates move the pin as the second box is left and search results read `Drop pin here` or `Move pin here`; the `Move pin` button is gone, since the pin already drags on the map. The `Flag for discussion` checkbox is its label alone; the field that opens when it is ticked takes the description. **Bug:** a reload with `Add / Revise` remembered for the tab kept the signed-out resting card (`Assigned web workpack … Work through …`) in the detail panel, since sign-in restored the mode without repainting it; the card now re-renders on sign-in when no task is selected. Three more checks in `add-revise-control.test.cjs`; stamp `verification-map.js?v=20260920a`.
- Map data panel (JB, 2026-09-20: "the unreviewed places button and all that text about 2023 is artifactual; perhaps a toggle for show/remove points, with points defaulting to the present"): the `Unreviewed places: on/off` select and the amber-dots note are replaced by one `Hide points` / `Show points` toggle; today's places show by default and the legend row reads `place on today's map, tap to revise`. The guide capture tool's `#portalPointsSelect` step is stale (it was already stale on the retired chooser).
- iPhone Safari (JB, 2026-09-20: with the map on top, the divider "goes blue but moving scrolls down the text"): the divider refuses `touchmove` while a drag is live (non-passive, since `touch-action: none` alone did not hold there) and carries `overscroll-behavior: none`. Best-effort fix, unverified on a device; stamp `verification-map.js?v=20260920b`.
...
- Review portal: each queue row shows one state pill (Needs review, Note to resolve, With the contributor, Awaiting PI, Rejected, Exported) in place of the raw status string, the draft status and the repeated "unresolved note" pill; the queue line is a rollup ("2 to review of 3 loaded"); an empty queue is one line with one action (Refresh, or Show all tasks when a filter hides the rows). The decision form's select is replaced by a chooser: Accept for export and Request changes stand as the two buttons, with Duplicate task, Defer, Reject and Exclude as system test behind one "More outcomes" disclosure; the chosen outcome shows on the pressed button, on a pill and on the Record button's label. A stale-snapshot refusal, a snapshot that failed to load and a refused decision are banners that carry the reason and one control (Reload again, Retry), never a dead end. A transport dot beside the account name says Connected, Loading, Saving or Connection problem. Test `review-state-dom.test.cjs` drives the renderers headless through a test hook.
- RA portal: each task row shows one state pill (Changes requested, Reopened, Draft saved, Open, Awaiting review, Skipped, Reviewed or the decision's own word, Exported) in place of the raw backend badge and the session's skip and closed badges, before the target-year pill; rows sort most urgent first. My work rows carry the same pill, and where the state has a next action the pill is the button that opens the task; the panel's count line and the signed-in card's "N available; M in My work" are one rollup ("2 need your action of 7"). The selected task's state sits under its heading with its hint. A transport dot beside the account name says Connected, Signing in, Saving, Signed out or Connection problem, and save refusals read in red. Empty states are one line and one action: No tasks match your filters (Clear filters), Nothing assigned in this batch yet (Refresh), Sign in to load this workpack. Test `task-state-dom.test.cjs` covers the rows, the rollup, the pill-as-button, the transport dot and the empty states; the ten DOM tests load the new script.
...
- The council list is now credited as a personal communication to Guy from council staff, not as a council publication or internal report, per the same ruling. Task ids are unchanged, so re-running `tasks:adminUpsertTasksFromStaticMap` updates the live batch `vu-vila-council-list-2026-001` in place.

### 2026-09-12 (frozen exports)

- Export batches are now frozen at the byte level (D20 step two). `exports:freezeExportBatch` is an action that rechecks every included task's eligibility and export authority (still `pi_accepted`, not training-excluded, the authoritative decision's pinned evidence version still current, a snapshot-linked decision still consistent, the batch's stored membership unchanged), builds the complete bundle, stores every file in Convex file storage, reads each back and verifies its SHA-256 and byte length, rebuilds the bundle inside the completing transaction and refuses if any row changed in between, and only then marks the batch `frozen` with `frozen_files` and a `manifest_hash`; a failed attempt deletes its blobs and leaves the batch `draft` with `last_freeze_failure`. The bundle follows a new contract, `pow-export-bundle.v1`: a manifest whose `manifest_hash` covers every other member (sorted ids, per-file SHA-256 and byte counts, the sorted set of evidence version and review snapshot hashes) and four new files, `evidence_versions.jsonl`, `evidence_head_changes.jsonl`, `task_acceptances.jsonl`, and `review_snapshots.jsonl`, so a frozen bundle carries the envelopes and snapshots its decisions pin. `exports:getExportBundle` is an action that serves a frozen batch's stored bytes after verifying them and never rebuilds it from rows; a draft batch, or a batch frozen before this change, is served as a live preview marked as such in a new `disposition` block. New `exports:withdrawExportBatch` marks a frozen batch `withdrawn` with a reason, keeps its bytes retrievable, stops further processing, and notes the withdrawal on each task; `createExportBatch` accepts `supersedesExportBatchId`, and the earlier batch becomes `superseded` when the new one's freeze completes. New `pow export verify <dir>` checks a materialised bundle: manifest hash, every file's hash and length, record counts, membership, pinned versions and envelopes, snapshot references, and that each task's latest accepted acceptance still names an accepted decision on an accepted draft. `scripts/materialise_convex_export.py` writes the frozen manifest verbatim and refuses to write `SHA256SUMS` over bytes that differ from it. Ten Convex regression scenarios and eighteen Rust cases cover the freeze, its failure paths, retrieval, withdrawal, supersession, and the verifier; a committed fixture bundle built by the TypeScript freeze (`schemas/fixtures/pow-export-bundle.v1`, generated by `scripts/export_bundle_fixture.mjs`) is verified by the Rust tests, and CI now checks that all three fixture sets are current. Batch size is bounded by the Convex per-call payload limit; see the limits section of `docs/development/frozen-exports.md`.

### 2026-09-12

- Repaired five findings an independent review reproduced on the intake gate and snapshot-linked acceptance (PR #111), before merge. A receipt-backed retry of `submitEvidenceDraft` now returns from its receipt before the gate and before any status write, so a retry after review or PI acceptance (or after the draft was retired) no longer re-marks the draft and drops the task back to `needs_review`. The review portal now renders the task, evidence, events, claims, and latest decision from the snapshot it will submit (new `review-snapshot-content.js`, unit-tested), never from the queue row cached at queue load, so a reviewer edit landing between queue load and task selection can no longer leave old evidence on screen under a hash for the new version, and every selection load is guarded by a selection token so a slow response for an earlier selection can no longer replace the displayed task's snapshot; on a stale-snapshot refusal it reloads the queue and re-selects the task so every panel shows the content the next decision will be made on. The review boundary text no longer refers to "map-note workflows". `recordAcceptance` applies the pinned-version and snapshot-consistency checks to an `accepted` outcome only, so a PI can return a task resting on a legacy (unlinked) decision or a moved version for the fresh review those checks require. `createExportBatch` checks only the decision the task's latest `accepted` acceptance names and keeps earlier accepted decisions as retained history, so a PI return, a fresh snapshot-linked review, and a new acceptance clear an old snapshot's refusal. `importSubmittedEvidenceDrafts` now applies the task-level intake gate after its `skipped_final` rule, so a new draft id can no longer land evidence on a `reviewed`, `pi_accepted`, or `exported` task without a reopen; the refusal names the task and fails the batch. Regression tests cover each scenario.

### 2026-09-11

- Added a shared intake gate and snapshot-linked review acceptance (PI ruling 2026-09-11). Once a task is `reviewed`, `pi_accepted`, or `exported`, `saveEvidenceDraft`, `submitEvidenceDraft`, `submitEvidenceDraftWithOccupancies`, and `submitUnresolvedNote` all refuse to add or replace evidence until the task is reopened; a receipt-backed retry of a submission recorded before the task closed still returns without repeating any write. `recordReviewDecision` now takes an optional `snapshotHash` and refuses an `accepted_for_export` decision without one, a stale one, or one whose evidence draft is missing; a matching hash records the `review_snapshots` row, pins `evidence_version_hash`, and writes the versioned (`decision_hash_version: 1`) decision hash, exactly as the existing batch route already did. A new `reviews:assertDecisionSnapshotConsistent` helper, called by `recordAcceptance` and `createExportBatch`, refuses to ratify or export a decision whose recorded snapshot no longer reproduces its hash, whose pinned evidence version has moved, or whose reviewer-confirmed derived locations have since changed. Pending acceptances resting on an existing version-0 (unlinked) decision now need a fresh snapshot-linked review before a PI can accept them; the decision itself stands as a historical record, and export batches keep including it unchanged when the task already carries an `accepted` acceptance row. The review portal now fetches and shows the snapshot it will submit (hash prefix, draft status, evidence version, and record counts) beside the decision form, sends it with every decision, blocks an unsnapshotted acceptance client-side, and on a stale-snapshot refusal reloads the snapshot and shows the server's message rather than resubmitting. New `convex/lib/intakeGate.ts`. See `docs/development/evidence-versions.md` ("Lifecycle: Retirement, Restoration, And Approval").

- Implemented the first two steps of the content-addressed review contract: the `pow-canonical-json.v1` canonicalisation contract (RFC 8785, TypeScript reference plus a pinned `serde_jcs` Rust implementation, 41 shared golden cases and 5 rejections) and server-created immutable evidence versions (`evidence-version.v1`). Every submission and every write onto submitted evidence now records a version with server attribution, recorded time, content and version hashes, and lineage; retries and unchanged resubmissions return the existing version; corrections link to the version they supersede and new dated observations start a family that references the earlier one. Added `pow object hash` and `pow object verify`, retrieval and audit queries, a migration mutation for pre-contract rows (not run), and regression tests. Decided, superseded, or withdrawn evidence rows can no longer be rewritten in place by any role; a batch confirmation of derived years now keeps every confirmed year on the parent row. Existing hashes keep their contracts; the portal is unchanged. See `docs/development/evidence-versions.md`.

- Repaired five defects an independent review reproduced in the evidence-version work above, before merge: canonicalisation now keeps a member named `__proto__` (cleanup and the content builders copy members as own properties) and refuses sparse arrays, with new golden cases in both fixture files; a correction's parent is the version pinned on the revision clone when it was opened (`revision_of_version_hash`), not the source row's current hash at submission, so an intervening reviewer edit or the retirement of the source's period set by the correction's own guided submission becomes a sibling rather than the parent; every keyed submission writes an `evidence_submission_receipts` row, so a token stays bound to the version its submission received even when the content was deduped and the row later moved, while a token reused against another draft is refused; rapid-entry retries return `evidence_version_hash` from that receipt, or `evidence_version_unavailable: "pre_contract"` for an observation recorded before the contract; and the lifecycle rule now covers `decideDerivedYear`, `confirmAllDerived`, and spreadsheet re-import, so a decided, superseded, or withdrawn record takes no content write from any route except the recorded occupancy-retirement bookkeeping. Regression tests cover each scenario.

- Retired evidence (superseded or withdrawn) now stays visible and can be brought back to active review through `evidence:restoreEvidenceDraft`, with no content change and no new version; a decided record is reconsidered through the review workflow instead. Every supersession, withdrawal, restoration, and evidence version is now recorded in a new append-only ledger, `evidence_head_changes`, readable through `evidenceVersions:listEvidenceHeadChanges`, so who moved a draft between active and retired, and why, is never lost. Review decisions now pin the exact evidence version they refer to (`review_decisions.evidence_version_hash`, outside the existing decision hash); PI acceptance and export-batch creation both refuse, fail closed, when a decision's pinned version no longer matches the draft's current one, so a later retirement, restoration, or write can never silently transfer an earlier decision to different content. When an author's later period set retires an already-decided record's periods, the retirement is now loud: a task event names the earlier and current versions and states plainly that the decision still refers to the earlier one. See the "Lifecycle: Retirement, Restoration, And Approval" section of `docs/development/evidence-versions.md`.
...
- Corrected location rulings across export batches and revision chains in `scripts/build_occupancy_dated_places.py`. Revisions resolve the source record’s identity through explicit task links, and successive accepted corrections apply to earlier pinned occupancy rows. Overlapping task snapshots use the latest dated export; ambiguous conflicts refuse the build. Regression tests cover partition and input-order independence, export attribution, nomination identity, successive corrections, overlapping snapshots, and invalid lineage.

- The public-map rebuild reads the reviewer's location ruling (handover 2026-09-07: "The master rebuild must read `location_outcome` and the accepted point. Not yet written."). `scripts/build_occupancy_dated_places.py`, the one script that turns an export into map features, now also reads `review_decisions.jsonl` (optional, so earlier exports still build). The latest accepting decision that carries a `location_outcome`, from a revision task holding the record's original point, rules for the whole site: every accepted occupancy row that sits on a task pin (`location_relation: same_as_task_point`, at either of the two points the ruling chose between) is drawn at the accepted point under `accept_moved_point`, at the original point under `keep_original_point`, and not at all under `uncertain`, since the export leaves the accepted point blank there. Rows with their own asserted location are untouched, and relocation lines follow the ruled points. Features gain `location_outcome` and, when another task's ruling moved them, `location_ruled_by_task_id`; the summary reports `location_uncertain_dropped`. The rule mirrors `acceptedPoint` in `convex/lib/locationOutcome.ts`; seven tests in `scripts/test_build_occupancy_dated_places.py` cover the three outcomes, latest-wins, ignored decisions and the pin test. Files: `scripts/build_occupancy_dated_places.py`, `scripts/test_build_occupancy_dated_places.py`, `docs/portal-location-and-occupancy-plan.md`.

- A build year alone no longer demands a denomination (Joseph Watts, 2026-09-08, St Brigid's Loburn: "it still requires me to enter information in the What was it, and did that change? section"). Reading the period cards back copied the first period's start date into the function chain's start whenever the chain had no date of its own, and the chain contract counts any start date as a touched chain, so a bare build year on the first card made the chain block mandatory and the submit refused with *Chain: Name the tradition or denomination at the start*. The read step now takes back only what was typed; the default moves to `chainWithDefaultStart`, a copy applied at the five places that validate, derive, preview or send a chain, and only once the chain counts as touched. A named denomination with no chain date still takes the period's start in the payload, and a chain date typed by hand still wins. **Gap question:** *Not sure* recorded no answer and only unhid the bounds panel, so it looked inert and a repaint hid the panel again; it now records `unsure` and the panel stays open until settled, and a new *Leave as one period* button beside *Record as two periods* settles it with the note *Gap not established: … recorded as one period* on the periods' uncertainty note. Test `chain-default-and-gap-unsure.test.cjs` reproduces the refusal on the old code and holds both fixes; guide sentence on the gap question and the optional denomination block; stamp `verification-map.js?v=20260909a`. Files: `apps/regions/nz/js/verification-map.js`, `apps/regions/nz/verification.html`, `apps/guides/ra.html`.

### 2026-09-07

- Location revision closed end to end (JB, 2026-09-07: "what if I want to revise the map location of a place"). **From a task:** any task's detail now carries *Move the pin — this record is mislocated* (signed in, rapid lane, not on a revision task itself), which opens the revise flow on the task's point with the wrong-location issue type preselected; `createIssueTask` takes `sourceTaskId`, records it as `issue_report.source_task_id`, and appends a `note_added` event to the revised task naming the revision. **Reviewer ruling:** review decisions gain `location_outcome` (`accept_moved_point`, `keep_original_point`, `uncertain`), offered as *Location ruling* on any revision with an original point and required when the pin moved and the decision is accepted for export (`convex/lib/locationOutcome.ts`, tested); the ruling joins the decision hash (absent keys drop out, so earlier rows still reproduce). **Export:** five trailing columns on `site_evidence_wide.csv`, `location_outcome`, `original_latitude`, `original_longitude`, `accepted_latitude`, `accepted_longitude`, filled from the task and the latest accepting decision; server list, portal mirror and template header changed together. The review map already drew the original point and a dashed line to the moved pin. Guide paragraph *Moving a mislocated record*; stamps `?v=20260907a`. Files: `convex/{model,schema,reviews,tasks,exports}.ts`, `convex/lib/{locationOutcome,wideEvidenceFields}.ts`, `apps/regions/nz/js/{verification-map,review-portal,wide-evidence-fields}.js`, `apps/regions/nz/{verification,review}.html`, `docs/templates/ra-historical-site-evidence/site_evidence_wide.csv`, `apps/guides/ra.html`.

- Link a new entry to a nearby record as probably the same place (Guy, 2026-09-07; PR #102). The proximity card's third choice keeps the pin and carries the link on both tasks' `nearby_site_refs` (`relation: "probable_same_place"`), with a warning check on the new task, a reciprocal ref and note on the earlier one, and *Probably the same place as* on both portals.

### 2026-09-05

- Phone panes and the contributor's position (JB, 2026-09-05, after Guy's first field day). **Panes:** below 900 px the entry pane and the map were a fixed half-and-half; now a divider bar (`#paneDivider`, `⠿ drag · entry ↕ map`) between them drags the split to one of three detents, 15, 50 or 85 percent for the entry, and neither pane ever leaves the screen (JB: "so the user is aware the other pane is still there"). The split is a CSS variable (`--entry-share`) on the shell's grid rows, animated, and the shell takes the dynamic viewport height (`100dvh`) so the browser toolbar no longer pushes the map off screen. The portal snaps the bar as the work changes: arming a pin gives the map 85, dropping the pin returns half (the pin is panned back into view if the smaller map lost it), confirming the pin or focusing a field gives the entry 85, and leaving the entry returns to the resting split, which a drag by hand sets and `localStorage["pow-pane-split"]` remembers; a double tap or Home returns to half, arrow keys step the detents. **Position:** *Use my location* in the pin locate card drops the pending pin at the device's fix (`navigator.geolocation`, high accuracy, 15 s timeout) at zoom 17 with a blue dot inside its accuracy ring, and the status says how rough the fix is; a crosshair button at the map's top left centres the map on the contributor without a pin; refusals, unavailability and timeouts each say what to do instead; the button and control appear only where the browser offers geolocation in a secure context. Test `pane-divider.test.cjs`; guide paragraph *On a phone*; stamp `verification-map.js?v=20260905b`. Files: `apps/regions/nz/verification.html`, `apps/regions/nz/js/verification-map.js`, `apps/guides/ra.html`.

- Guy's two field findings from Port Vila (email of 2026-09-05; JB could not reproduce on a desktop). **Zoom-out on Add a missing place:** the button forces the satellite basemap so the building shows, and `setBasemap` removed the outgoing tiles before adding the incoming; for that instant the only layer with a zoom ceiling was the country-scale unreviewed-dots tier (`maxZoom` 7, since the 2026-09-04 overview layer), so Leaflet's layer-derived zoom limit snapped the view to zoom 7, the whole country. It fired only when the basemap actually changed, which is why a contributor who had chosen Streets or Hybrid by hand saw it on every press and a tester left on the auto-switched satellite never did. Now the incoming tiles go on before the outgoing come off, in both `verification-map.js` and `review-map.js`; a jsdom + Leaflet 1.9.4 reproduction confirmed the clamp (18 → 7) and the fix (18 → 18). **Signed out after the photo gallery:** the Google ID token lived only in page memory, so a phone discarding the background tab and reloading it landed on the sign-in card with an empty form. The token (at most an hour, on the device that signed in) is now kept in `localStorage["powConvexAuth:v1"]`, restored by the next page load with its refresh timer, and the user named again by `restoreSession()` before the panel paints, on both portals; an expired, refused, or unknown token is dropped. Only the sign-out button now calls Google's `disableAutoSelect` (an expiry must not make the next automatic re-selection less likely). The guided form's typed values reach the device 400 ms after each edit (`powFormSnapshot:<country>:<task>`) and return when the task is next opened; the confirmed pin of a new place rides on the rapid draft while the entry is open and the entry resumes on it after a reload (`resumeRapidPinFromDevice`, in add mode after sign-in and on choosing Add places), leaving the pin flow by choice drops it. Tests `convex-task-client.test.cjs` and `basemap-and-drafts.test.cjs`; guide paragraph on drafts updated. Files: `apps/regions/nz/js/{verification-map,review-map,review-portal,convex-task-client}.js` (stamps `?v=20260905a`), `apps/regions/nz/{verification,review}.html`, `apps/guides/ra.html`. Still open: a session longer than an hour needs a real session exchange (Clerk or `@convex-dev/auth`) rather than the raw Google token.

### 2026-09-04

- Add the Mongolia research map (`apps/regions/mn/`; hub count 102): 22 annual register years 2004–2025 of aimag administrative counts of temples and churches from the National Statistics Office of Mongolia statbank (PxWeb table DT_NSO_2003_001V1, CC BY 4.0 under the 1212.mn terms of use), on the 22-unit geoBoundaries MNG ADM1 frame joined by the statbank's own region codes, with the Buddhism, Christianity, Islam and Other counts carried in the per-row composition and shown in the popup. Every surface states the buildings-not-people construct and that the 2010/2020 census religion product stays held pending corrected NSO per-aimag volumes. For every year the 22 units reconcile exactly to the statbank's national row for every type; the 467 type cells the statbank returns as unpublished (2004–2011 and 2013) are carried as absent, never zero.

- Add the Antigua and Barbuda national census-religion map for 1991, 2001, and 2011 (`apps/regions/ag/`; builder and manifest in pow-research). Three waves of printed counts on one national polygon under the small-country clause; the 2001 and 2011 printed lines differ from their printed totals by +3 and −12 and ship as documented discrepancies without repair (the Saint Lucia treatment); the 2011 parish cross-tabulation stays held pending the Statistics Division. Hub count 101.

- PR-P1, the PI acceptance layer's backend (JB rulings R-P1–R-P5 of 2026-09-04, all as recommended; brief `docs/development/pi-acceptance-layer-brief-2026-09-04.md`). A reviewer's `accepted_for_export` decision is now a recommendation: the task stays `reviewed` ("awaiting PI acceptance") until a principal investigator, role `pi` (JB, JW; granted through `adminUpsertUser`), records an acceptance. New append-only table `task_acceptances` (hashed like review decisions, `self_decided` flag per R-P1, `legacy` flag for the backfill); new task status `pi_accepted` between `reviewed` and `exported`; task events `pi_accepted` and `pi_returned` (with `acceptance_id`); mutation `acceptances:recordAcceptance({ taskId, outcome, note })` requiring the `pi` role, a `reviewed` task, a current accepted decision, a note of at least 8 characters, and never the PI's own submission; `returned` sends the task to `needs_review` (R-P5). `createExportBatch` now selects `pi_accepted` tasks only and refuses any named task a PI has not accepted (R-P4: curators and admins still batch); the batch records `included_acceptance_ids`. Queries `listTaskAcceptances` and `listPrincipalInvestigators`. Migration `acceptances:backfillLegacyExported` (idempotent, `dryRun` option) gives every already-exported task one legacy acceptance row attributed to a service user. Pure rules in `convex/lib/acceptance.ts` with `acceptance.node-test.mjs`. Client wrappers added (`convex-task-client.js?v=20260904a`); both portals count `pi_accepted` as validated for now, and the review queue's status filter gains *reviewed, awaiting PI acceptance* and *accepted by a PI*; the full ring reading (R-P3) and the PI acceptance panel are PR-P2. **Before merge:** deploy Convex (`npx convex deploy -y`), grant `pi` to JB and JW with `adminUpsertUser`, run the backfill.

- Points control: on or off, "period" retired, review page starts with the dots off (JB rulings, 2026-09-04, on my recommendation). "Period" filtered the OpenStreetMap places by their date tags, which most places lack, the same filter that produced Australia's sparse grey subset; history belongs to the occupancy slider driven by reviewed periods (`docs/portal-location-and-occupancy-plan.md` section 8, `docs/development/public-map-occupancy-slider-brief-2026-09-02.md`), not to OSM tags. The RA portal's control is now *Unreviewed places: on / off* (renamed from "Unvalidated"), on by default because the dots are the revise entry. The review page starts with the dots off, since the reviewer's work is the queue and the overview tier costs a few hundred kilobytes per tile at country scale; the control's own note says "Unreviewed places are off; turn them on here to see every open case in amber". The dated product now stands in only where leaflet.vectorgrid is absent. Files: `apps/regions/nz/js/{verification-map,review-map}.js` (`?v=20260904f`, `?v=20260904b`), `apps/regions/nz/{verification,review}.html`, `apps/guides/ra.html`.

- Revise from the activity chooser, and the guide recaptured as v7 (JB, 2026-09-04, after #89). **Bug:** signed in and still on the chooser (Assigned tasks / Add or revise places), an amber dot's *Revise this place* went nowhere: the chooser's CSS hides the detail panel and the pin cards, so the revise card and the pin flow rendered into hidden elements. Now a revise taken from the map switches the portal into the add activity first (`ensureAddModeForRevise`), and the popup's *Revise this place* goes straight into the pin flow on the record when signed in with the rapid lane (`reviseFromFeature`), rather than through a second card. The chooser and the add card now say "amber dot"; the RA guide's revise paragraph says the same. **Guide v7:** all ten flows recaptured on main after #89 (38 frames, every flow console-clean), uploaded to R2 `pow-guides-media` under the immutable `v7/` prefix (all 38 verified 200 on the public host), both guides repointed v6 → v7; every map frame now shows the amber open-case dots and the *not yet reviewed (open case)* legend row. `tools/guide-captures/capture.mjs` now expects the repo on `localhost:8000`, the origin the tile worker's CORS list allows, so the unreviewed dots draw in the frames (on 5182 the tiles were refused and the frames showed no dots). Files: `apps/regions/nz/js/verification-map.js` (`?v=20260904e`), `apps/regions/nz/verification.html`, `apps/guides/{ra,pi}.html`, `tools/guide-captures/{capture.mjs,README.md}`.

- Open cases in amber, every place on arrival, files in the form, cancel at stage two, and the review map (JB's afternoon sitting, 2026-09-04). **Ruling R-D1′ (JB):** any place of worship no reviewer has confirmed is an open case and wears amber; this overrides the morning's slate R-D1. The unreviewed dot is an amber `#f59e0b` disc with a white halo, task markers with no decision wear a thin amber `vm-unvalidated` ring, and both legends say so. **Every place shows:** JB saw Australia with "only a subset printed in grey" because a country with a dated product drew that product's subset in every mode and the tiles started at zoom 8; now `Unvalidated: all` draws every place from the shop front's tiles for every country (the public map's `places-overview` tier below zoom 8, the full `places` tier from 8 in), `period` keeps the dated subset, and every country arrives in `all`. The two tiers, the colours, and the click hit-test live in the new shared `apps/regions/nz/js/unvalidated-places.js` (test `unvalidated-places.test.cjs`). **Revise under an armed pin:** with Add a place pressed but no pin yet, a click on an existing place opens its popup with *Revise this place* first and *Add a new place here instead* second, rather than dropping the pin on it; a placed pin still owns the map click, and the popup's revise routes ask before discarding a pin or typed text. **Photos and PDFs in the form:** the add and revise form carries the file picker and caption itself (JB: "what I cannot do when adding or revising a place is adding an image or pdf"); the files upload against the task the save creates and the confirmation screen lists them, with its own picker for more. **Cancel at stage two:** the confirmation screen offers *Cancel this candidate* (or *revision*) with an inline yes/keep confirm; it takes the audited `withdrawEvidenceDraft` route, the candidate leaves the map, the list and the past-submissions count, and the audit history keeps it as withdrawn. **Review map:** `review.html` draws the RA portal's map above the review cards (`js/review-map.js`): the same basemaps, the same amber unreviewed places, the queue's submissions as validation-ring markers, a marker click opening the task's cards, a queue pick flying to the marker, and a moved pin drawn beside the record's original point. **Lifecycle explained:** the review card's Lifecycle panel and the review boundary now say what a lifecycle event is (a dated event in the place's life: worship beginning, ending, moving, or the building changing use), distinct from the target-year statuses. Two design tasks logged in `docs/portal-submission-review-plan.md`: the PI-only acceptance layer (only JB or JW may accept a design into the backend) and the country-switch layer (the heading still says India after panning to New Zealand). Files: `apps/regions/nz/js/{verification-map,unvalidated-places,review-map,review-portal}.js`, `apps/regions/nz/{verification,review}.html`, `docs/ui-style-guide.md`, `docs/development/ui-design-audit-2026-09-03.md`; stamps `verification-map.js?v=20260904d`, `review-portal.js?v=20260904b`.

- Guide screenshots recaptured after PR #85 (JB, 2026-09-04): all ten flows, 38 frames, published under the immutable `v6/` prefix of `pow-guides-media` (every key verified 200 on the public host); `apps/guides/ra.html` (30 figures) and `apps/guides/pi.html` (8) repointed from v5 to v6. Every map frame now shows the `⠿ move` grip on the unvalidated-places control; the add-mode sidebar frames show the Add places card rather than the retired My nominations panel. The superseded temporal plan's section 4 heading spells Woodberry correctly.

- Guide screenshots recaptured after PR-H3 (JB, 2026-09-04): all ten flows, 38 frames, published under the immutable `v5/` prefix of `pow-guides-media` (`--remote`; wrangler 4 writes to a local simulated bucket without it) and both guides repointed (`apps/guides/ra.html` 30 figures, `apps/guides/pi.html` 8). `tools/guide-captures/capture.mjs` follows the week's DOM: the submit flow names the tradition at the start of the function chain before submitting (PR-F refuses a chain without it); the skip flow finds the skip block by its button (five optional blocks now share `details.skip-form`); the pin-drop flow leaves assigned mode through the activity chooser, centres the map on the pin before the confirm gate (the pin must be in view), and drives the rapid current-observation form (`pinCurrentStatus`, the OSM quick-fill source, `#pinRapidSubmit`) with a `submitCurrentObservation` stub; a failed flow now leaves a `<flow>-FAIL.png` frame. Three pin-drop captions changed with the flow (chooser first; observation options; the submitted state).

- RA portal phone walkthrough fixes (JB, 2026-09-04, two findings from the first pass on a phone after PR-H3; `docs/development/ra-portal-walkthrough-2026-09-04-phone.md`). The unvalidated-places control on the map is movable: a grip bar (`⠿ move`) drags it by touch or mouse, it stays inside the map, a resize re-clamps it, a double tap sends it home, and the spot is kept on the device (`localStorage["pow-points-control-offset"]`). In Add or revise places the sidebar holds the selected work: the My work list stays with Assigned tasks; a bounced nomination's changes-requested alert is the only panel that remains; past nominations and revisions open from a `Revise a past submission (n)` button on the Add places card into a `My past submissions` list below the card (the section moved beneath the detail panel, `Hide` closes it, changing activity closes it); the signed-in line counts past submissions rather than My work in add mode; the `Show task list` link is gone from add mode, the card's own buttons being the way out. Principle recorded as R-W3. Test `phone-walkthrough-dom.test.cjs`; stamp `verification-map.js?v=20260904c`.

- PR-H3, the UI design pass across the RA portal, the review portal, and the public map shell (`docs/development/ui-design-audit-2026-09-03.md`; JB rulings R-D1, R-D2, R-D3 of 2026-09-04, all as recommended). One token system on the three surfaces (the audit's section-3 block: one action blue, one caution amber, the system font stack declared once, radii 6/12/16/999; `verification.html` 98 → 24 colour literals, all in `:root`; `review.html` 37 → 23, all in `:root`; the shell's chrome scoped under `.map-chrome` with the face at 0.85 alpha and `--pill-opacity: 1`, lifting pill text from about 3.4:1 to over 10:1 over the light basemap). Type: base 16 px in the working tools, meta and pills 14 px, nothing under 13 px, labels 600 and prose 400, headings in ink; the shell's seventeen font sizes collapse to six (attribution 10 px, then 13, 14, 16, 18, 22). Buttons: one filled primary per row (Submit for review filled, Save draft outlined, Submit unresolved note a text link; attachment buttons small outlines); the review portal's Confirm is primary, Override outlined, Reject outlined red with a filled-red submit; disabled controls keep readable text (grey face, muted ink, no opacity); hover and focus styles on every review-portal control. Markers and legend on the RA map: the validated ring takes the action blue, not-assessed is hollow white with a grey border, priority leaves colour and becomes a word in the task row, one legend on the map (fills, then rings, then the unvalidated dot) and the sidebar pill legend goes. **R-D1**: the unvalidated dot (tile-backed and geojson) is a slate `#64748b` disc with a white halo, overriding the 2026-09-02 amber; amber now means only uncertain, caution, or disputed. **R-D2**: on phones (≤ 640 px) a Contribute pill joins the bottom action row and opens a sheet with only the portal link, built by the same `contributeDecision()` as the wordmark panel so the zoom gate and country never disagree; the OSM editor route does not appear there. **R-D3**: the Review boundary instructions fold into a `<details>` open on first visit and closed after (`localStorage`, try/catch, defaults open). Layout: sidebar `minmax(360px, 440px)`, one-column field grids except short date pairs; the workflow chips take the section headings' names. Files: `apps/regions/nz/{verification,review}.html`, `apps/regions/nz/js/{verification-map,review-portal}.js`, `apps/shared/map-shell.css`, `apps/regions/_shared/{region-map.css,region-map.js}`, `apps/global/styles/maplibre-flat.css`, `apps/guides/ra.html` (grey dot copy), `docs/ui-style-guide.md`, and the 101 page HTMLs (stamps `region-map.js?v=20260904b`, `maplibre-flat.css?v=20260904a`, `region-map.css?v=20260904a`, `map-shell.css?v=20260904a`; `verification-map.js?v=20260904a`, `review-portal.js?v=20260904a`). Not in this PR: the guide screenshot recapture (they still show amber dots), `datamaps-switcher.js`'s own injected greys, and the Contribute panel's route order (audit B6, unruled).

### 2026-09-03

- Contribute from the shop front and every country page (JB rulings R-H1 to R-H3, 2026-09-03; PR-H2 in `docs/development/global-map-contribute-brief-2026-09-03.md`). The Contribute panel's project route no longer depends on a page's `wordmarkLinks`: a page whose config names no portal routes to the shared portal by its own country code (`../nz/verification.html?country=xx`, `review.html?country=xx`), so all 100 country pages gain "Submit evidence" and "Review evidence" with no page config edited; pages with their own links keep them. The global map resolves the country under the view centre when the panel opens (the page manifest first, then a new world outline manifest `apps/shared/data/world-outlines.json` from Natural Earth 1:110m for the 98 countries without a page, built by `scripts/build_world_outlines.py` with the page manifest's ring simplifier) and names it on the route ("Submit evidence to religionmap.org · Fiji"); below the data pill's offer zoom, or over open water, the row reads "Zoom to a country to submit evidence" with "About the project" beneath it. The panel renders on open only. Every place popup on the global map now carries "Revise this place", targeting the portal for the dot's own tile `country_code` (falling back to the country under the point); the portal accepts an iso code where the project code differs (`gb` opens `uk`). "Religion Map" becomes "religionmap.org" in the panel copy. The route decision is a pure function on the shared resolver (`RegionResolve.contributeRoutesFor`) with its own test, `apps/shared/region-resolve.test.cjs`, which also checks the world outlines. Stamps `20260904a` on `region-map.js` and `region-resolve.js` across all 101 pages.

- The evidence and review portal for every country (JB rulings R-H1 to R-H4, 2026-09-03; `docs/development/global-map-contribute-brief-2026-09-03.md`, PR-H1). A world country registry (`scripts/build_country_registry.py` → `apps/shared/data/country-registry.{json,js}` and `convex/lib/countryRegistry.generated.ts`; page countries from the region manifest, the rest from Natural Earth 1:110m; 198 countries) lets `verification.html?country=xx` and `review.html?country=xx` open for any country: no census years (the per-year grid, the target-year select, the periods preview and the assignment copy all say so and nothing derives until a year is set), no assigned work (the nomination batch stands in as the assignment batch, the chooser goes straight to add or revise), the registry camera. Hand-tuned countries keep their configs; an unknown code opens a neutral world view (`ZZ`) instead of New Zealand. The server's rapid-intake registry falls back to the generated world bounds (nominations accepted, approximate area allowed, no rapid assigned batch), so the Convex deploy precedes the merge. "Unvalidated" replaces "context dots" in the control, legend and guide (JB: nearly every place is one). A country without a dated product draws its unvalidated dots from the places vector tiles through Leaflet.VectorGrid 1.3.0 (pinned beside Leaflet), in *Unvalidated: all* from zoom 8; the tile click opens the same popup, task match and revise card as a dated dot, and a dot whose tile `country_code` is another country names that country and links to its portal. The unvalidated dots now show on arrival for everyone. R-H4: signed out, the dot's popup reads "Sign in to revise this place"; the click parks the place as the pending deep link, the sign-in card names it, and its card opens after sign-in, the same path as the public map's `?revise=1` link, which now also waits for sign-in in every mode. Tests: `apps/regions/nz/js/country-config.test.cjs`, `scripts/test_build_country_registry.py`, and the world-registry cases in `convex/lib/rapidEntry.node-test.mjs` (the old "Australia is refused" case is superseded). Stamps `20260903e` on the portal and review scripts.

- RA portal walkthrough fixes (JB, 2026-09-03, five findings from the first signed-in pass after PR-F; `docs/development/ra-portal-walkthrough-2026-09-03.md`). Entry focus: while a pin flow, guided form, periods pane, or history pane is open the sidebar shows only the entry — the task list, My work, filters, stats, the long help, and the assignment counts leave (a `body.entry-open` class the CSS reads; "Show task list" on the signed-in line brings them back), and the pin cards with the address search now sit directly under "Add a missing place"; a new entry releases the previously selected task. Move pin: with both coordinate boxes empty, "Move pin" takes the first search result still on screen, the result buttons read "Move pin here: …", and a confirmed location says so instead of ignoring the request. Current-use options carry a one-line helper each, and a fifth option, "Used to exist here, but no longer does" (`place_no_longer_exists` → existence absent, worship use not worship, action `no_building_present`, the guided form's own finding, relabelled "No building here now"; choosing either opens the periods block under "When did this place exist and was it used for worship?" with the first card's end waiting for a date; server vocabulary, derivation table, and consistency check extended). Buttons say what they do: "Back to map (keeps your draft)", "Cancel placement", "Discard this entry (deletes it)", "Back to the map", "Close without filing". Dates on revise: the period cards and the chain block now sit inside the rapid form (nominations and revisions; open for a revision) under "When was this place used for worship?", anchored to the observation date, with their own source and confidence by default (ticking "same source" takes the direct-observation note as their account, refused when short, never padded); they are validated before the observation goes and recorded right after it through `submitOccupancies` against the new parent, and if that second write fails the observation stands, the cards are kept, and *Add where and when* retries with them. **Ruling R-F1′ (JB, same day) built in the same change**: presence stays epistemic; a period of `annual` or `occasional` use derives present with `use_level: intermittent`, only `use_frequency: uncertain` derives uncertain (rule 11 keeps its id for that case); `use_level` (`regular` | `intermittent`) rides on `derived_target_year_states` and its segment firings, shows in the reviewer panel and is confirmed with the presence into `target_year_use_levels` on the parent, exports as `target_year_<Y>_use_level` beside the status (NZ wide row 125 columns; server list, portal mirror, and CSV template moved together), and the preview strip reads "2018 present, intermittent (inside the period, annual use)". Derivation version `occupancy_derivation_v3`: the first resubmission on a task returns its confirmed presence years to unconfirmed once. Convex before static. Files: `convex/lib/{rapidEntry,occupancies,wideEvidenceFields}.ts`, `convex/{model,schema,occupancies,exports}.ts`, `convex/lib/{rapidEntry,occupancies,functionChain,occupancyDerivationMirror,wideEvidenceFields}.node-test.mjs`, `apps/regions/nz/js/{verification-map,occupancy-contract,wide-evidence-fields,review-portal}.js` (stamps `20260903c`, `20260903b`, `20260903b`, `20260903b`), `apps/regions/nz/js/{assigned-periods-dom,portal-walkthrough-dom}.test.cjs`, `apps/regions/nz/{verification,review}.html`, `apps/guides/ra.html`, `docs/templates/ra-historical-site-evidence/site_evidence_wide.csv`, `docs/development/{function-chain-brief-2026-09-02,place-of-worship-definition-2026-09-03-v0.1.5,ra-portal-walkthrough-2026-09-03}.md`, `docs/api/convex-functions.md`, `.github/workflows/ci.yml`.

- PR-F of the ruled temporal plan — the function chain, desacralisation, and intermittent use (`docs/development/function-chain-brief-2026-09-02.md`, JB rulings R-F1–R-F4 of 2026-09-03). Under the period cards, in the guided form and the periods pane, one block asks "What was it, and did that change?": the tradition or denomination at the start as the source gives it, then *Add a change* from the seven ruled changes (denomination changed; shared use began; shared use ended; building rebuilt; use became intermittent; desacralised; worship resumed, ruling R-F5 of 2026-09-03, permitted only after a desacralisation and opening a period with the stated reopening as its start basis; other, with a required note), each with the periods' date widget (known, around, between, by). A state ends when the next change begins, so the RA never types an end date for a state; a desacralisation writes the enclosing period's end (closure stated, reason `desacralised`, which joins the end-reason vocabulary on server, mirror, import, and export); intermittent use splits the period at that date and sets the new period's frequency. Each period gains `use_frequency` (regular, the default; monthly; several times a year; annual; occasional; uncertain), and the ruled threshold (R-F1) is presence rule 11 `intermittent_use`: annual, occasional, and uncertain use covers a year without establishing a place of worship in use, so the year derives uncertain (`occupancy_derivation_v2`; revised the same day by R-F1′, see the walkthrough entry above; the derived-state hash consumes the frequency). The chain compiles to ordered `historical_claims` carrying `chain_id`, `chain_index`, `chain_change`, and `chain_label` with the periods' provenance, superseded as a set on resubmission across revision parents; the chain itself is stored typed on the parent draft (`function_chain`) and saved with the pending cards. A new derivation, `derived_target_year_functions`, names the state in force for each census year (`inside_state` → the label; `within_change_window` or `within_start_window` → uncertain with both candidates; before the first state or after a desacralisation → nothing), previewed in the form beside the presence sentence and shown in the reviewer's occupancy panel as a denomination line per year with Confirm / Override (label) / Reject; *Confirm all eligible* covers stated years inside one state (R-F4); only a reviewer action writes `target_year_denominations` and `target_year_denomination_basis` on the parent. The export gains `target_year_<Y>_denomination` and `_denomination_basis` (122 columns for NZ; server list, portal mirror, and template header moved together) and a `derived_target_year_functions.jsonl` file; the reviewer sees the chain's claims under the periods and the frequency beside each period. Definition v0.1.5 records the frequency threshold. Because the derivation input hash now consumes the frequency (derivation v2), the first resubmission of a set after this deploy returns every confirmed presence year of that parent to unconfirmed once, for the reviewer to confirm again. A resubmission without a chain supersedes the author's earlier chain and its derived denominations, so a revision cannot leave an earlier chain active. Tie tests hold the client mirror equal to the server for every function rule and assert the exact outputs of the Kohekohe fictional continuation (brief §4b; the record is §4a) (2013 present / Presbyterian; 2018 and 2023 uncertain under rule 11, no denomination after 2014); a stub-DOM regression adds a change through the real click handler, watches the chain write the cards, and compiles the atomic payload. Files: `convex/lib/{functionChain,occupancies,occupancyImport,wideEvidenceFields,validators}.ts`, `convex/lib/{functionChain,functionChainMirror,occupancies,occupancyImport,occupancyDerivationMirror,wideEvidenceFields}.node-test.mjs`, `convex/{evidence,exports,model,occupancies,schema}.ts`, `apps/regions/nz/js/{function-chain-contract,occupancy-contract,occupancy-review,review-portal,verification-map,wide-evidence-fields}.js` (stamps `verification-map.js?v=20260903b`, `occupancy-contract.js?v=20260903a`, `function-chain-contract.js?v=20260903a`, `wide-evidence-fields.js?v=20260903a`, `occupancy-review.js?v=20260903a`, `review-portal.js?v=20260903a`), `apps/regions/nz/js/assigned-periods-dom.test.cjs`, `apps/regions/nz/{verification,review}.html`, `apps/guides/ra.html`, `docs/templates/ra-historical-site-evidence/site_evidence_wide.csv`, `docs/development/place-of-worship-definition-2026-09-03-v0.1.5.md`, `docs/operational-definition.md`, `docs/api/convex-functions.md`.
- Contribute entry on the public map with a deep link into the portal (JB, 2026-09-03: RAs need a route from the public map into the submit and review lanes; the OSM editor link stays because editing OpenStreetMap is a different act from submitting evidence to this project). The wordmark's "fix OSM map" link and the per-page "Submit evidence" / "Review evidence" links become one "Contribute" entry whose panel offers two routes: "Improve OpenStreetMap" (the iD editor at the current view, as before) and, on pages with a portal, "Submit evidence to Religion Map" for project members with "Review evidence" beneath it; pages without a portal (the global map) show the OSM route and "About the project". The routes are read from the page's existing `wordmarkLinks` config, so no page config changed. A place's popup now opens with "Revise this place", which deep-links to the portal as `verification.html?revise=1&name=…&lat=…&lng=…&osm_type=…&osm_id=…`; the portal opens the same revise card a click on the place's amber dot opens (or the matched task), waiting for sign-in where the sidebar is gated and saying so on the sign-in card. `?report_issue=1` keeps opening the standalone issue form. The parser is a pure function with its own test. Files: `apps/regions/_shared/region-map.js` (stamp `20260903a` on all 101 pages), `apps/shared/map-shell.css`, `apps/regions/nz/js/verification-map.js` (stamp `20260903a`), `apps/regions/nz/js/deep-link-context.test.cjs`, `apps/regions/nz/verification.html`, `apps/guides/ra.html`, `.github/workflows/ci.yml`.

### 2026-09-02

- PR-E of the ruled temporal plan — census-year states derived from the periods entry on assigned tasks (`docs/development/assigned-task-periods-brief-2026-09-02.md`, JB rulings R-E1–R-E4 of 2026-09-02; revised on the PR #72 review). In the guided form the period cards replace the hand-set target-year grid as the temporal entry; the grid survives collapsed under "Set census-year statuses by hand" with a required reason, saved as `target_year_entry_reason` and shown to the reviewer, and the action prefills it only while open while the current-observation defaults still follow the action. An ordinary submission needs at least one period (a duplicate claim, the hand grid with its reason, or the unresolved-note route stand in). The cards are part of the draft: persisted on the device per country, user, and task, saved with every draft under an explicit typed and size-limited `pending_occupancy_cards` validator (cleared by the server once recorded), restored on reopen, and discarded with the form on task switch or skip; sign-out removes only the departing user's country-scoped keys. A preview strip states the derived census-year states with the rule in words and flags conflicts, on a verbatim client port of the presence rules held equal to the server by `convex/lib/occupancyDerivationMirror.node-test.mjs`, which also asserts the exact 2013/2018/2023 outputs of every worked case. After the first complete card the form asks once whether there was a spell with no worship here; "Yes" adds the later period, "Not sure" takes bounds (a latest-only stop is refused rather than forged) and never an invented date. The same prompt and preview render in the post-submission periods pane. A still-in-use card is anchored to the source date, not the day the page opened, and follows it when it changes. The periods rest on the evidence's own source and assessment by default, with the numeric assessment confidence mapped to the occupancy vocabulary and a blank or short source account refused rather than padded with invented wording. Final assigned-task submission is one idempotent Convex transaction across evidence, period rows, derivation, events, and the review transition; failure leaves the saved draft and cards editable, and the server enforces the required-period and hand-grid-reason rules. Reopening a task (a revision in particular) loads the RA's active periods into the cards, and the server supersedes the author's set on the task's earlier parents through bounded task-contributor-status indexes when the revision's set is recorded. Period validation uses the parent evidence date, with the recorded date only as a legacy fallback; a registered-mutation regression proves the atomic route refuses a period that extends beyond its parent's date. Vocabulary: the start basis gains `reopening_stated` with rule 2b `before_stated_reopening` (absent), so a stated reopening never has to be recorded as a founding; reviewer text, `Confirm all eligible`, the client mirror, and the collection schema move together. The capture backend and submit flow now exercise the current occupancy calls, and an actual-class stub-DOM regression covers the period form's render, visible preview and gap prompt, bounded Not sure interaction and exact worked derivation, edit-to-payload path, retry persistence, and user-scoped cleanup. Guide step-2 capture recaptured, visually checked, and published under the immutable `v4/open-and-verify/open-and-verify-02.png` media key. Files: `convex/{evidence,model,occupancies,reviews,schema}.ts`, `convex/lib/{assignedTaskPeriods,occupancies,occupancyImport,limits}.ts`, `convex/lib/{assignedTaskPeriods,evidenceSubmissionMutation,occupancies,occupancyImport,occupancyDerivationMirror}.node-test.mjs`, `apps/regions/nz/js/{verification-map,occupancy-contract,occupancy-review,review-portal}.js`, `apps/regions/nz/js/assigned-periods-dom.test.cjs` (stamps `20260902o`, `j`, `f`, `f`; client `20260902f`), `apps/regions/nz/js/occupancy-contract.test.cjs`, `apps/regions/nz/{verification,review}.html`, `apps/guides/ra.html`, `tools/guide-captures/{capture.mjs,manifest.json}`, `schemas/collection.v1.schema.json`, `docs/development/{assigned-task-periods,occupancy-build}-brief-2026-09-02.md`, `docs/api/convex-functions.md`.
- PR-G visibility pass (JB, 2026-09-02: the grey dots were still hard to see, and it was not obvious a clicked place could be revised). Context dots become one class on every basemap: an amber disc (`#f59e0b`, 5.5 px) with a dark hairline over a white halo, replacing the slate dot on Streets and the pale disc on imagery; the legend swatch follows. A recorded place's popup now opens with one full-width primary button, "Revise this place" (or "Reopen this place's issue"), with Street View, Open OSM, Copy coords, and OSM history beneath it; the revise pane shows the same primary button directly under its red heading, now 24 px, and the explanatory text sits below. Sidebar and guide wording say "amber dot" and "Revise this place". Task-marker popups keep their quiet "Report an issue" entry. Colour meaning recorded in `docs/ui-style-guide.md`. Files: `apps/regions/nz/js/verification-map.js` (stamp `20260902m`), `apps/regions/nz/verification.html`, `apps/guides/ra.html`, `docs/ui-style-guide.md`.
- One-click named sources in the rapid form (JB, 2026-09-02: checking OSM or Google Street View is the usual case). Under "How do you know this?" the rapid current-observation form (nominations in every enabled country, VU assigned tasks, revisions) gains two buttons, *OpenStreetMap record* and *Google Street View*: each switches the basis to a named public source and fills the title and link (the matched OSM object or the map at the pin; the Street View viewpoint at the point), then asks the RA for the observation or capture date; the status line restates that the OSM record is context for identity. The guided form already had *Use OSM URL* and *Use Street View URL*. Client only; no contract change. Files: `apps/regions/nz/js/verification-map.js` (stamp `20260902k`), `apps/regions/nz/verification.html`, `apps/guides/ra.html`.
- PR-D of the ruled temporal plan — Bob Woodberry's Vanuatu mission stations as a bulk occupancy import (`docs/development/woodberry-import-brief-2026-09-02.md`, plan §4). The bulk-import row gains the occupancy columns, one period per row with rows of a place sharing `source_locator` and numbered by `segment_index` (`convex/lib/occupancyImport.ts`, the one reading of the columns; the set is then validated by `assertOccupancySet` as an RA submission is). New admin-key ingest `batchImport:adminImportOccupancyBatch` (named service or admin actor) lands each accepted place submitted for review: a `needs_review` task carrying the first period's location assertion, a submitted guided draft with every target year `not_assessed`, its `site_occupancies` rows, and the derived census-year proposals as `derived_unconfirmed`, written through `recordOccupancySet`, the write route now shared with `submitOccupancies`; per-place reports, idempotent per source, caps of 100 places or 400 rows. Builder `scripts/build_vu_woodberry_import.py` reads the two hash-checked station workbooks from their pow-research quarantine and writes the import CSVs, run files, and a report naming every repair (Port Vila longitude), exclusion (Tikopia; six atlas rows without a usable point), and reading rule (stated foundings, same-year close/open as a status change, censoring as "after, last seen only", approximate areas with builder radii); nothing derived from the workbooks is committed. Build of 2026-09-02: 40 Catholic stations with 48 periods, 40 Protestant atlas stations with 40 periods, all 80 passing the server validators in a dry run; the live run awaits JB (Convex deploy first). Files: `convex/lib/occupancyImport.ts`, `convex/lib/occupancyImport.node-test.mjs`, `convex/batchImport.ts`, `convex/occupancies.ts`, `scripts/{build_vu_woodberry_import,test_build_vu_woodberry_import}.py`, `scripts/README.md`, `docs/portal-batch-import-and-corrections.md`, `docs/api/convex-functions.md`, `.github/workflows/ci.yml`.
- PR-C of the ruled temporal plan — the public-map slider on reviewed occupancies (`docs/development/public-map-occupancy-slider-brief-2026-09-02.md`, plan §3.5, follow-up F1). Builder: `scripts/build_occupancy_dated_places.py` merges one feature per reviewer-accepted occupancy (a `derived_year_locations` row citing it is `reviewer_confirmed`) into each country's `dated_places.geojson` beside the OSM date-tag features, with coordinates, `start_year`/`end_year`, the four window bounds, `radius_m`, `pow_site_id`, and a dashed-line `transition` feature between the two members of a relocation; `scripts/materialise_convex_export.py` now writes the four occupancy JSONL files. Renderer (`apps/regions/_shared/region-map.js`): an amber dashed ring where the census year falls inside a start or end window (or after a certain start when the end is undated, rule 10), a pale amber disc for approximate locations (true radius from zoom 13), a dashed transition line while a relocation is in progress; the alive predicate is unchanged and the solid dot now excludes window years; legend and popup name the reviewed tier only when a product carries it. F1: the 1600 typo guard becomes a per-country `DATE_FLOOR_YEARS` table (`convex/lib/countryYears.ts`, mirror `apps/regions/nz/js/date-floor.js`, tie test) read by `assertHistoricalClaim`, `assertOccupancySet`, and both client contracts; NZ/VU/AU stay at 1600, the Americas 1500, older-world countries 1000 (assumption for JB). The slider domain itself is unchanged (see brief §5). No product changes yet: no occupancy has been confirmed. CI now runs every portal contract test and the builder's unit tests. Files: `scripts/{build_occupancy_dated_places,test_build_occupancy_dated_places,materialise_convex_export}.py`, `apps/regions/_shared/region-map.js` (stamp `20260902a` on every region page), `apps/regions/nz/js/{date-floor,historical-claim-contract,occupancy-contract,verification-map}.js`, `apps/regions/nz/verification.html`, `convex/lib/{countryYears,historicalClaims,occupancies}.ts`, `convex/{historicalClaims,occupancies}.ts`, `convex/lib/countryYears.node-test.mjs`, `docs/development/temporal-place-layer.md`, `.github/workflows/ci.yml`.
- Second route into the occupancy pane (brief section 5, `docs/development/occupancy-build-brief-2026-09-02.md`): "Add where and when" now also appears on a recorded place's site card, both in the dot's popup (task markers and matched grey context dots) and on the recorded-observation card in Add or revise places, under the same rule as "Add known history" (the author of the submitted parent evidence, while the task awaits review). The RA's periods already recorded for that parent load into the cards via a new row-to-card inverse in the contract mirror (`segmentFromRow`, `provenanceFromRow`; a between with bounds Y−1..Y+1 restores as "around Y"), because a new submission replaces the earlier set; the pane says so, and "Done — back to this place" returns to the record. Files: `apps/regions/nz/js/{verification-map,occupancy-contract}.js`, `apps/regions/nz/js/occupancy-contract.test.cjs`, `apps/regions/nz/verification.html`, `apps/guides/ra.html`. Same PR, JB's layout notes (2026-09-02): on the revise pane, "Cancel and return to the map" now sits beside "Record evidence for this place", and the heading "Revise this place or report an issue" (pane title and the collapsed report form's summary) is red (`--danger`).
- Haloed markers on imagery basemaps (JB, 2026-09-02: grey and blue were hard to see on satellite). When Hybrid or Satellite is active the map container carries `basemap-imagery`: task markers gain an outer white halo with a dark hairline and shadow outside the unchanged validation ring, the dashed in-review and nomination borders gain the halo, and the canvas context dots switch from subordinate slate to a pale disc with a dark edge over a white halo disc. Streets is untouched. Files: `apps/regions/nz/js/verification-map.js` (`setBasemap`, `syncContextDots`; cache stamp `20260902h`), `apps/regions/nz/verification.html`, `apps/guides/ra.html`.
- PR-B′ of the ruled temporal plan — the occupancy lane (`docs/development/occupancy-build-brief-2026-09-02.md`, JB rulings R3 and R4 of 2026-09-02). An RA records "Where and when was this place used for worship?" as period cards (Began: known / between / by / unknown with basis; Ended: still in use / known / between / after / unknown with basis and reason; Location: same as the pin or placed on the map with its own radius), reached from the post-submission pane beside "Add known history"; "around Y" compiles to Y−1..Y+1 and is shown before save; a relocated period opens the next card at a new place. Server: `site_occupancies` rows (supersede-on-revise, basis asymmetry enforced, one place at a time), the ruled presence rules 1–9 plus rule 10 `end_unknown` (an addition, ratified by JB 2026-09-02), location rules L1–L5 (L4 imputes from the nearest dated period, radius unchanged), `derived_target_year_states` and `derived_year_locations` as `derived_unconfirmed` proposals, and the append-only `derived_state_events` trail. Reviewer portal: an occupancy panel with the drawn interval, per-year derived presence and location with the rule in words, Confirm / Override / Reject, "Confirm all eligible", the event trail, and a scatter of the period points; only a reviewer action writes `target_year_statuses` plus the new `target_year_basis` (`reviewer_confirmed_derivation` or `reviewer_override`) on the parent draft; a derived value contradicting an observed status raises `lifespan_conflicts_observation` and blocks confirm. Exports: the wide CSV gains per wave `_basis`, `_latitude`, `_longitude`, `_uncertainty_radius_m`, `_location_basis` (116 columns for NZ; server list, portal mirror, and template header moved together), rows overlay the draft's current statuses and confirmed locations, and the bundle gains four JSONL files. Files: `convex/lib/occupancies.ts`, `convex/occupancies.ts`, `convex/schema.ts`, `convex/model.ts`, `convex/exports.ts`, `convex/lib/{wideEvidenceFields,validators,rateLimits,historicalClaims}.ts`, `apps/regions/nz/js/{occupancy-contract,occupancy-review,verification-map,review-portal,convex-task-client,wide-evidence-fields}.js`, `apps/regions/nz/{verification,review}.html`, `apps/guides/ra.html`, `docs/templates/ra-historical-site-evidence/site_evidence_wide.csv`, `docs/api/convex-functions.md`.
- OSM edit history as context (JB, 2026-09-02). A grey dot's popup gains an on-demand "OSM history" button, the revise pane shows the timeline automatically, and the reviewer pane shows it for any task matched to an OSM object. The new `osm-history.js` fetches the object's version history from the public OSM API (CORS-allowed for religionmap.org, one request per object per session), reduces it to dated changes of the evidential tags (amenity, disused/was amenity, name, religion, denomination, building, start/end/opening dates), and states the first place-of-worship tagging as earliest OSM evidence, not a founding date, plus retagging and deletion flags. Editor usernames are deliberately omitted; changeset ids and a link to the openstreetmap.org history page remain. Files: `apps/regions/nz/js/{osm-history,verification-map,review-portal}.js`, `apps/regions/nz/js/osm-history.test.cjs`, `apps/regions/nz/{verification,review}.html`, `apps/guides/ra.html`.
- PR-B0 of the ruled temporal plan: the `site_evidence_wide.csv` column list is now one shared, server-validated constant (`convex/lib/wideEvidenceFields.ts`, a function of the country's census waves). Before, `exports.ts` took the header from the first eligible draft's stored field list, so drafts saved under a different list silently lost columns. Now the exporter builds the header from the shared list and places every row by column name (a draft with a divergent stored list loses nothing; the manifest counts such drafts as `field_list_mismatch_count`), and the draft save, spreadsheet import, and submission routes refuse a `generated_wide_row` whose field list differs from the shared list with a "reload the portal" message. The portal builds its list from a verbatim mirror (`apps/regions/nz/js/wide-evidence-fields.js`); `wideEvidenceFields.node-test.mjs` holds the mirror, the server list, and the RA template header equal for every shipped country. Files: `convex/lib/wideEvidenceFields.ts`, `convex/exports.ts`, `convex/evidence.ts`, `apps/regions/nz/js/{wide-evidence-fields,verification-map}.js`, `apps/regions/nz/verification.html`.
- Revising an existing map record now collects the same evidence as adding a new place (JB, 2026-09-02: "shouldn't we allow the same information boxes, including uploads?"). From a grey context dot, "Record evidence for this place" runs the pin flow with the record as prefill — the pin starts on the record's point and may be moved or widened to an area with a radius — then the rapid current-use form, attachments as Step 2 of 2, and known history. The task is created (or an open issue for the record claimed) in the country's `ra-issues` batch assigned to the RA, the observation submits against it, and it reaches the review queue with the record's original point, the RA's location assertion, and the "what is wrong or new" framing shown in a new reviewer "Issue report" panel. The RA's revisions appear in their nominations panel. "Just flag it" keeps the note-only path, where "submitted in error" also stays. Server: `createIssueTask` gains `locationAssertion`, `originalLatitude/Longitude`, and `assignToReporter`; the rapid path accepts observations on the issue batch (`issueBatchId`). Files: `convex/tasks.ts`, `convex/rapidEntry.ts`, `convex/lib/rapidEntry.ts`, `apps/regions/nz/js/{verification-map,review-portal}.js`, `apps/regions/nz/{verification,review}.html`, `apps/guides/ra.html`.
...
- Generalised rapid current-observation entry from Vanuatu-only to a closed per-country intake registry (VU and NZ today; JB ruling). The canonical mutation is `rapidEntry:submitCurrentObservation` (the Vanuatu-named alias stays registered); intake bounds live in `COUNTRY_INTAKE_BOUNDS` with antimeridian-correct handling and input longitude normalisation for NZ; existing-task rapid submissions are restricted server-side to the country's manual nomination batch unless the country's assigned batch is rapid-designed (VU), and per-country location-assertion modes are enforced server-side (VU building-level only, portal aligned). NZ assigned tasks keep the guided form; only nominations go rapid, and a nomination already holding a guided draft stays on the guided form. Files: `convex/rapidEntry.ts`, `convex/lib/rapidEntry.ts`, `convex/lib/locationAssertions.ts`, `convex/tasks.ts`, `apps/regions/nz/js/{verification-map.js,convex-task-client.js}`.
- Cut portal helper prose to essentials ("clear buttons, not words", JB ruling): 11 blocks removed, 7 shortened, 2 collapsed behind details; the blanket "do not enter private or sensitive data" line is retained on the signed-in portal. Updated `apps/guides/ra.html` to the new flows (chooser, separation, hybrid basemap, pin mechanics, attachments).

### 2026-08-28

- Replaced the keyless Carto basemap with OpenStreetMap standard tiles in the verification portal and in the shared region-map runtime's keyless fallback. Carto's keyless endpoint now watermarks every tile with "API KEY REQUIRED", which made building-accurate pin placement unverifiable in the RA portal. OSM standard tiles ship no secret, render building footprints cleanly at placement zooms, and the invited-RA portal traffic sits well inside the OSMF tile usage policy; the MapTiler alternative would couple the portal to a paid plan under fee review. The shared runtime keeps its internal `carto-light` id so saved basemap preferences and the MapTiler-failure fallback continue to work. Files: `apps/regions/nz/js/verification-map.js` (cache pin `?v=20260828e`), `apps/regions/nz/verification.html`, `apps/regions/_shared/region-map.js` (cache pin `?v=20260828a`, 101 pages), `ROADMAP.md`.
- Closed two review findings on the uncertain-location and historical-claim lanes before merge. An approximate-area nomination no longer exports to `site_evidence_wide.csv` as an exact manually matched point: the wide row's `geocoding_basis` becomes `described_locality` (radius up to 2 km) or `regional_only` (larger), both added to the controlled vocabulary, and the source notes record the uncertainty radius and any source wording. In the historical-claim form, ticking "state remains open" now clears the latest supported date it disables, so the validator can no longer reject a value the RA cannot edit. Files: `apps/regions/nz/js/verification-map.js` (cache pin `?v=20260828d`), `apps/regions/nz/verification.html`, `docs/templates/ra-historical-site-evidence/controlled_vocabularies.csv`.
- Added contributor-confirmed uncertain location evidence to the shared country nomination portal without weakening the Vanuatu rapid-current contract. Ordinary nominations may retain either an identified building point or an approximate-area centre with an uncertainty radius, location basis, confidence, and the source or informant wording. The reviewer sees the same versioned assertion, which also travels in `tasks.jsonl`. Approximate centres remain provisional evidence and never become accepted site points without human review. File upload remains disabled. A strengthened planning invariant requires every later permitted image, PDF, spreadsheet, GeoJSON, or other uploaded file to enter private project-controlled quarantine and remain private unless a new explicit publication decision is made.
- Added optional, repeatable historical-claim entry to all 23 configured country verification portals through their shared New Zealand portal runtime. After submitting rapid or guided evidence, an RA can record one structure event, worship-function event or state, denomination or affiliation state, leadership state, shared-use state, or other historical claim at a time. The same action remains available while the parent evidence awaits review.
- Each `historical_claim_v1` record retains source wording or a confirmed dictated account, supported partial date bounds or an explanation of unresolved dates, an evidence reference date, confidence and its basis, source details, and sensitivity. The authenticated contract remains author-bound, user-idempotent, rate-limited, and restricted to active submitted evidence. It does not infer dates, fill target-year states, alter the parent evidence, or write to the master or public map. Reviewers inspect each claim separately, and the export files preserve `historical_claims.jsonl`. Files: `convex/{historicalClaims.ts,model.ts,schema.ts,exports.ts}`, `convex/lib/{historicalClaims.ts,rateLimits.ts,validators.ts}`, `apps/regions/nz/{verification.html,js/historical-claim-contract.js,js/verification-map.js,js/review-portal.js,js/convex-task-client.js}`, tests, export materialiser, FAQ, and API/portal documentation.
- Closed two review findings on the Vanuatu rapid-entry lane before merge. A newly nominated rapid candidate now registers in the portal after submission: the assignment view also queries the manual nomination batch, so the nomination stays on the map, in the task list, in My work, and visible to the pin-drop proximity check, including after a reload — previously it vanished from the interface the moment the confirmation was dismissed, with no route back for the author correction the server supports. Server-side, a generic evidence submission can no longer supersede an active rapid observation on the same task: `supersedeOtherActiveDrafts` now refuses rapid drafts, so the rapid correction path remains the only way to replace one, completing the single-write-route invariant. Files: `apps/regions/nz/js/verification-map.js` (cache pin `?v=20260828a`), `apps/regions/nz/verification.html`, `convex/evidence.ts`.

### 2026-08-27

- Hardened the Vanuatu rapid-entry contract. Every general write route (`saveEvidenceDraft`, `importSubmittedEvidenceDrafts`, `submitEvidenceDraft`, `submitUnresolvedNote`, `reviseEvidenceDraft`) now rejects `rapid_current_v1` drafts and rapid fields, so only `rapidEntry:submitVanuatuCurrentObservation` can create or replace a rapid observation. The server enforces the exact mapping from the observer's current-status answer to `action`, `existence_status`, and `worship_use_status`, with an exhaustive test table. Rapid observations are ineligible for external AI review by contract. The RA correction path no longer routes through the generic revision clone: the original observer submits a corrected observation from the read-only task view, the earlier record is retained as superseded, and reviewers cannot rewrite it. First-entry wording is now one controlled answer, the evidence fields, and one `Submit for review` action; the assignment placeholder copy is rapid- and country-aware. On phone widths in assignment mode the sidebar now scrolls within the upper half of the screen instead of pushing the map ~1,700px below the fold, and the static validation-ring legend no longer covers the phone map. Files: `convex/{evidence.ts,rapidEntry.ts,claudeReviews.ts}`, `convex/lib/{rapidEntry.ts,limits.ts,sensitivity.ts}` and their node tests, `apps/regions/nz/{verification.html,js/verification-map.js}`, `tsconfig.json`, API documentation.
- Clarified version 0.1.2 of the operational definition. The GitHub landing page now lists the three jointly required criteria below the linked definition, and the canonical specification gives each criterion its complete label. Earlier versions remain complete as dated records. Files: `README.md`, `docs/operational-definition.md`, and `docs/development/place-of-worship-definition-2026-08-27-v0.1.2.md`.
- Placed the canonical operational-definition link in a prominent definition block at the top of the GitHub landing page. Published version 0.1.1 as a focused public specification of the rule, its three criteria, the time-indexed analytical unit, and evidence handling. Earlier versions remain complete as dated records. Files: `README.md`, `FAQ.md`, `docs/operational-definition.md`, and `docs/development/place-of-worship-definition-2026-08-27-v0.1.1.md`.
- Added JB to the public audience routing and created a public role page with the stable scientific, governance, and review entry points. Person-specific assignments and progress remain in the private research tier. Clarified the public README's project description and contribution route. Files: `README.md`, `docs/people/README.md`, and `docs/people/jb/README.md`.
- Restored the complete version 0.0.1 discussion draft dated 2026-08-22 after its supersession notice had replaced the substantive text. Dated definitions and discussion drafts are immutable scientific records. Later versions may add a status notice and cross-reference while retaining the earlier account in full. Files: `docs/development/place-of-worship-definition-2026-08-22.md`, `docs/operational-definition.md`, and `AGENTS.md`.
- Adopted and synchronised a working operational definition of a place of worship: the time-indexed worship-function state at a reproducibly mappable site for which source evidence supports recurring religious worship by or for a community. The stable `docs/operational-definition.md` entry point opens with a concise summary and retains the complete rule, while dated snapshots preserve the exact definition used by earlier studies. The decision note records its conceptual lineage, the distinction between candidate evidence and accepted states, its consequences for OSM change measurement, and the boundary questions that remain open. The 2026-08-22 discussion draft now points readers to the adopted version. Files: `README.md`, `docs/operational-definition.md`, `docs/development/place-of-worship-definition-2026-08-27.md`, `docs/development/place-of-worship-definition-2026-08-22.md`, `LEXICON.md`, `FAQ.md`, `schemas/site.schema.json`, and `AGENTS.md`.

### 2026-08-26

- Added a security-first Vanuatu rapid-entry path for invited RAs. One map-first form distinguishes physical existence from worship use at an exact observation date, leaves 1989, 1999, 2009, and 2020 unassessed, and sends each entry to the existing human review queue. The authenticated Convex mutation is atomic, user-idempotent, role- and ownership-checked, Vanuatu-bounded, size-limited, and transactionally rate-limited; it derives provisional workflow fields server-side and cannot write to the master or public map. Reviewers can inspect the exact current-status choice and evidence basis. Present observations remain in the JSONL evidence trail and do not enter the target-year wide CSV without separate target-year evidence. Images and audio remain disabled and outside Convex. Files: `apps/regions/nz/{verification.html,js/verification-map.js,js/rapid-entry-contract.js,js/convex-task-client.js,js/review-portal.js}`, `convex/{rapidEntry.ts,convex.config.ts,model.ts,schema.ts}`, `convex/lib/{rapidEntry.ts,rateLimits.ts,limits.ts}`, `.github/workflows/ci.yml`, portal and API documentation.
...
- Narrowed the external-AI exclusion added with the guided evidence intake: only guided drafts that carry a denomination claim (the denomination action, or a raw label) stay outside the automated AI batch-review lane. Ordinary guided evidence about existence, dates, and location continues to receive bounded source checks; Vanuatu and privacy-flagged evidence remain excluded by the sensitivity gate. Files: `convex/lib/sensitivity.ts`, `convex/claudeReviews.ts`.
...
- RA portal and workbench ergonomics round (review: `docs/development/ra-portal-ergonomics-2026-08-14.md`, committed with the change). The portal's evidence form is now protected against silent loss of typed work: a dirty flag guards task switches behind a confirm and tab close behind `beforeunload`, while same-task programmatic rebuilds (target-year change, refresh, re-sign-in after an expired session) snapshot the typed values and reapply them after the rebuild — reapplied values count as touched, so action defaults never overwrite them. Save/Submit lock during the async save, matching the file's other write paths. My work gains a "Show all N" toggle past the 25-item slice; the optional address, lifecycle, and related-ids blocks collapse into `<details>` (auto-opening when a draft or snapshot carries values, or when the action implies closure); "Refresh task list" now disables during flight and reports "Task list refreshed — N available, M in My work" or the actual error. The review portal gains an "Open next in queue" button on the decision-recorded pane (reviewer-clicked, never auto-selected; follows the pre-reload queue order), a bottom-sticky decision panel capped at 60vh (normal flow at phone widths), and Cmd/Ctrl+Enter to record. The workbench gains the same dirty-guard-with-confirm on task, view, and country switches plus keyboard-operable task rows. Deliberately deferred to the C2 Convex-cutover lane: reconciling the portal and workbench vocabularies (finding 5) and guarding the nominate flow's editors. Files: `apps/regions/nz/js/verification-map.js` (pin `?v=20260814b`), `apps/regions/nz/verification.html`, `apps/regions/nz/js/review-portal.js` (pin `?v=20260814b`), `apps/regions/nz/review.html`, `apps/workbench/src/{App,screens/EvidenceForm,screens/TaskList}.tsx`.
- Converged the global map onto the shared region-map runtime: `apps/global/index.html` shrinks from 2,951 lines to a 62-line `REGION_CONFIG` consumer of `apps/regions/_shared/region-map.js`, following the NZ/VU extraction playbook. The runtime gains config-keyed unions (HAS_CENSUS and HANDOFF_HOME absence gates, path bases, keyPlacement, geocode-absent branches, the four-state offer pill with the global hint mode) that verification traced as inert on all 100 country pages; injected chrome is byte-identical. Full adjudication in `apps/regions/_shared/DRIFT-REPORT-GLOBAL.md`; an ultra review then confirmed zero unexplained drift and its top findings shipped with the branch: the MapLibre guard now runs after the chrome injection, so a blocked or failed CDN load leaves the wordmark, onboarding, and navigation on screen instead of a blank page (restoring the old global page's static-chrome resilience); and the pill's census-toggle mode now requires census data behind it (`HANDOFF_HOME && HAS_CENSUS`), so a future dots-only country page falls through to the global pill semantics instead of rendering a dead toggle. Files: `apps/global/index.html`, `apps/regions/_shared/region-map.js`, `apps/regions/_shared/DRIFT-REPORT-GLOBAL.md` (new); cache pin `?v=20260814a`, 101 pages.
...
- The overlay registry lands, phase 1 (JB opened the lane this sitting; design docs/development/multi-domain-overlay-design.md, build record overlay-registry-phase1-2026-07-17.md): the runtime's census machinery now derives from a domain registry — `RC.overlays` when a page declares domains, else a permanent legacy shim that builds the single religion domain from the existing config keys, so every page derives byte-identical values and never migrates. `setOverlayDomain` switches domains end to end (constants rebind, level cache clears, defaults snap with year carry-over), a domain select renders only when a page declares two or more domains, place dots and the Points row become religion-domain furniture that hide under any other domain, and a loaded product whose declared `domain` disagrees with its config slot fails loudly. No page changes behaviour today; the machinery exists for the VU language pilot (phase 2, waits on Guy's tables). Browser-gated: NZ (full paint, two levels, five metrics), US (fifteen timeline eras 1850-2020, overridden labels), VU (pulotu source switch round-trips through the new machinery), VC (eleven ordered share metrics), BT (single place_count); domain select hidden on all; console clean. The product-indicators metric registry is deliberately deferred to a `metricsFromProduct` flag for non-legacy domains — switching the label source of truth silently would change visible text on pages whose products already carry `indicators`. File: `apps/regions/_shared/region-map.js` (cache pin `?v=20260717e`, 100 pages).
- Pakistan's pages now carry the citation PBS requested in its written reply of 14 July 2026: "Pakistan Bureau of Statistics (PBS), Population and Housing Census 2023" replaces "Census of Pakistan 2023" on the map page, the overview, and the area-summary product, and the "while written confirmation is sought" hedges retire in favour of the confirmation record — the ship-with-attribution posture (project-lead ruling 2026-07-11) is now confirmed by the source. Region catalogue rebuilt for the summary's new hash. Files: `apps/regions/pk/*`, `apps/shared/data/region-catalog.json`.
- The site moves to religionmap.org (JB ruling): the apex is the canonical address, www redirects to it, and every alternate domain 301s path-preserving so deep links keep working. Cloudflare DNS on the new zone points at GitHub Pages (four apex A records, four AAAA, www CNAME, DNS-only for certificate issuance); a six-line `domain-redirect` Worker (`tools/domain-redirect/`) owns powmap.org and www.powmap.org as custom domains and redirects with path and query intact (curl-verified: `powmap.org/apps/regions/nz/?x=1` → `religionmap.org/apps/regions/nz/?x=1`). Both new zones carry `v=spf1 -all` and reject DMARC — no mail will ever legitimately originate from them. Outstanding: placesmap.org (the old address) is not on Cloudflare — its DNS sits at the registrar's parking nameservers — so its redirect needs the zone added to Cloudflare first; until then old placesmap links 404. Files: `CNAME`, `tools/domain-redirect/`.
- The country-search filter no longer makes iOS zoom the page on focus (JB report): Safari auto-zooms whenever a focused input's type is under 16px, and the filter's 13px type triggered that on every tap, throwing the panel off its top pin. Touch screens now get full 16px type in the filter — the documented threshold that stops the zoom at the source — while desktop keeps the 13px design. File: `apps/shared/datamaps-switcher.js` (cache pin `?v=20260717d`, 101 pages).
- The set-north badge moves to the right edge on every map surface (JB request): on desktop it seats directly below the top-right wordmark pill — the fix/review options on the data maps — and on phones, where the wordmark hides, it takes the top-right corner itself. The badge remains compass-on-demand (visible only while the map is rotated off north), and the corner grammar in the shared sheet records the amendment. Browser-verified on the NZ data map and the global map at desktop and phone widths: badge at `top: 46px` under the wordmark on desktop, at the corner on phones, clear of the wordmark in both. Files: `apps/regions/_shared/region-map.js`, `apps/global/index.html`, `apps/global/styles/maplibre-flat.css`, `apps/shared/map-shell.css` (comment only); cache pin `?v=20260717c`, 101 pages.
- The country switcher panel now places itself in visual-viewport coordinates on touch screens (JB report of the panel dropping out of view): fixed positioning anchors to the layout viewport, so under page pinch-zoom the old `top: 12px` could sit entirely outside the visible area. The panel now translates into whatever part of the page is on screen, refits on the visual viewport's scroll channel (pinch-panning moves the viewport without resizing it), and sets its width inline on both pointer branches so a pointer-mode change never inherits a stale width. The top-pin itself shipped this morning (`?v=20260717b`) and is confirmed working — search row first, one-option list included; a phone that still shows the panel at the bottom is serving the pre-09:00 deploy. Browser-verified with a coarse-pointer simulation at 375 px down to a one-country list. File: `apps/shared/datamaps-switcher.js` (cache pin `?v=20260717c`, 101 pages).
- Page pinch-zoom gains a recovery path on both map runtimes (JB report: zooming in on a phone loses the menus). Page zoom is distinct from the map's own zoom — it scales the layout viewport and strands the fixed chrome off-screen — so two recoveries now ride the reset button: while the page is zoomed the button follows the visible bottom-left corner via `visualViewport`, and reset itself re-clamps the page scale to 1 by briefly pinning the viewport meta before restoring it, keeping pinch zoom available as an accessibility affordance. Reset already restored pills, dock, census layer, and view; it now also restores the page's own scale. Files: `apps/regions/_shared/region-map.js`, `apps/global/index.html` (cache pin `?v=20260717c`).
- The switcher's prefetch budget now spends estimated transfer bytes instead of raw file sizes (tribunal item 1): the catalogue's payload records gain a zlib-6 `gzip_bytes` estimate (`scripts/build_region_catalog.py`, contract now `["path","bytes","sha256","gzip_bytes"]`), and the 1 MiB warming budget admits 97 of 100 countries' boundary+summary pairs where raw-byte accounting admitted 67. Hover, focus or touch on a switcher row — and a border-handoff offer on screen — now warm the boundary alongside the page and summary, so the destination's census layer opens from cache; oversized payloads still degrade to a partial warm (browser-verified: Ireland warms as a full pair, the US seats its page shell and skips the payloads the budget cannot hold). Files: `apps/shared/datamaps-switcher.js`, `apps/shared/data/region-catalog.json`, `scripts/build_region_catalog.py` (cache pin `?v=20260717b`, 101 pages).
- The denomination key now follows the place dots (tribunal item 5, JB prompt): choosing "Points: off" hides the key pill on phones — phone chrome earns its place — and disables it with a "Place dots are off" title on desktop, where hiding would break rediscovery; any mode with dots restores it at once through the same paint path (`syncPlaceDotEra`). Browser-verified on the phone breakpoint (hide on off, restore on all); the desktop disable rides the counts panel's existing render pass. File: `apps/regions/_shared/region-map.js` (cache pin `?v=20260717b`, 100 pages).
- The country switcher's filter now ranks like an alphabet jump (JB request): names beginning with the typed letters lead ("ne" → Nepal, Netherlands, New Zealand), names with a later word beginning with them follow ("z" still finds New Zealand), and mere substring matches trail; the full list also sorts by name rather than country code, so Australia now precedes Austria as the alphabet demands. Accent folding is unchanged — "cote" still finds Côte d'Ivoire. Browser-verified on the NZ page. File: `apps/shared/datamaps-switcher.js` (cache pin `?v=20260717a`, 101 pages).
- On touch screens the country switcher panel now pins to the top of the viewport, search row first (JB report): anchored to the bottom pill, the panel used to shrink down into the soft keyboard as the filtered list got shorter, hiding the search box at exactly the moment the filter was working. The height cap follows `visualViewport` (iOS keyboards resize only that, never the window), and a visual-viewport resize re-fits the panel instead of closing it. Desktop placement is unchanged. Browser-verified with a coarse-pointer simulation down to a one-country list. File: `apps/shared/datamaps-switcher.js`.
- The data maps pill's caret zone now says what it opens (JB report that the arrow was not sufficient): a visible "Countries ▴" label sits where the bare glyph was, on the global map and all 100 country pages, so the pill reads "● NZ Data | Countries ▴" at home and "France Data → | Countries ▴" mid-offer. The glyph turns while the panel is open, driven by the aria-expanded state the switcher already maintains. Tribunal-ratified (Fable 5 and GPT 5.6 both independently recommended a visible word on the caret zone; both rejected icon-only affordances). Files: `apps/regions/_shared/region-map.js`, `apps/global/index.html`, `apps/global/styles/maplibre-flat.css` (cache pins `?v=20260717a`).
- The search dock is now draggable on every map surface and no longer overlaps the bottom pill row on phones (JB report). The dock joins the pill drag roster on the global map and all 100 country pages — drag by the panel chrome, double-tap sends it home; drags never start from the input or the selects, and the width-only resize guard already in the drag system keeps the phone keyboard from snapping a moved dock home. The phone offset also rises from 64 px to 92 px: the bottom band wraps to two pill rows since the Data Maps pill joined it, and the old offset overlapped the row by a measured 20 px at 430 px width. Browser-gated at phone width and on the desktop global surface: the open dock clears the pill row by 8 px, a touch drag moves it and edge-snaps on release, and a mouse drag moves it on the global map. Files: `apps/regions/_shared/region-map.js`, `apps/global/index.html`, `apps/global/styles/maplibre-flat.css` (cache pin `?v=20260716j`, 101 pages).
- Interface copy now treats data as plural (JB house style): "About these data" heads the census evidence drawer, pending notes read "religious-affiliation data are pending", and the verification map's unconfigured-country error reads "task data are". The per-country product notes that define the no-data state ("No-data is never treated as zero") keep their ratified wording — there "no-data" names the state itself. Files: `apps/regions/_shared/region-map.js`, `apps/regions/nz/js/verification-map.js`.
- The global map now warms a country data map while the visitor is still reading it (JB request), so the likeliest handoff opens from cache. The warm target is the visitor's own country when a permissionless signal names one that has a data map: the device timezone first (a new generated index, `apps/shared/data/tz-index.json`, maps 222 IANA zone names to the 100 catalogue codes in 5.4 KB), then an explicit locale region subtag (ranked second because browsers often report en-US regardless of place; bare "en" never invents a region). New Zealand is the fallback and, when the guess lands elsewhere, the top-up. The warm rides the switcher's existing speculative-transfer machinery — the 1 MB budget, the data-saver and 2G guards, and the idle scheduling all apply — and queues each country's page, default area summary, and boundary file, so oversize payloads simply drop out: a US visitor warms the US page shell (the 20 MB county summary never queues) plus the full ~650 KB New Zealand map, a UK visitor warms the UK page and summary, and an NZ visitor warms NZ alone with a third of the budget left for hover warming. The site itself has no geolocation signal at rest (GitHub Pages exposes no visitor-country header, and prompting for browser geolocation on load would be hostile), which is why the device's own settings carry the guess. Country pages keep their neighbour warming and gain no home warm; a `dm-home` sessionStorage override supports QA. The catalogue rebuild also refreshed every page's `html_bytes`, stale by 29 bytes since the wordmark-link retirement. Browser-gated locally: Pacific/Auckland resolves to the full NZ warm; a forced `dm-home=us` warms the US page shell plus the NZ top-up; and a real NZ page load after the warm reads both census files from cache (transferSize 0) inside the live page runtime. Files: `apps/shared/datamaps-switcher.js` (cache pin `?v=20260716j`, 100 pages and the global surface), `scripts/build_region_catalog.py`, `apps/shared/data/tz-index.json` (new), `apps/shared/data/region-catalog.json`.
- Phones now hide the whole top-right wordmark pill (JB ruling: the phone is an exploration tool, and the pill's contents — evidence submission, the OSM editor, the theme select, the repo link — are desktop affordances; cross-map travel already rides the bottom-bar Data Maps pill). This supersedes the member-by-member hide, which since the wordmark data-maps link retirement had left the global page's pill an empty lozenge on phones. The hide rule carries a class qualifier so it outranks the surface sheets' unqualified `#wordmark` display rules, which load after the shared sheet and previously won the cascade tie. `map-shell.css` is unpinned on every page, so no cache pins move. Browser-gated at 430 px and 900 px on the global map and the NZ page: the pill computes `display: none` on phones and renders on desktop. Files: `apps/shared/map-shell.css`, `apps/global/styles/maplibre-flat.css` (comment only).
- The census data panel gains a phone-only dismiss chip (JB report: the expanded panel covers half a phone screen, and field use showed the census button's collapse caret goes undiscovered). A × chip hangs off the panel's top-right corner — a 44 px hit box per the style-guide touch floor around a 28 px disc in the dock-close idiom — and closes through the same state as the census button, so the button's `aria-expanded` and caret stay truthful and remain the reopen path. Phones also drop the panel's two reference-prose lines (the colour sentence and the construct note) to give the map back its screen; the wash and caveat disclosures stay, because they change what the paint means, and both dropped sentences remain on desktop and in each country's overview page. Desktop chrome is untouched: the chip and the trim live behind the 640 px query. Browser-gated at 430 px and 900 px on the NZ page: chip collapse and pill reopen with truthful aria state, colour and construct notes hidden on the phone and shown on desktop, wash notes shown on both. Files: `apps/regions/_shared/region-map.js` (cache pin `?v=20260716j`), `apps/regions/_shared/region-map.css` (cache pin `?v=20260716h`), 100 pages.

### 2026-07-16

- Retired the top-right wordmark "Data maps" link on the global map and all 100 country pages; the bottom-bar Data Maps pill is now the sole entry to the country switcher and hub, and it degrades to a plain hub link without JavaScript. No script or stylesheet changes, so no cache pins move. Browser-gated on NZ and the global map: wordmark renders its remaining links, switcher opens from the pill in both surfaces. Files: `apps/global/index.html`, 100 country page configs.
- Settled the Data Maps pill on two working states after a second owner field-test (JB) and a three-model review (Sonnet 5 interaction pass, Opus 4.8 correctness/mobile pass, Codex gpt-5.6-sol adversarial pass; design record: `docs/development/country-broadcast-review-2026-07.md`). Over home the main zone is the census toggle — "● NZ Data", one tap hides or restores the choropleth, the blue dot and `aria-pressed` following the layer — and over another data country it is a one-tap travel offer — "Australia Data →", navigating immediately with the census view carried. The green territory overlay and the two-tap arming are gone, as are the sticky "Back to …" states: each offer replaces the last as the user moves, tracking covers returns, the reset button restores the initial state (camera, filters, census on, offers dropped), and the switcher panel now pins a one-tap "← Back to <country last departed>" row remembered per tab. Review fixes shipped: the toggle is keyboard-operable (`tabindex`, Enter/Space) and names each country's own data noun (survey/register/attendance pages no longer claim "census"); rapid toggles can no longer desync the choropleth from the dot (in-flight census loads are shared and the latest intent wins); panning no longer spams the pill's live region; and long country names ellipsise without ever truncating the navigation arrow. Browser-gated on NZ/AU journeys and the global surface. Files: `apps/regions/_shared/region-map.js`, `apps/global/index.html`, `apps/global/styles/maplibre-flat.css`, `apps/shared/datamaps-switcher.js` (cache pins `?v=20260716h`, 100 pages and the global surface).
- Reshaped the country-aware offer into a split Data Maps pill after owner field-testing (JB) rejected the separate travel pill: the bottom-bar pill's main zone now follows the map itself — resting "Data Maps" opens the searchable country panel; over a neighbour it reads "<Country> Data →" in emerald; after a handoff, "Back to <Origin> →"; from open water, "Back to <Home>" — while a caret zone opens the search panel in every state. The emerald territory highlight is gesture-gated: panning only relabels the pill, the first tap (on the pill or on the country's land) arms the offer and paints the outline, and the second tap navigates with the census view carried; a user gesture disarms, programmatic camera nudges do not. The global map seats the same split pill with a "Zoom for <Country> data" hint that delivers the promised zoom on click. All tribunal fixes carry over (shared resolver, nested-hole topology, honest per-state `href`, aria-live shell, ellipsised long names, styledata re-assertion); design record updated in `docs/development/country-broadcast-review-2026-07.md`. Browser-gated on NZ→AU (offer → arm → outline → navigate → carried census view → "Back to New Zealand →" arrival, caret panel in offer state) and the global map (offer/arm, hint-click zoom, ocean rest, resting-tap panel fallthrough). Files: `apps/regions/_shared/region-map.js`, `apps/regions/_shared/region-map.css`, `apps/global/index.html`, `apps/global/styles/maplibre-flat.css`, `apps/shared/datamaps-switcher.js` (cache pins `?v=20260716g`, 100 pages and the global surface).
- Made the border-handoff pill a country-aware travel control on every surface (tribunal design record: `docs/development/country-broadcast-review-2026-07.md`). On country pages the emerald pill tracks the camera — resting over a neighbour offers "Continue into <Country> →" while the neighbour's territory takes an emerald outline beneath the place dots (outline only: a fill would tint the census choropleth); tapping foreign land arms the same offer, which now survives programmatic camera nudges and clears on a home-or-water tap. The global map seats the same pill for the first time: "Continue into <Country> →" with an emerald wash plus outline at country scale, "Zoom for <Country> data" (dimmed) when too far out — clicking delivers the promised zoom — and hidden over open ocean; the Switch Country pill never changes identity. The pill is an anchor whose `href` carries the true destination (country, camera, origin, census view), so cmd-click, middle-click, and copied links are honest. One shared resolver (`apps/shared/region-resolve.js`) replaces the two implementations that a tribunal reviewer measured disagreeing at 1,761 half-degree grid points, and nests outline rings into MultiPolygon topology (cached per region) so enclaves such as Lesotho no longer double-paint inside their neighbour's highlight. Theme switches re-assert the highlight. Reviewed before landing by three Fable agents and one Codex gpt-5.6-sol instance at high effort; the manifest split (boxes-only arming payload) and generator-side ring nesting are recorded as open work. Browser-gated on NZ (tap, nudge, water-tap, outline, href contents) and the global map (all three states, hint-click zoom). Files: `apps/shared/region-resolve.js` (new), `apps/regions/_shared/region-map.js`, `apps/regions/_shared/region-map.css`, `apps/global/index.html`, `apps/global/styles/maplibre-flat.css`, 100 country pages (resolver include; cache pins `?v=20260716f`).
- Border handoffs now carry the census view (roam v1, design record: `docs/development/datamaps-pill-review-2026-07.md`). The fragment gains a `d=metric:year` segment beside the camera and filters, written whenever the census paint changes, so the view is shareable and a handoff lands the neighbour on the same construct: an unknown metric falls back to the arrival country's default, a wave the arrival lacks clamps to its nearest available year (the same rule that guards a level switch, now also applied on first load), and geography level never carries because levels do not correspond across countries. The Pulotu source drops the segment — its metrics name another dataset's constructs. Browser-gated: NZ writes `d=religious_affiliation_percent:2023` on load; an NZ→AU arrival with `d=no_religion_percent:2013` opens Australia on No religion % clamped to 2016, legend and rewritten fragment agreeing. File: `apps/regions/_shared/region-map.js` (cache pin `?v=20260716e`, 100 pages).
- Added a Switch Country pill to the bottom action row on the global map and all 100 country pages, and demoted the top-right wordmark "Data maps" entry to a plain hub link — one control now owns the country-switcher panel. The pill is an anchor to the hub, so visits without JavaScript still navigate; enhanced, it opens the searchable switcher centred above itself, capped at 60vh so the list reads as a menu and stays clear of the top chrome. The design went through a four-reviewer panel (three Fable agents plus one Codex instance; findings and decisions: `docs/development/datamaps-pill-review-2026-07.md`), whose fixes ship with it: the filter chip joins the phone shrink rule and an overfull row wraps upward instead of running off-screen; the refresh button steps up out of the bottom band on phones (the handoff offer narrows to keep the corner clear); the hub escape during a stalled catalogue fetch now requires a one-second wait so a double-click cannot yank the user away; a soft-keyboard resize re-places the panel instead of closing it; cmd/ctrl+Enter in the filter keeps its new-tab meaning; and the trigger declares `aria-controls` and `aria-busy`. The pill labels itself Switch Country on desktop and Countries on phones, mirroring the census pill's label swap; the action-row grammar amendment is recorded in `apps/shared/map-shell.css`. Browser-gated on NZ and the global map: pill and wordmark states, panel open/close/escape/re-anchor, phone-width row measurements (293 px three-pill, wraps at four), console clean. Files: `apps/global/index.html`, `apps/global/styles/maplibre-flat.css`, `apps/regions/_shared/region-map.js`, `apps/regions/_shared/region-map.css`, `apps/shared/datamaps-switcher.js`, `apps/shared/map-shell.css`, `docs/development/datamaps-pill-review-2026-07.md` (cache pins `?v=20260716e`, 100 pages and the global surface).
- Touching a neighbouring country's territory now surfaces the border-handoff offer (JW request): a tap or click on foreign land shows the same land-verified "Continue into ... " pill the centre-crossing detection produces, sharing one resolution helper so both name the same country. The tap only offers — navigation still takes a second tap on the pill, so a stray touch never navigates. Taps that hit place dots, census areas, or Pulotu points keep their existing popups untouched; taps over open water or the home country change nothing. The pill also moves from slate glass to emerald — the chrome's "go" colour, distinct from amber (caution) and the blue data ramp. Gated in a real browser: Tasman-view tap on Australia offers and navigates, Wellington-harbour and home-land taps stay quiet, and the Selwyn District census popup still opens. Files: `apps/regions/_shared/region-map.js`, `apps/regions/_shared/region-map.css` (cache pins `?v=20260716d`, 100 pages).
- Fixed the arrival return offer dying before it was ever seen (JW report: Bahamas to United States, no way back). The runtime's own paired jumpTo camera nudges after layer refreshes fire programmatic movestart events, and the movestart handler treated them as the user moving on, clearing the handoff origin marker before the pill rendered. Only user gestures (movestart events carrying originalEvent) now consume the arrival offer; programmatic moves merely re-hide and re-show it. Round trip gated in a real browser on the local build: BS offers the US, arrival offers Back to Bahamas, a drag consumes it, Nassau offers Continue into Bahamas, and the BS arrival offers the US back. File: `apps/regions/_shared/region-map.js` (cache pin `?v=20260716c`, 100 pages).
- Added symmetric neighbour overrides to the catalogue generator for isolated countries the three-degree adjacency rule misses; the first override pairs New Zealand with Australia, so the NZ page now idle-warms the AU page (the hop its users actually make) and vice versa. Files: `scripts/build_region_catalog.py`, `apps/shared/data/region-catalog.json`.
- Added a dataset passport to the data panel on all 100 country pages (design record: `docs/development/sidebar-design-space-2026-07.md`). The panel now names its contents with zero interaction: the statistics provider(s) and construct on one line, the geography, wave span, and displayed year on the next — generated from each product's own `source_datasets`, `construct`, `boundary_set`, and `site_snapshot` metadata, never from hand-written page prose. Boundary and place-snapshot datasets are excluded from the identity line by their declared ids and stay in the drawer. Below the legend ramp a colour sentence states direction and scale behaviour plainly (sequential: darker blue means higher, one scale across every wave, clamping named when active; diverging: orange decline, blue growth). A collapsed "About this data" drawer carries the source list with licences, the boundary lineage, provider-mandated reuse text (e.g. the IPUMS NHGIS note), the page's flag note and attribution line, and a link to the full `overview.html` record; its body scrolls independently so long source lists never grow the panel. The Pulotu source shows its own passport; pages whose products carry no metadata show nothing rather than a guess. Browser-gated on NZ, US, NL, WS (passport + drawer + colour note), NZ diverging metric, and VU Pulotu; console clean. Files: `apps/regions/_shared/region-map.js`, `apps/regions/_shared/region-map.css` (cache pins `?v=20260716b`, 100 pages).
- Added a generated catalogue for all 100 country data maps. The country switcher reads the catalogue first and retains the live hub parser as its compatibility path when the catalogue is absent or malformed. After the active map first becomes idle, the switcher prefetches neighbouring page HTML; hovering, focusing, or touching a country option also prefetches that page and its default area summary when the remaining budget permits. Prefetching stops on data-saving and 2G connections, runs during idle time, and uses conservative raw-byte estimates to keep speculative transfer below the 1 MB compressed-transfer ceiling. The existing border handoff now shares the same guard and budget. Files: `scripts/build_region_catalog.py`, `apps/shared/data/region-catalog.json`, `apps/shared/datamaps-switcher.js`, `apps/regions/_shared/region-map.js` (cache pin `?v=20260716b`, 100 country pages and the global switcher surface).
- Fixed one-way border handoffs between country data maps. Each handoff now carries the country just left and offers that route back on arrival. The arrival-only offer clears as soon as the user moves the destination map; ordinary land-verified border detection then resumes, preserving the existing protection against stale or oscillating offers. The country switcher remains available on the destination page as an independent route between all 100 data maps. File: `apps/regions/_shared/region-map.js` (cache pin `?v=20260716a`, 100 pages).

### 2026-07-14

- Added a border-handoff pill to every country data map (JW request): when the map centre leaves the country's data extent, a pill above the bottom action row offers the country now under the centre ("Continue into Australia →") — or the way home over open water ("Back to New Zealand") — and a handoff carries the camera hash so the neighbouring page opens on the same view, with the neighbour's page prefetched while the offer is on screen. Country extents ride a committed manifest generated from the shipped boundary files (`scripts/build_region_bboxes.py`, shapely dissolve and simplify): clustered bounding boxes with west > east wrapping the antimeridian as the fast prefilter, plus a simplified true outline per country for the point test that rectangles cannot settle — Munich sits inside L-shaped Austria's bounding rectangle, and only the outline lets the Austria page offer Germany there. Islet rings drop only when a country has a mainland-scale ring, so atoll nations keep their full outlines. Countries without corpus maps offer nothing rather than a wrong neighbour, and offers are land-verified: a wide foreign rectangle over empty ocean never claims the centre. No manifest, no feature: a failed fetch leaves the page exactly as before. A Codex (gpt-5.6) cross-model review returned six must-fixes, all applied: outward-padded floor/ceil boxes with a builder invariant that no outline vertex or feature corner escapes its boxes, unwrap-aware cluster unions, polygon-level antimeridian shifting so holes stay in their exterior's frame, hide-on-movestart so a stale offer is never clickable mid-pan, circular-axis margin containment (Tuvalu's grace band now wraps ±180), and an unrelated uv.lock mutation reverted. Gate verified in browser on the New Zealand (Wellington quiet, Cook Strait quiet, the Chathams quiet across the antimeridian, mid-Tasman "Back to New Zealand", Sydney "Continue into Australia" with the handoff landing on the Australia page at the identical camera), Fiji (quiet across the antimeridian, Tonga and Samoa offers, open-Pacific way home), and Austria (Vienna and Innsbruck quiet, Munich → Germany, Zürich → Switzerland, Ljubljana correctly silent) pages; console clean. Files: `apps/regions/_shared/region-map.js`, `apps/regions/_shared/region-map.css` (cache pin `?v=20260714b`, 100 pages), `apps/regions/_shared/data/region-bboxes.json` (new), `scripts/build_region_bboxes.py` (new), `docs/api/workflow-scripts.md`.
- Linked the 100 country data maps together from every map surface (PI request, prompted by JW's session): the top-right "Data maps" wordmark link on all 100 country pages and on the global map now opens a searchable country-switcher panel — type-to-filter over the country names (accent-folded), keyboard navigation, the current country marked "you are here", and a footer link to the full hub with its source notes — so moving between data maps is one tap instead of the two-hop trip through the hub. The panel is a single shared module (`apps/shared/datamaps-switcher.js`) that fetches and parses the hub page's map cards at first open, keeping the hub the single source of truth: a newly launched country appears in every switcher with no extra step. Progressive enhancement throughout — without JavaScript, or if the hub fetch fails, the link navigates to the hub exactly as before, and the shared runtime is untouched (no fleet cache bump; each page gains one script include, pinned `?v=20260714a`). The hub itself gains a script-injected quick filter over its 100 cards with a live "N of 100 countries shown" count. UI design and module authored in the main lane (Fable); a Codex (gpt-5.6) cross-model review returned six must-fixes — modifier-click passthrough, combobox semantics for assistive technology, keyboard autofocus on coarse-pointer devices, stalled-fetch fallback, dragged-wordmark panel stranding, and bottom-anchor placement — all applied. Gate verified in browser: panel open/filter/keyboard/Enter navigation on the Samoa, Côte d'Ivoire and New Zealand pages, "you are here" marking, no current-marker on the global map, hub filter to "5 of 100 countries shown", Esc and outside-click close, console clean. Files: `apps/shared/datamaps-switcher.js` (new), `apps/regions/index.html`, `apps/global/index.html`, and the script include on the 100 country pages.
- Launched the Samoa research map (hub to 100) as a coarser-grain partial layer: religious-affiliation and no-religion shares for the four statistical regions in 2021, aggregated exactly from the committed 51-constituency product through the census workbook's own hierarchy — the four region rows close to the constituency table with zero deviation on every field, and no value is allocated or inferred. The hatched wash carries the amber Partial layer tag and states its reason in the census panel: no licensed boundary exists for the 51 electoral constituencies, so the region layer renders until the Samoa Bureau of Statistics supplies a constituency vector, at which point the finer product (already committed) supersedes it. The boundary is the Pacific Data Hub 2011 PHC region layer (Creative Commons Attribution, SPC Statistics for Development Division), its vintage and the source's own boundary caveat disclosed verbatim. SBS data remain under the project-lead use-for-now ruling while a reuse ask is outgoing; the page reverts in a single commit if SBS objects. Gate verified in browser: hatch on all four regions, partial-layer tag and reason note, the Savaii popup reproducing the 26-line composition verbatim (Congregational Christian Church of Samoa 11,750 re-summed independently against the constituency table), console clean on a fresh load; the small inter-region slivers where the source polygons do not tile are the published geometry, shipped unrepaired under the disclosed caveat. Build lane ran on Codex (gpt-5.6); both validators re-run by the conductor. Pages: `apps/regions/ws/index.html`, `apps/regions/ws/overview.html`.
- Launched the Saint Kitts and Nevis research map (hub to 99): a national three-wave census-religion series (1991, 2001, 2011) shading one national polygon under the small-country clause, because the Department of Statistics publishes religion for the whole federation only — parish geography exists for every other census variable, parish religion for none. The 1991 wave carries the source's printed one-decimal percentages exactly (Table 2.3 prints no 1991 counts; each value carries a ±0.05 rounding bound and the Bahai/Evangelical-into-Other grouping is disclosed); 2001 and 2011 derive from printed counts whose category lines close integer-exact at 46,325 and 47,195; the 2011 religion-table total is recorded against the 46,398 resident-population figure without repair; the 1991→2001 change discloses its mixed percentage/count basis. Reuse is accepted under the DOS-SKN Open Licence Agreement with its acknowledgement notice carried verbatim; the boundary is geoBoundaries KNA ADM1 (CC BY-SA 2.0) dissolved to one national feature. Gate verified in browser: wash on all three waves, index-based year control, popup reproducing the full 21-line 2011 composition verbatim, console clean on a fresh load. Build lane ran on Codex (gpt-5.6), stopping correctly at two hard gates (source-hash drift; the 1991 percentage-only construct) before the conductor's rulings; both validators re-run by the conductor. Pages: `apps/regions/kn/index.html`, `apps/regions/kn/overview.html`.
...
- Built the Saint Vincent and the Grenadines census-division product (Codex lane, conductor-verified), STAGED — the corpus's first percentage-only partial layer: 26 division-wave rows (13 census divisions × the 2001 and 2012 censuses) carrying the eleven Christian-denomination share columns of the 2012 Census Report's Tables 2.12 and 2.13, which print as image-only percentage tables. The values ride a conductor transcription double-read at two resolutions; the sole comma-decimal cell is documented; rows deliberately do not sum to 100 because the non-Christian, no-religion, and not-stated remainder is unprinted at division grain, and the product declares itself a partial layer on that basis. Boundaries are the US Census Bureau Demobase ADM1 layer — the only located vector matching the thirteen divisions exactly — carried under the US-government-work posture with USCB's provenance statements and accuracy disclaimer verbatim. The area-summary.v2 schema gains the optional top-level partial_layer declaration; all 21 v2 products pass the gate. The shared runtime gains seven opt-in denomination-share metrics, unchanged for every existing page. Build: `pipeline/build_vc_area_summary.R` (private tier); data: `apps/regions/vc/data/`.
- Contained the census-panel year-tick row inside the data panel on long series (PI request): the shared runtime now labels only as many year ticks as the strip can hold — every year keeps its span so the flex row still places ticks along the slider, but an unlabelled span is empty and shrinks to nothing, where the old CSS `visibility: hidden` thinning kept each hidden span's min-content width and let Sweden's 54 register years push the tick row ~790px past the panel edge (Italy's 24 survey years overflowed by 57px; Poland's 10 and the US's 15 fitted, and the US series renders exactly its previous eight labels). First and last years always label, the legend title carries the active year, and the ticks row clips at the panel edge as a backstop. Gate verified in browser on Sweden through the module's own render (54 spans, 12 labels, zero overflow) and on the Italy, Poland, US, and 360px-mobile series in the live panel. Files: `apps/regions/_shared/region-map.js`, `apps/regions/_shared/region-map.css` (cache pin `?v=20260713g` at merge, 98 pages).
- Added the partial-layer visual language to the shared data-map runtime (PI rulings this sitting): a level that declares itself partial — a unit or wave subset, a coarser or percentage-only grain, or a single-denomination register — now renders its choropleth with a diagonal hatch over the unchanged colour ramp (valued units only; no-data washes never read as hatched data), carries an amber "Partial layer" tag on the data pill, and states its declared reason in the census panel. The declaration is opt-in per level (config or summary file), so every existing page renders byte-identically. Gate verified in browser on a flagged test configuration: hatch on valued units across light and dark ramp shades, no hatch on no-data units, tag and reason note bound to the active level. Files: `apps/regions/_shared/region-map.js`, `apps/regions/_shared/region-map.css` (cache pin `?v=20260713d`, 97 pages).
- Extended data-map panel and banner dragging to touch (PI request): the pills — denomination key, census panel, and wordmark — and the map popups now drag on phones exactly as they do under a mouse, with the same drag-then-swallow-the-trailing-click gesture and a wider start threshold on touch (10px against the mouse's 4px, since fingers jitter through a tap). The pill chrome and popup bodies opt out of browser pan/zoom via `touch-action: none` so the gesture reaches the module's pointer handlers; native controls (the selects, the time slider, the later-foundations checkbox) keep their own touch handling; the phone-scrollable denomination key list keeps its scroll (touches there never start a drag — the key still moves by its pill); and the scrollable onboarding card stays non-draggable by design, on phone and desktop alike. The bottom-left reset pill already snaps every dragged pill home on phones as on desktop (`resetSite` → `clearDragTransforms`), so refresh resets the map unchanged. Files: `apps/regions/_shared/region-map.js`, `apps/regions/_shared/region-map.css`, with a fleet-wide cache pin (`?v=20260713c` at merge, 96 pages).
- Rounded out the touch-drag work with five approved enhancements (PI request). A touch drop now glides the pill to the nearest screen edge (the picture-in-picture idiom — free placement strands pills over the map centre on phones; mouse and pen keep free placement). Double-tapping a displaced pill sends just that pill home, so users need not reset the whole map to fix one panel. A once-per-session toast ("Drag to move — double-tap sends it home") teaches the gesture on the first press-and-hold of a pill, and a completed drag marks it seen. The global map gains full parity: its key bar (newly wrapped in the region pages' `#key-wrap` idiom so bar and panel travel together) and wordmark drag on every pointer type, its popups accept touch, and its reset pill now snaps dragged pills home. And the resize handler that snapped every pill home now fires only on a width change — on phones the on-screen keyboard fires a height-only resize, and opening search no longer discards the user's arrangement. The work also surfaced and fixed a latent gesture bug on both surfaces: a mouse drag fires its trailing click at once, but a touch or pen drag fires none at all, so the armed click-suppression flag survived the gesture and swallowed the next legitimate tap on the pill or popup — it now disarms itself 150ms after the drop (the pin's post-drag pattern). Files: `apps/regions/_shared/region-map.js`, `apps/regions/_shared/region-map.css`, `apps/global/index.html`, `apps/global/styles/maplibre-flat.css` (shared touch rules centralised here; cache pin `?v=20260713c` at merge, 97 pages).
- Launched the Bhutan research map (hub to 97): 11 carried register years 2013-2023 of dzongkhag administrative register counts of religious institutions from the National Statistics Bureau's Dzongkhag at a Glance series, levels only on the 20-dzongkhag frame. Every information surface states the buildings-not-people construct (Bhutan collects no census or survey data on personal religious affiliation) and the register-variance disclosure (each dzongkhag office defines the register locally; counts are not comparable across dzongkhags). Gate verified in browser: the register choropleth renders with the year control live, the Wangduephodrang popup carries starred source-history flags for revised and redefined figures, Samdrup Jongkhar renders as disclosed no-data in every year, 2023 paints exactly the two published dzongkhags, and the console is clean on a fresh load. The shared runtime gains an opt-in place_count metric for building-register products, unchanged for every existing page. Pages: `apps/regions/bt/index.html`, `apps/regions/bt/overview.html`.
- Relocated the project's research workflows — source assessments, provenance manifests, extraction pipeline, and planning records — to a private research tier for the current work phase (tag `pre-split-20260713` marks the boundary). The public repository continues to carry the maps, their data products, the platform, and the RA documentation; every map page continues to state its source, construct, and licence position directly, and the overview pages now say where provenance detail lives and how collaborators can request it.
- Launched the Malaysia research map (hub to 96): four census waves 1991-2020 on the stable printed 160-district frame with a year control, the fused No Religion/Unknown metric, and a null-safe wave-to-wave affiliation-change metric (nothing on 1991, nothing where either operand is null). Gate verified in browser: the conductor's catch was a broken runtime join — the boundary carried no area_code property and the page pointed codeProp at area_unit_id, so no wash could ever bind; fixed in the builder (area_code now rides every feature), the product regenerated with the summary byte-identical and only the geojson properties and manifest hash changing, and the wash verified live on all four waves. Popups verified against the product: Perlis reproduces its state-report Table 2 composition exactly across four waves; Pendang renders em-dashes for its three unpublished waves and the seven-category 2020 Table 7 frame with the documented No Religion (16) + Unknown (506) combine; 1991's Sarawak null districts render as no data. A second conductor catch on the overview: the draft used W.P. Putrajaya's 100.0% affiliation as a contrast without saying its No Religion/Unknown line is unprinted that wave — replaced with Besut (154,152 of 154,168, a genuinely printed line of 16) and an explicit non-comparability sentence. Console clean on a fresh load. The page lane ran on Codex (gpt-5.6), conductor-gated. Pages: `apps/regions/my/index.html`, `apps/regions/my/overview.html`.
- Built the Malaysia district census-religion product (Codex lane over four conductor gate cycles, conductor-verified), STAGED — the corpus's largest build: 640 district-wave rows across 160 districts and the 1991/2000/2010/2020 censuses from 160 checksummed DOSM sources (152 district PDFs, two genuine Jadual census workbooks recovered from behind mis-uploaded files, four single-district state reports, and the Kedah/Terengganu Table 7 blocks). Each gate cycle resolved a real source structure by ruling rather than improvisation: the coverage stop led to the frame-closing follow-up harvests; the boundary stop led to the documented OSM derivation under the PI's ODbL ruling (level-4 extent substitutions, verified Sarawak dissolves, genuine Selama and Lojing sub-relations with zero-overlap geometric differences, 160/160 join); and the reconciliation stop exposed that DOSM's religion blocks sum below the printed district population totals with the printed percentages riding the religion-block denominator — ruled to the Jamaica precedent, with population_total as the religion universe and the printed total carried as a signed per-row deviation record (1991 systematically short by 65,411 nationally), never repaired. The fused No Religion/Unknown line is disclosed on every row; the two Table 7 districts carry the seven-category frame with a documented combine; 74 cells stay null as printed or unpublished; Baling 2020's printed-percentage discrepancy is documented in the manifest. The conductor re-ran both validators, re-derived the closure on all 640 rows, re-verified spot districts against the harvest record, and re-checked the subtraction-pair geometries. Build: `scripts/build_my_area_summary.R`; manifest: `docs/manifests/my-census-religion-1991-2020.json`.
- Built the provisional measurement-regime declarations table (Codex lane, conductor-verified), part one of the ruled measurement-regime view: 99 country-product rows extracted from the 103 committed manifests and the area-summary quality flags under a conductor-defined v1 vocabulary (instrument, wave span, grain, value basis, no-religion-category presence, not-stated handling, universe, licence posture, small-cell marks, boundary licence), with every non-unknown value provenance-bound to a named manifest key or flag token and unknown preferred over inference. The conductor re-verified vocabulary compliance, provenance completeness, and four spot rows, and reproduced the build deterministically. Two fields crossed the majority-unknown threshold and are flagged as candidate v2 vocabulary revisions rather than silently shipped: not_stated_handling (79 of 99 unknown) and small_cell_marks (92 unknown — the flag tokens only exist where small cells occur, so absence is not evidence of absence). The four unmapped manifests are multi-country or intermediate inputs, each with its reason recorded. The vocabulary is provisional pending the project lead's review; the global-map view (part two) remains unbuilt by design. Table: `apps/global/data/measurement_regimes.json` (+ .csv); script: `scripts/build_measurement_regime_table.R`; vocabulary: `docs/development/measurement-regime-vocabulary.md`.
- Completed the Malaysia DOSM district-report URL harvest (Opus browser lane, conductor-verified), removing rank 11's standing blocker: the 160-district 2020 frame reconciles exactly, with 152 district-report PDFs and 2 XLSX-only districts downloaded and sha256-checksummed (1.15 GB, spot checksums re-verified at the gate), two districts confirmed unpublished, and Perlis plus the three federal territories located in the state-report series for a small follow-up harvest. The record documents the catalogue's unstable pagination (a single traversal silently hides ~45% of a result set) and the exhaustive title-prefix enumeration that defeats it, plus the Malay-only Tumpat title and the mis-uploaded XLSX districts. Johor Bahru reproduces the probe's validated sample byte-for-byte with exact Table 3 margins. Record: `research/countries/my/url-harvest-2026-07-13.md`.
- Recorded the Bhutan build attempt (Codex lane, conductor-held): the lane stopped correctly at the coverage census when the Wayback Machine went intermittent mid-retrieval and the fetched sheets exposed extraction-barring layout defects (unrecoverable year-header assignments, a sheet missing its institutions row, damaged PDFs); the conductor held the build on the Mongolia pattern with two rulings recorded for the retry — place_count carries the Religious-institutions count on the Taiwan register analogue, and the Religious-monuments indicator stays manifest-only with a second-indicator question for the PI. No deliverable shipped; the probe's BUILDABLE verdict stands and the hold is operational. Record: `research/countries/bt/build-attempt-2026-07-13.md`.
- Recorded the Bhutan route probe (Opus lane, conductor-reviewed), BUILDABLE and a refuted census premise: NSB's Dzongkhag at a Glance sheets print per-dzongkhag annual counts of religious institutions and religious monuments (compiled by the Dzongkhag Administration) across the 2016-2023 editions — an administrative register of religious infrastructure in the Taiwan construct class, never population affiliation. PHCB 2017 publishes no religion table at any grain; Bhutan collects no census religion, a refutation the probe records against a domain-wide sweep. Boundary geoBoundaries BTN ADM1 (20 dzongkhags, CC BY 3.0) joins one-to-one. Licence: an all-rights-reserved NSB footer with no located grant — the ratified build-then-ask family, with the NSB courtesy ask recorded. The live NSB host is unreachable from the probe environment; every capture rode the Wayback Machine with checksums. Sensitivity: the product counts buildings, never people, and the conductor ruled a stage-only build with the framing question recorded for the PI before any page ships. Probe: `research/countries/bt/route-probe.md`.
- Recorded four PI rulings (2026-07-13, seventh sitting): PARTIAL AREA LAYERS SHIP as a standing ruling — where the record supports only a partial subnational layer (a subset of units or waves, a coarser or percentage-only grain, or a single-denomination register), it ships with clearly differentiated styling or an explicit partial-coverage indicator rather than holding, with two guards (a partial layer never blocks a later complete product, and consent/sensitivity/licence holds are untouched); Sweden's diocese layer, Saint Vincent's division percentage tables, and Antigua's 2011 parish crosstab are named first candidates, and the runtime's partial-layer visual language is a queued design lane. Bhutan's buildings-not-people sensitivity framing is APPROVED as drafted, unblocking the page once the build retry lands. Malaysia SHIPS FLAT for now with clear notation — the near-flat page stands and the Muslim-share re-emission is a non-blocking revisit. The measurement-regime vocabulary gains a PI-directed v2 maintenance dimension (series status, next expected wave, update posture), recorded in the vocabulary doc with the manifest-field-first sequencing note.
- Launched the Sweden research map (hub to 95), the corpus's second national-only register launch: 54 annual waves 1972-2025 of Church of Sweden administrative membership on one national polygon, register-construct language on every information surface and no no-religion metric anywhere (a non-member supports no no-religion claim). Gate verified in browser: the national wash renders, the popup carries the full 54-year table with 1981* rendering an en dash on the no-data wash (the year control's index-based input verified on the 1981 index), the printed 94 for 1977 displays as 94.0% under the conductor's trailing-zero ruling (the render-as-carried rule targets precision loss; zero-padding to the series' uniform one-decimal display alters no value), and console clean on a fresh load. The lane stopped correctly at the toFixed gate rather than editing the shared runtime, and its public paraphrase of the review posture ("No reuse terms stated; used with attribution pending confirmation") replaced the manifest's internal phrasing on every public surface. Both licences disclosed distinctly (church series unstated; boundary OGL v3.0). The page lane ran on Codex (gpt-5.6), conductor-gated. Pages: `apps/regions/se/index.html`, `apps/regions/se/overview.html`.
- Built the Sweden national Church of Sweden membership series (Codex lane, conductor-verified), STAGED under the ship-national-only ruling and the 2026-07-13 build-then-ask licence ruling: 54 annual rows 1972-2025 from the church's printed membership register PDF (checksum-pinned), counts and percentages carried verbatim with no recomputation (7,754,784 members and 95.2% of the population in 1972 falling to 5,365,542 and 50.7% in 2025), and 1981 preserved as the source's sole null-metrics row with its no-population-base note quoted verbatim. The construct is a single-denomination administrative register on the Norway/Denmark mapping: members ride the affiliation slots, no_religion and population_total stay null on every row (a non-member is not a no-religion claim, and the project never derives a count from a percentage), and no composition ships. Licence needs_review under the no-stated-terms build-then-ask precedent — a dedicated assessment lane found no reuse terms on any church surface or inside the PDF, and the Church of Sweden and MUCF courtesy asks are recorded. The build lane's catch: geoBoundaries SWE ADM0 is Open Government Licence v3.0 (2019 vintage, Open Data Soft/WFP-derived), not the ODbL the brief assumed — verified at source and carried in the manifest. Both validators re-run and passed at the conductor gate. Build: `scripts/build_se_area_summary.R`; manifest: `docs/manifests/se-church-membership-1972-2025.json`.
- Launched the Jamaica parish research map (hub to 94): single wave 2011 on the 14 parishes from STATIN Volume 6 Table 5, with the heterogeneous printed frame disclosed on every surface (onboarding, both metric notes, popup denominator note, flag note, and the overview) — eleven parishes print 19 categories, Saint James and Saint Catherine 20, and Saint Andrew 23, and the no-religion slot is assigned by role across the three printed spellings, each kept verbatim in the popup composition. Gate verified in browser: the parish choropleth renders on both metrics with the metric flip live, the Clarendon popup reconciles exactly against the product (83.5% affiliation, 14.9% no religion; Anglican 4,183, Baptist 19,756, New Testament Church of God 29,510, the verbatim None line 36,469, Not Stated 3,997), console clean on a fresh load. Every numeric claim on both pages was re-derived from the staged product at the gate (national margins 2,050,759 + 571,982 + 61,374 = 2,684,115; extremes Kingston 62.5% affiliation and 34.0% no religion against Manchester 86.6% and 11.8%; smallest shipped cell Trelawny Moravian at 31; zero under-ten cells, no dagger). The page lane ran on Codex (gpt-5.6) under the PI's token-conservation routing, conductor-gated. Pages: `apps/regions/jm/index.html`, `apps/regions/jm/overview.html`.

### 2026-07-12 (sixth sitting)

- Added the Jamaica 2011 wave (Opus lane, conductor-verified), STAGED: 14 parishes from STATIN Vol 6 Table 5, the Total column integer-exact to the 2,684,115 religion universe at every printed margin. The build lane stopped correctly on a source-vs-probe contradiction — the parish frame is heterogeneous as printed (nineteen categories in eleven parishes; St Andrew twenty-three with all four small non-Christian groups broken out, St James twenty with Hinduism, St Catherine twenty with Islam) — and the conductor ruled ship-as-printed under the verbatim-over-uniform precedent, with the frame heterogeneity and the small-group parish-to-national non-additivity (1,482 absorbed in the parish Other lines by source design) disclosed in the manifest and on every row's flags. The no-religion slot takes each parish's single affirmative none line by role across its three printed spellings (571,982 nationally); Not Reported/Not Stated stays in the denominator outside both slots (61,374); zero small-cell rows survive to print. The probe's over-generalised uniform-frame claim carries a dated correction. Build: `scripts/build_jm_area_summary.R`; manifest: `docs/manifests/jm-census-religion-2011.json`.
- Launched the Grenada parish research map (hub to 93), the corpus's first preliminary-data page: single wave 2021 on the seven parish units from Table 23 of the CSO 2021 PRELIMINARY census results, with the preliminary status disclosed on every surface (onboarding, both metric notes, popup denominator note, flag note, attribution, and the overview intro, reading, sources, and licence blocks). Gate verified in browser: the parish choropleth renders, the Saint Mark popup reconciles exactly against the product (3,938 total; Anglican 304, Pentecostal 684, Buddhist 2, Hindu 1 — printed zeros and the source spellings Mormom and Independent Baptiste kept verbatim), the fewer-than-ten dagger fires, the two no-religion lines stay distinct in the composition block, console clean. Every numeric claim on both pages was re-derived from the committed product by the page lane and re-checked at the gate. Pages: `apps/regions/gd/index.html`, `apps/regions/gd/overview.html`.
- Launched the Finland research map (hub to 92), the corpus's first national-only register launch under the ship-national-only ruling: 36 annual waves 1990-2025 of administrative religious-community membership on one national polygon, gate verified in browser — the wash renders (2025 correctly takes the lightest shade as the series minimum), the popup carries the full 36-year table with the 2025 row at 64.1% members and 35.9% not members, and the tap-for-composition block lists the 23 mutually exclusive verbatim register categories. The conductor's catch at the gate: the lane's composition carried all 26 published lines including the TOTAL line and the two roll-up headings (CHRISTIANITY and OTHER RELIGIOUS GROUPS, each the exact sum of its children), which would double-count in any reader's sum; the conductor ruled composition a true partition, re-emitted the product to the 23 leaves summing exactly to the register total every year, and kept the full 26-line published frame recorded verbatim in the manifest. Register-construct language rides every information surface; both licences disclosed distinctly (StatFin CC BY 4.0; boundary ODbL 1.0). Pages: `apps/regions/fi/index.html`, `apps/regions/fi/overview.html`.
- Built the Finland national religious-community membership series (Opus lane, conductor-verified), STAGED under the national-only ruling: StatFin table 11rx — administrative register membership by the Population Information System, a register construct named on every surface and never census affiliation, belief, or practice — across 36 annual waves 1990-2025 on a single geoBoundaries FIN ADM0 polygon. The 23 mutually exclusive leaves close to the published total exactly in every year; the two register slots (member of any registered community, not a member) sum to 100 by construction (1990: 89.79% members of 4,998,478; 2025: 64.08% of 5,652,881 — the Evangelical Lutheran fall from 4,390,739 to 3,457,125 rides the composition verbatim with all 26 StatFin categories per row). Data licence CC BY 4.0; the boundary is ODbL 1.0 (OSM via geoBoundaries, the ratified OSM-boundary precedent), with a National Land Survey CC BY 4.0 swap recorded as deferred. Build: `scripts/build_fi_area_summary.R`; manifest: `docs/manifests/fi-religious-community-membership-1990-2025.json`.
- Recorded five PI rulings (2026-07-12, sixth sitting): Grenada — build the 2021 preliminary parish wave with the preliminary status disclosed on every surface (the build landed the same sitting, queue row 83); Thailand — keep the shipped near-flat page, no Buddhist-reference re-emission (row 67 closed); Finland — ship national-only (the FI data lane opened, row 98); Sweden — ship national-only, with the build waiting on the probe's church-statistics licence question (row 99); Palestine — own standalone page under the build-then-ask posture with the mixed licence position disclosed verbatim (row 34, implemented the same sitting).
- Built the Grenada 2021 preliminary parish religion product (Opus lane, conductor-verified), STAGED under the PI ruling to build with the preliminary status disclosed on every surface: 7 parish units from Table 23 of the CSO 2021 preliminary census results, 26 verbatim categories with per-row composition, integer-exact to 108,279 at every margin (conductor re-derived the parish sums, the slot arithmetic 94,088 + 6,493 + 7,698, the roll-up total 44,777, and the seven distinct geometry hashes). Town of St. George rolls into St. George parish, disclosed in the manifest; the no-religion slot sums the two distinct no-religion lines (Atheist 49 + No Religious Affiliation 6,444) with both kept distinct in composition and Not Stated held in the denominator outside both slots; small_cell_under_10 fires on six of seven parishes; a census_2021_preliminary_release token leads every row's flags. CSO Open Licence Agreement accepted; boundary geoBoundaries GRD ADM1 (CC BY-SA 2.0). Regenerate on the final 2021 report. Build: `scripts/build_gd_area_summary.R`; manifest: `docs/manifests/gd-census-religion-2021.json`.
- Brought the live Palestine pages up to the licence ruling (PI, 2026-07-12; Opus lane, conductor-gated in browser): the map attribution and the overview licence surfaces now disclose the mixed position verbatim — current PCBS website terms state CC BY 4.0 while the cached 1997 and 2007 publications print All Rights Reserved, with no rights-reservation notice in the cached 2017 preliminary report — under the build-then-ask posture with single-commit revert language, resolving queue row 34's two holds (the shared-surface question was already resolved by the recorded SHIP ruling; both the PS and IL overviews carry the coexistence note). The conductor's catch on the lane's draft: "requires conductor review" was internal register on a public surface, replaced by project language everywhere, including two pre-existing occurrences on the live page. Gate verified in browser: the governorate choropleth renders, the full mixed-licence credit rides the attribution strip, console clean. Pages: `apps/regions/ps/index.html`, `apps/regions/ps/overview.html`.
- Recorded the Jamaica route probe (Opus lane, conductor-reviewed), BUILDABLE single-wave and a refuted queue premise: STATIN publishes religion by parish for 2011 (Volume 6 Table 5, counts, 19-category parish frame, integer-exact to 2,684,115 across the 14 parishes; the four small non-Christian groups are source-collapsed into Other at parish level) — the earlier sweep missed it because the thematic census volumes live on a separate subdomain from the main census index. Licence an open STATIN reuse grant with attribution, no ask needed; boundary geoBoundaries JAM ADM1 (14 parishes, CC BY-SA 2.0) joins one-to-one. The 2022 census has no religion release yet and 2001 is national-only. The Jamaica build lane opens.
- Recorded the Angola route probe (Opus lane, conductor-reviewed), HELD and a refuted queue premise: religion is asked in both the RGPH 2014 census and the IIMS 2015-16 survey but published nationally only in both; the only province-grain route is restricted microdata, closed under the standing hold. Licence consent-first (INE all-rights-reserved, no open grant located); boundary clean. Unblock: an INE request for a religion-by-province count table bundled with reuse permission.
- Recorded the Iraq route probe (Opus lane, conductor-reviewed), HELD on two independent gates: the 2024 census recorded broad religion on the enumeration form (five codes, no sect) but published no religion table at any grain, and the licence is consent-first (exclusive state ownership with a legal-prosecution clause, quoted verbatim in the probe). Sensitivity is on — any future Iraq product follows the Myanmar sensitive-page discipline. Unblock: a single CSO/KRSO data-and-reuse request.
- Backfilled area-summary.v2 composition on four more products (Opus lanes, conductor-verified), completing the recorded re-emit list behind the fourth sitting's FJ/TK backfill: Cambodia (50 province-waves, the four verbatim Table 2.5.1 percentages per wave with the 2008/2019 label variants kept distinct, printed zeros emitted as genuine values), Solomon Islands (20 province-waves, 13 verbatim SINSO categories in 2009 and 17 in 2019, counts reconciling exactly against the slot fields), Marshall Islands (25 atolls, all sixteen Census Volume 1 Table 9 categories as printed integers with no suppression in source), and Sri Lanka (92 of 100 district-waves; the eight null-coverage rows — the seven unenumerated 2001 districts and Kilinochchi 1981 — omit composition entirely, absent rather than zeroed). Every product moves to GATE PASS (area-summary.v2) with pre-existing fields byte-identical, verified by the conductor by reconstruction against HEAD (Cambodia's builder re-stamps generated_at, the one accepted regeneration field); labels ship verbatim per source and taxonomy codes attach only where the printed label names the tradition. Scripts: `scripts/build_kh_area_summary.R`, `scripts/build_sb_area_summary.R`, `scripts/build_mh_area_summary.R`, `scripts/build_lk_area_summary.R`.
- Launched the Bangladesh two-wave page, completing the fifth sitting's staged data: the district map now reads the two-wave product, the year control switches between the 2011 and 2022 censuses, and a Muslim-share change metric rides the stable 64-zila frame under the Peru two-wave precedent. Gate verified in browser: the 2011 wash renders with Bhola as the sole no-data zila and its popup reads em-dashes, never zero; Lalmonirhat's popup fires the fewer-than-ten dagger on its 2011 row only, with the verbatim composition including the Buddhist count of 5; Rangamati reconciles on both waves (35.1% to 36.3%); change renders on the 2022 wave and nothing on 2011 (the first-wave rule); console clean on a fresh load. Every numeric claim on both pages was re-derived against the committed product before adoption (63-zila sum 142,266,902; 2011 national composition Muslim 90.32%, Hindu 8.60%, Christian 0.31%, Buddhist 0.63%, Others 0.14%; frame closure to 144,043,697 with deviation zero). The conductor's one catch against the staged page diff was the hub card's stale one-wave badge, now the two-wave badge. Pages: `apps/regions/bd/index.html`, `apps/regions/bd/overview.html`.
- Re-emitted the Bangladesh two-wave district product under area-summary.v2 with the per-row composition field (Opus lane, conductor-gated): 127 populated rows carry the five verbatim BBS categories (64 in 2022, 63 in 2011); Bhola 2011 stays withheld with no composition, absent rather than zeroed; every pre-existing field and quality_flag survives byte-identical, proven by reconstruction against HEAD, and the slot arithmetic reconciles on all 127 rows. The BD manifest's recorded product hashes await the next full builder run (the surgical re-emission deliberately left geometry and the superseded 2022 manifest untouched). Script: `scripts/build_bd_area_summary.R`.
...
- Added the Bangladesh 2011 wave, turning the district product two-wave (Opus lane, conductor-gated), STAGED: all 64 Community Reports assembled (63 newly retrieved, every URL and sha256 pinned), with 62 zilas extracting exactly from Table C-13 — every zila's five categories sum to its printed total — and the national frame closing to the enumerated anchor 144,043,697 with deviation zero. Two source-PDF defects were confirmed on re-download and handled without repair: Lalmonirhat was RECOVERED under the conductor's Zila-Series ruling (Table P05, full text layer, frame verbatim, total matching the Vol-2 enumerated figure exactly, cross-checked against the same report's PT14; its Buddhist cell of 5 fires the fewer-than-ten token), and Bhola stays WITHHELD as a null-metrics row because both its Community Report (pages 229-240 omitted) and its Zila-Series copy (image-only religion tables) fail the text-layer rule — the unblocks (a text-based mirror, a re-issued BBS copy, or an OCR ruling) ride the manifest's deferred sources. Both waves carry the minority-share two-slot design; the universe difference (2011 full enumerated; 2022 sex-classified excluding 8,124 hijra) is disclosed per wave and immaterial to shares, so change is supported on the stable 64-zila frame. The live page still reads the frozen 2022 files; surfacing the 2011 wave is a follow-up page lane. Staged under the BBS ship-and-ask posture (task 1; the outgoing draft covers both waves). Build: `scripts/build_bd_area_summary.R`; manifest: `docs/manifests/bd-census-religion-2011-2022.json`, superseding the 2022 manifest bidirectionally.
- Fixed the mobile onboarding card (PI direction to the ES lane, conductor-verified): the "About this map" card on every data-map page was fixed-position with no height cap, so long onboarding copy on phones pushed the close button below an unscrollable fold; the card now caps at the viewport (dvh with a vh fallback, safe-area aware) and scrolls internally with scroll-chaining contained — verified at 390px, where 1,305px of copy scrolls inside a 662px card with the actions reachable. `maplibre-flat.css` also gains a fleet-wide cache pin (`?v=20260712d`, 92 pages) so phones pick up this fix and the earlier Data-maps navigation fix promptly.
- Launched the Spain autonomous-community research map (hub to 91), the second survey-construct launch of the sitting: single wave 2025 from the CIS Estudio 3505 open microdata on the 19 units, gate verified in browser — the choropleth renders with the PI small-cell ruling applied exactly (the wash fires on precisely Aragón, Asturias, La Rioja, Ceuta, and Melilla, enforced by a build gate that stops the script if the set ever diverges), the Aragón popup carrying both ‡-marked shares on its n=98 base with the seven-category composition and the ruling stated in the footnote, and the ruling also stated on the intro and overview. The product was re-emitted into per-construct files at the conductor's direction (the Netherlands pattern): the self-definition file carries only its own tokens, and the mass-attendance construct ships as a data-only companion file (12 sub-100 bases on the filtered believer subset) carried in the overview and popup context, with attendance-as-map-metric recorded as an open runtime design item. Pages: `apps/regions/es/index.html`, `apps/regions/es/overview.html`.
- Launched the Argentina macro-region research map (hub to 90), the corpus's first survey-construct launch under the 2026-07-12 rulings: two CEIL-CONICET waves (2008, 2019) on the six standard macro-regions, gate verified in browser — the six regions render on both metrics with the year slider live, the Patagonia popup exact against the source (2008 affiliation 88.3 / no religion 11.7; 2019 75.4 / 24.3 with the Católica 51.0 composition), survey-estimate language on every surface, the 2019 lower-bound disclosure riding the legend and popup, and no change metric (frames differ and the 2019 affiliation is a printed-only lower bound). Two conductor fixes at the gate: the boundary file gained `area_name` display names on its six features (manifest sha256 re-hashed, validators 99/99), and the popup's duplicated caveat paragraph was trimmed to a distinct denominator note. Licence blocks state the CC BY-NC-SA alignment between the 2019 source and the project's own terms. Pages: `apps/regions/ar/index.html`, `apps/regions/ar/overview.html`.
- Recorded the Antigua and Barbuda capture attempt, HELD on a server-side engine fault: with task 8 ruled, the conductor ran the live REDATAM session end-to-end — form mechanics proven, but the ATG engine 404s its own temp output even for its untouched default crosstab, so no number was captured and none substituted. Unblocks: a later-sitting retry (minutes of work; mechanics recorded) or a Statistics Division ask (`info.stats@ab.gov.ag`, from the portal's own footer). Record: `research/countries/ag/build-attempt-2026-07-12.md`.
- Bumped the licence prominence on every web surface (PI request): the map attribution chip is bold, and the overview footer note is larger and brighter with an emphasised licence link.
- Adopted the project licence, CC BY-NC-SA 4.0 (PI ruling, 2026-07-12; DECISIONS.md D19): the project's original work — code, docs, map pages, manifests, derived summaries, figures — now carries Creative Commons Attribution-NonCommercial-ShareAlike 4.0, chosen over BY-NC-ND because share-alike is what makes downstream users inherit the project's restrictions, which the PI required. Implemented on every surface: `LICENSE.md` (with the carve-outs — per-manifest source licences govern source data, and share-alike sources like Chile's CC BY-SA and OSM's ODbL prevail for their products), the README, the hub, the shared runtime's attribution line on all 89 map pages, and the shared overview footer; fleet asset pins bumped to 20260712d so cached clients pick the change up.
- Recorded three PI rulings (2026-07-12): Spain task — unweighted survey respondent n IS the small-cell wash denominator, stated on the map's information surfaces; the ES page lane opens. Argentina — the route-probe is accepted, and the CC BY-NC-SA source question dissolves under the project's own licence adoption; the AR page lane opens. Task 8 — "PI and conductor agree": the Guinea treatment stands ratified and the single-wave-subnational reasoning clears Antigua and Barbuda's 2011 parish product; an AG build lane opens.
...
### 2026-07-12 (census-note sitting)

- Added two researcher-facing documents and linked both from the README: `docs/religious-change-highlights.md`, which describes what the shipped census-religion products document about religious change (Belize district series, a 39-country first-versus-latest-wave comparison, Chile commune distributions, counter-directional falls, and the held-out groups with reasons), and `docs/using-the-census-religion-data.md`, a tutorial covering the product anatomy, the two-slot design and its minority-share trap, a worked Belize example, the cross-wave comparability checklist, and licence-and-attribution practice. Every number is recomputed from shipped products by the new `scripts/build_census_religion_note_figures.R`, which emits the note's three SVG figures and country table to `docs/assets/census-religion-note/` and refuses minority-share products by indicator declaration. Register/membership series, survey-instrument breaks (Lithuania), mid-series frame changes (Cook Islands), the Kazakhstan refusal signal, and single-wave products are excluded from the comparison and documented as such. (Branch opened 2026-07-12, merged 2026-08-25 after a numbers re-check against the current products.)
