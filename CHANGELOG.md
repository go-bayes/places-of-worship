...
### 2026-09-24 (Clerk sessions, contributor access C1)

- Both portals now sign people in through Clerk (Google or an email code), which keeps a session for up to seven days across reloads and a phone that discards the tab, and mints a sixty-second Convex token per request from its `convex` JWT template. This replaces the raw Google ID token of at most an hour that Guy's Port Vila field day kept losing; the device copy `localStorage["powConvexAuth:v1"]` is removed on load. `convex/auth.config.ts` lists the Clerk provider (`CLERK_JWT_ISSUER_DOMAIN`, application id `convex`) beside Google, which stays for one rollback week (R-C3). Convex still decides access: a Clerk account with no invitation sees its address, one line saying it has no project access, and a sign-out button, never the portal.
- New tables `user_identities` (sign-in identifiers linked to a user besides `auth_subject`) and `role_events` (append-only record of invitations, claims, re-keyings, resets and role changes). A shared `resolveUser` serves `requireUser`, `me` and `claimInvite`, so a member re-keyed from Google to Clerk still resolves to the same row under a Google token. `claimInvite` now requires a verified email, never activates or re-keys a disabled or service row, re-keys an active member only from an issuer on `AUTH_MIGRATION_SOURCE_ISSUERS` to the Clerk issuer (empty disables it), and activates a pending row found by its own identifier, which repairs the lock-out `inviteUser` caused by resetting an active member to pending. `users:adminResetAuthSubject` (CLI only) repairs a stuck account. The change is additive: new tables, new functions, no field removed. Design: contributor-access brief, sections 4.2 to 4.4.
- Review repairs (gpt-6-sol and gpt-6-astra on PR #153): the sign-out button reports completion only once Clerk confirms it; a refused sign-out (Clerk drops its local copy of the session even then, so a reload would bring it back) shows "Sign-out did not finish…" with `Try sign-out again`, and a reload retries the sign-out rather than restoring the session. The portals register their session-ended handler before any restore, so a session restored on load and then ended in another tab still clears the page. An ended session now clears the page as the sign-out button does (tasks, the open task's evidence, draft caches, forms), while unsent guided and rapid drafts stay on the device carrying their owner's user id and come back only for that user. The publishable key must be well formed and name an approved Clerk Frontend API host (`sure-lizard-50.clerk.accounts.dev` today). Clerk's Google button and inputs take the portal's control outline; its header stays, since later steps name the address a code went to. Test `session-end-dom.test.cjs` added to CI; stamps `?v=20260924-clerk2`.
