...
- Archived first-pass research records can now be submitted to the shared backend for receipts, the second step of the agent-judgment design ruled on 2026-09-19 (R-J1 to R-J7; JB's go-ahead for J2, 2026-09-24). `first_pass.py submit HASH --store PATH --deployment dev|local` verifies a record's parent graph and sends each record, parents first, to the internal mutation `firstPassReceipts:ingestFirstPass`, behind the existing `POW_INTERNAL_AGENT_INGEST_ENABLED` gate. The backend checks the exact bytes against their SHA-256, re-applies the first-pass schema and semantic rules, requires each parent to hold a receipt for the same place, checks any portal `context` against the task, draft, and evidence version it names, and writes one `agent_first_pass_receipts` row (`first-pass:<sha256>`, storage tier `convex_only`, bytes kept until an independent copy verifies). A record with a dossier also yields a `status_assessment` judgment of the place and an `annotation` judgment per claim annotation in `agent_judgments`. An identical retry returns the existing receipt and writes nothing. `first_pass.py restore` rebuilds a record's history in a clean local archive from the receipts and verifies every hash. Reviewers, curators, administrators, and the PI can read receipts with `getFirstPassReceipt` and `listFirstPassReceipts`. The receipt contract (`object-receipt.v1`, `convex/lib/objectReceipts.ts`) is general so that a later inspection import can reuse it. The backend change is additive: one new table, one new module, and no change to any existing table, argument, or return type. The receipt grants nothing: it creates no task, draft, version, or review decision, and every record stays provisional. A synthetic researched fixture, `scripts/agent_research/fixtures/first-pass-researched.json`, joins the tests.
