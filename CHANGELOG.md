...
- Added two options briefs prompted by the project lead's immersive-exploration and circulation-signal ideas. `docs/development/immersive-exploration-options-2026-09-10.md` compares rendering routes (MapLibre 6 globe and terrain, keyless terrain and building sources, CesiumJS with Google Photorealistic 3D Tiles, street-level and 360 imagery, WebXR) with verified terms and prices, records that Google's Maps Platform Terms bar Google 3D Tiles and Street View imagery on the same screen as a non-Google map, and proposes a keyless staged plan with nine open rulings (R-I1 to R-I9). `docs/development/circulation-signals-options-2026-09-10.md` frames footfall and presence signals as evidence about the worship-function state, registers the sources with their coverage, resolution, cost, terms and privacy models, records that Stats NZ disestablished Data Ventures in December 2023 and that no open New Zealand small-area temporal presence dataset exists, sets out the law and a ten-point aggregate-only policy, and designs a New Zealand calibration pilot with nine open rulings (R-C1 to R-C9).
- Staged a keyless immersive pilot at `apps/regions/_staged/immersive/` (not linked from any navigation): MapLibre GL JS 6.9.0 globe with Mapterhorn terrain and hillshade, OpenFreeMap building extrusions, project place tiles, the NZ dated-places product with a year filter, an eight-stop Canterbury Anglican tour whose title cards come from the data, Street View link-out, and a shareable camera-and-stop URL hash. Verified in headless Chrome at desktop and phone widths; real devices remain unverified.
- Added `schemas/circulation-signal-register.v1.json` and the filled register `docs/development/circulation-signal-register.v1.json` (kept beside the brief because `data/` is git-ignored) (31 entries, validated), and `scripts/circulation_calibration_design.R`, which builds the stratified site frame from the existing NZ data and draws a seeded probability sample for a future calibration study; its results note is `docs/development/circulation-calibration-design-note-2026-09-10.md`. No signal data were used because none is openly available.
