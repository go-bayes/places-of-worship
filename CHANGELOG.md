...
- A `Quick photo` button under `Add / Revise` on the contributor portal (JB, 2026-09-22: "a quick photo entry button, where a user snaps a shot of a possible PoW for further (potentially AI) review"). One press opens the phone's camera (`capture="environment"` on a hidden file input; a computer offers its file picker) and asks for the position at the same time. The photo shows on a card with the fix ("about 8 m", the blue ring on the map, the map moved to the spot) and an optional name or note; `Send for review` lands it on the rapid lane as a flagged partial entry (`flagForDiscussion`, `could_not_determine`, basis `other`, privacy `needs_review`, today's date), with an approximate-area location assertion whose radius is the fix's accuracy clamped to the contract's 25 m to 100 km and basis `local_investigator_account`, confidence moderate to 50 m and low beyond. The task lands as `unresolved_note` in the review queue with the note "Quick photo capture: ... details not entered." and the photo uploads against it on the recorded screen (caption `Quick photo capture`), through the existing pending-files path. The nearby check runs against the fix; `placement_zoom` is the map's zoom after it moves to the spot. No position (refused, unavailable, timed out): `Send` stays closed and `Drop the pin instead` carries the photo into the ordinary add flow, counted on the pin form's picker and uploaded with whatever the contributor saves there; leaving that flow drops it. A failed send keeps the card and reuses its submission id, so a retry cannot land twice. `Add / Revise` reads `Cancel` while the card is open and asks before discarding the photo; Escape does the same. The button shows only signed in, in `Add / Revise`, with attachment storage wired (one probe per sign-in) and no entry open. Nothing in the backend changed: the Claude batch-review lane does not yet read attachments, so "AI review" of the photo is a follow-up on that lane; the note and the caption mark the captures for it. Test `quick-photo.test.cjs` in CI (six checks); style guide and RA guide updated; stamp `verification-map.js?v=20260922c`. Unverified on a device: the camera-first input and the geolocation prompt ordering on iOS Safari.
