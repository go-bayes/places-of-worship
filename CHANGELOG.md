...
- Moving an existing Google member to the new sign-in needs proof that the person holds that Google account, not only its mailbox (R-C18, JB 2026-09-24), and the proof is bound on the server. The member signs in with the new sign-in first; `users:requestIdentityMigration`, called by that sign-in with its verified email, opens a pairing between it and the Google-bound member row (new table `identity_migration_pairings`; ten minutes; one open pairing per member; only a hash of its nonce stored). `users:approveIdentityMigration`, called by the member's current Google sign-in, approves that pairing and never learns which sign-in asked. `claimInvite` re-keys only with an approved, unexpired, unspent pairing for that very sign-in and member, and spends it in the same transaction, so a verified email alone, or an approval meant for another sign-in, never moves an account. `role_events` records `pairing_requested`, `pairing_approved` and `pairing_consumed`. On the portals the confirmation appears only to a member signed in with the new sign-in: "Confirm your existing account for the new sign-in", with Google's own button; nothing is kept in the browser, and the step stops if its session ends or changes.
- The publishable key must name an approved Clerk host before any Clerk script loads. The RA guide's sign-in paragraphs, the style guide's sign-in card, the Convex function inventory and the setup notes are updated. Tests: `convex/users.node-test.mjs`, `convex-task-client.test.cjs`, `session-end-dom.test.cjs` and `basemap-and-drafts.test.cjs`, all in CI. Stamps `convex-task-client.js?v=20260924-clerk9`, `state.css?v=20260924-clerk9`, `convex-config.js?v=20260924-clerk6`, `review-portal.js?v=20260924-clerk5`, `verification-map.js?v=20260926-clerk6`.
