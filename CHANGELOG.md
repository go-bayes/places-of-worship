...
- PR-E of the ruled temporal plan — census-year states derived from the periods entry on assigned tasks (`docs/development/assigned-task-periods-brief-2026-09-02.md`, JB rulings R-E1–R-E4 of 2026-09-02). In the guided form the period cards replace the hand-set target-year grid as the temporal entry; the grid survives collapsed under "Set census-year statuses by hand" with a required reason, and the action no longer prefills it unless it is open. A preview strip under the cards states what the periods will propose for each census year, with the rule in words, and flags a conflict with any hand-set status before submission; it runs on a verbatim client port of the server's presence rules (`occupancy-contract.js` gains `derivePresence`, `describePresence`, `gapBounds`; `convex/lib/occupancyDerivationMirror.node-test.mjs` holds the two equal on a fixture set that fires every rule). After the first complete card the form asks once whether there was a spell with no worship here; "Yes" adds the later period, "Not sure" takes what the RA knows as bounds (an end between two dates or unknown, a start by a date) and never an invented date, so the doubtful years derive uncertain and the rest present or absent (ruling R-E4). On submit the periods follow the parent evidence in one flow, resting on the same source and confidence unless the RA unticks that and fills the periods' own source block; if the periods call fails the parent stands and the cards reopen in the periods pane. A period at a different place uses the same pin flow and returns to the form. Client only. Files: `apps/regions/nz/js/{verification-map,occupancy-contract}.js` (stamps `20260902l`, `20260902h`), `apps/regions/nz/verification.html`, `apps/guides/ra.html`, `convex/lib/occupancyDerivationMirror.node-test.mjs`, `docs/development/assigned-task-periods-brief-2026-09-02.md` (rule 2 basis note).
