...
### 2026-09-24 (Clerk sessions)

- Both portals now sign people in through Clerk, with Google or an emailed code. A session lasts up to seven days across reloads and a phone that discards the tab, and each request carries a sixty-second Convex token from Clerk's `convex` JWT template, replacing the Google ID token of at most an hour; the old device copy `localStorage["powConvexAuth:v1"]` is removed on load. `convex/auth.config.ts` lists the Clerk provider (`CLERK_JWT_ISSUER_DOMAIN`, application id `convex`) beside Google, which stays for one rollback week. Convex still decides access: a Clerk account with no invitation sees its address, one line saying it has no project access, and a sign-out button, never the portal.
- New tables `user_identities` (sign-in identifiers linked to a user besides `auth_subject`) and `role_events` (append-only record of invitations, claims, re-keyings, resets and role changes). A shared `resolveUser` serves `requireUser`, `me` and `claimInvite`, so a member moved from Google to Clerk still resolves to the same row under a Google token. `claimInvite` requires a verified email, never activates or re-keys a disabled or service row, re-keys an active member only from an issuer listed in `AUTH_MIGRATION_SOURCE_ISSUERS` to the Clerk issuer (empty disables it), and activates a pending row found by its own identifier, which repairs the lock-out `inviteUser` caused by resetting an active member to pending. `users:adminResetAuthSubject` (CLI only) repairs a stuck account. The backend change is additive.
- Sign-out reports completion only once Clerk confirms the session has ended on its server; otherwise the card says the sign-out did not finish, offers `Try sign-out again`, and a reload retries rather than restoring the session. A session ended in another tab or by expiry, a session replaced directly by another account's, and a token the backend refuses all clear the page as the sign-out button does, before the replacement is admitted or the caller hears of the refusal; a request is never sent under a session other than the one it was made in. The cleanup also empties the RA portal's my-work and past-submissions panels and closes the list; a user restored on load is admitted only if the same session is still the page's once tasks have loaded; and only an actual error response, or an HTTP 401 whatever its body, is read as a refused sign-in. A session end also closes an open quick photo (its preview URL revoked, file inputs emptied, a photo picked or a position answered after the end dropped, and a capture never sent by another session), resets the pin, occupancy, issue and evidence-reminder state, closes the map popup and clears the status line; late continuations of the ended session (a draft load, an upload's completion, a post-save upload, a debounced device copy) leave the next session's page alone. On the review portal a late startup restore no longer replaces a reviewer the new session admitted. The client counts every session change, so work begun under a session is recognised as stale even when the same session returns (A to B and back to A): a request is neither sent nor answered, and a sign-in chain admits nobody, across such a switch. A position fix or address search serves only the pin entry that asked for it, in the same session. A session found on load is signed out, never silently restored, when a failed sign-out's marker cannot be kept because the browser's storage refuses it. A submission the server records after the session changed still removes the submitter's own device copy of what it sent, and a kept rapid draft carries its submission id, so a retry is recorded once. A submission id belongs to the content it was sent with: an edit after sending mints a fresh one, and a late receipt clears only the version it sent. A guided draft save that is recorded removes the exact device snapshot it carried. Periods of an observation recorded after its session ended are parked against that task, never left for the next place, and sent at the submitter's next sign-in. Moving an existing account: pairing requests are limited per sign-in rather than per member (six an hour), and a newer request no longer revokes a pairing the member's Google sign-in has already approved. Every signed-in request on both portals is tied to its session, and a response that arrives after sign-out, or for another user, is dropped: on the review portal this covers the queue, task and occupancy loads, attachment links, claims and releases, extra opinions, returns for comment, derived-year decisions and recorded decisions; the task-history cache serves only the user it was fetched for.
...
- Moving an existing Google member to the new sign-in needs proof that the person holds that Google account, not only its mailbox (R-C18, JB 2026-09-24), and the proof is bound on the server. The member signs in with the new sign-in first; `users:requestIdentityMigration`, called by that sign-in with its verified email, opens a pairing between it and the Google-bound member row (new table `identity_migration_pairings`; ten minutes; a newer request revokes unapproved open pairings; at most six requests per sign-in in any hour; only a hash of its nonce stored; every read of the table is bounded). `users:approveIdentityMigration`, called by the member's current Google sign-in, approves that pairing and never learns which sign-in asked. `claimInvite` re-keys only with an approved, unexpired, unspent pairing for that very sign-in and member, and spends it in the same transaction, so a verified email alone, or an approval meant for another sign-in, never moves an account. `role_events` records `pairing_requested`, `pairing_approved` and `pairing_consumed`. On the portals the confirmation appears only to a member signed in with the new sign-in: "Confirm your existing account for the new sign-in", with Google's own button; nothing is kept in the browser, and the step stops if its session ends or changes.
- The publishable key must name an approved Clerk host before any Clerk script loads. The RA guide's sign-in paragraphs, the style guide's sign-in card, the Convex function inventory and the setup notes are updated. Tests: `convex/users.node-test.mjs`, `convex-task-client.test.cjs`, `session-end-dom.test.cjs` and `basemap-and-drafts.test.cjs`, all in CI. Stamps `convex-task-client.js?v=20260926-clerk13`, `state.css?v=20260924-clerk9`, `convex-config.js?v=20260924-clerk6`, `review-portal.js?v=20260926-clerk6`, `verification-map.js?v=20260926-clerk11`.
...
- Archived first-pass research records can now receive receipts from the shared review backend, so the judgments they carry are recorded beside other review evidence. The backend accepts a record only in the archive's exact wire format, re-applies its schema and rules, refuses text carrying personal details, and requires each earlier revision to hold a receipt for the same place. A receipt grants nothing: it creates no task, draft or review decision, every record stays provisional, and an operator can rebuild and verify a record's history from its receipts.

### 2026-09-24 (agent provenance repairs)

- Agent-assisted research records now carry stronger provenance (JB, 2026-09-24). A claim citing a source outside the agreed list of source domains for its country is refused and counted, a record must name the model the provider reported and not only the model requested, and cost is read for every model that billed, including helper models the provider chose. The Python, TypeScript and Rust validators apply one host-normalisation policy, held to agreement by shared test cases. A parish's own website is refused until the source list names it.
