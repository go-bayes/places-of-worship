...
- Agent research and validation pipeline: design brief and pilot (JB, 2026-09-10, after Joseph Watts's one-place evidence dossier for St Martin's, Loburn, and his question about "a swarm of agents"). **Brief** `docs/development/agent-research-pipeline-brief-2026-09-10.md`: the per-place research dossier as a first-class artefact between the annual OSM audit and the review portal, with a row-by-row mapping of Watts's spreadsheet onto the existing contracts and seven versioned field additions the schema lacks (OSM version chain, source date distinct from retrieval date, evidential weight, a status assessment with basis, a stale-OSM flag, reader agreement, a hashed quarantine); the architecture (independent readers from different model families with no shared context, a deterministic validator, per-claim agreement, escalation of disagreement to a human task, reviewer confirmation, PI acceptance) and how it satisfies R-A1, R-A6, R-A7 and the locator rule; safety and controls (fetched content as data, allowlists, personal-details quarantine at capture, the kastom gate, licences and robots per source, cost caps, idempotency, reproducibility); a cost model from verified prices and the pilot's measured usage; rulings applied and eleven open rulings R-R1 to R-R11 including the Sonnet tension between `convex/claudeReviews.ts` and R-A4. **Pilot** `scripts/agent_research/` (standard library plus openpyxl for the import, `uv run`): `schemas/agent-dossier.v1.json`; `import_watts_xlsx.py` (the committed fixture is redacted); `research_place.py` with `claude` (`claude -p`, WebSearch and WebFetch only), `codex` (`codex exec`, live search, read-only sandbox) and `openrouter` (implemented, untested: no key) backends, each writing full provenance; `validate_dossier.py` (robots, one request per second, quote-on-page check, OSM API centroid and version-chain check, agreement); `run_pilot.py` and the report under `reports/`; `test_agent_research.py` (14 tests). Nothing writes to Convex or any live surface.
