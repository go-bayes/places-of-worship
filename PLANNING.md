...
The RA pilot should keep the map as the starting point. The shared backend is
now the preferred evidence-entry path; spreadsheet rows should remain an export
or fallback/debug output, not the main RA workflow.

### New or missing places of worship

RAs need a clear way to nominate places of worship that are not already on the
project map or master list. "Missing" does not always mean absent from OSM:
OSM may already contain a candidate object that the project has not accepted or
linked. The current assignment page can save evidence against seeded Convex
tasks, but it cannot yet create a new standalone candidate from the map. The
next reusable build step is a Convex-backed `Nominate missing PoW` flow that can
serve the NZ pilot, Vanuatu, later country task maps, and the larger public
site:

1. Add a visible `New place` or `Nominate missing PoW` control near the task
   list and map.
...
4. Before producing a row, show nearby existing sites and ask the RA whether
   ...
   them.
5. Call the shared Convex manual-candidate mutation to create a provisional
   `candidate_site_id` and ordinary review task. This id is not a master
   `site_id`.
6. Reuse the same evidence form, validation rules, target-year controls, source
   fields, and save/submit buttons as selected-site work.
7. Produce the same export shape as selected-site work, with `candidate_site_id`,
   blank `matched_current_site_id` unless a plausible match exists, any
   relevant OSM candidate id stored as source or matching evidence, and
   `review_status = needs_review`.
8. Keep the component country-config driven so NZ can use 2013/2018/2023,
   Vanuatu can use 1989/1999/2009/2020, and later country maps can supply their
   own target years, source types, date bounds, and nomination options.
9. Preserve the master boundary: nominations create provisional tasks only.
   Reviewers may reject, link, merge, split, request more evidence, or accept
   for export to `pow`; no nomination writes directly to the master or public
   map.
...
The current map can now write to Convex when the deployment is configured and
the RA signs in. Browser `localStorage` and spreadsheet copy remain fallback
paths only. Convex is the preferred near-term layer because the task map needs
live multi-user state more urgently than it needs canonical geospatial storage.
The task backend should define tasks and provisional task state without
accepting data into the master. A minimal task record should include:

1. stable `task_id`,
2. `matched_current_site_id` or `candidate_site_id`,
3. assignment or editor metadata,
4. status such as `open`, `in_progress`, `provisionally_closed`,
   `needs_review`, `reviewed`, or `reopened`,
5. linked evidence row ids,
6. timestamps and editor initials or authenticated user id,
7. reviewer notes and reopen reason.

The map should read this task state to prevent accidental duplicate labour and
write provisional task state when an RA saves, submits, or skips a task. Review then
...
Every row generated by a country map or country evidence sheet should make that
country's target years explicit. For New Zealand estimation work, the target
years are 2013, 2018, and 2023. For the first Vanuatu protocol, the target
years are 1989, 1999, 2009, and 2020.
...
1. For each country target year, record one of `present`, `absent`,
   `uncertain`, or `not_assessed`.
...
### Country-Specific Target Years And Time Slider

Do not hard-code New Zealand target years into the general interface model. The
country configuration should supply:

1. `country_code`,
2. country display name,
3. target years for structured status entry,
4. minimum lifecycle year accepted by the interface,
5. timeline anchor years for review and visualisation,
6. source and geography notes for the RA brief.

Default country settings:

| Country | Target years | Minimum lifecycle year | Notes |
| --- | --- | ---: | --- |
| New Zealand | 2013, 2018, 2023 | 1800 | Current pilot and map UI. Earlier dates may be entered when sources support them. |
| Vanuatu | 1989, 1999, 2009, 2020 | 1600 | Source-first protocol for historical, mission, colonial, and census-linked evidence. |

The time slider should be a review and visualisation tool rather than the
primary data-entry surface. It should show reconstructed states and diffs once
evidence exists: present, absent, uncertain, not assessed, gain, loss, and
changed use. Early years with no evidence must display as unknown or not
assessed, not as absence of worship use.

For Vanuatu, the first slider design should range from 1600 to the current
year and snap to known evidence years, census years, OSM snapshot years, and
historical anchors such as 1606, the 1840s, 1887, 1906, 1967, 1980, 1989,
1999, 2009, and 2020.
...
- Convex: pricing and limits checked on 2026-05-13 at
  `https://www.convex.dev/pricing` and
  `https://docs.convex.dev/production/state/limits`. Start the pilot on Free,
  move to Starter only on quota pressure, and reserve Professional for daily
  backups, log streaming, exception reporting, support/compliance needs, or
  sustained high usage. Keep raw OSM snapshots, media, accepted diffs, and
...
1. Keep the hosted Convex pilot stable for the 50-case web assignment:
   preserve Google sign-in, invited-user access, the seeded
   `nz-temporal-ra-workpack-001` batch, and the public `placesmap.org`
   assignment link while André starts work.
2. Keep the first Convex pilot on Free and monitor function calls, storage, and
   database I/O. Move to Starter only on quota pressure; move to Professional
   when backups, log streaming, exception reporting, support/compliance needs,
   or sustained high usage justify it.
3. Add a `country_configs` table to Convex and seed NZ and Vanuatu settings:
   target years, lifecycle lower bound, source-type options, map centre/zoom,
   and country-specific nomination options. Use this to remove hard-coded
   country settings from the frontend and manual candidate tasks.
4. Ask André to use the assigned web link
   `https://www.placesmap.org/apps/regions/nz/verification.html?batch=nz-temporal-ra-workpack-001`
   for the first batch. `Save draft` and `Submit for review` are the preferred
   path; copying spreadsheet rows is fallback only.
5. Implement the reusable Convex-backed `Nominate missing PoW` flow for
   assignment mode. It should call the existing manual-candidate mutation,
   create a provisional task, and then reuse the same evidence form so the
   component can later serve Vanuatu and the larger public site.
...
8. Finish and commit the OSM date-tag implementation with an automated
   parser/gain/loss fixture so the target-year derivation is protected against
   regressions.
9. Promote the two New Zealand OSM "places to check" files from local execution
...
10. Add the first reviewer export path from Convex evidence drafts to
   `site_evidence_wide.csv` and validate it with `pow`. The initial bridge now
   exposes a Convex file bundle and local materialiser; the next action is to
   exercise it on one accepted New Zealand decision end to end.
11. Add the first authenticated review portal surface: a role-gated queue where
    JB/JW can inspect submitted evidence, record accept/reject/defer/revise or
    needs-more-evidence decisions, and push the decision state back to the
    RA-facing task history. Treat the current workpack as a batch filter over
    the shared task list, so later New Zealand batches and Vanuatu tasks merge
    into the same coordination layer.
12. Decide whether the first durable storage action is a project Google Drive
...
13. Curate a small RA-ready set of places to check from the 4,777 OSM
...
14. Ask André to contact selected church bodies for source-backed records of
...
15. Finish the next NZ cleanup slices from the remaining review queue.
16. Design the shared global cleaner and review-queue schema.
17. Refactor `scripts/extract_global_data.R` into explicit extract, normalise,
   clean, and export stages.
18. Promote any reusable ignored local outputs and any Drive-only project data
    ...
    instructions.
19. Add hash-backed data manifests and per-country counts. Start with the NZ OSM
...
20. Define the shared country backend schema and NZ boundary adapter contract.
21. Validate the first NZ territorial-authority `area_summary` product against
    frontend layer and download needs.
22. Pilot the RA-facing historical evidence templates in
    ...
    broader data entry.
23. Define the first read-only NZ master site-data export and automated
    verification checks described in
    `docs/master-verification-workflow-plan.md`.
24. Continue converting the RA map-triage guide into the first map-first
    ...
    map. Convex-backed save/submit is now the preferred RA path; local row
    generation is only fallback and testing support.
25. Scope the OSM temporal verification subproject as an annual-snapshot
...
28. Draft the authenticated NZ portal staging pilot described in
...
29. Extend the NZ `area_summary` product to SA2 geography after checking
    boundary metadata and point-to-area assignment quality.
30. Replace 2018-specific NZ overlay assumptions with year-aware map controls,
    legends, popups, and export metadata.
31. Align the NZ map interface with the global map after the data overlay is
    stable, preserving NZ-specific analysis controls.
32. Prototype site, area, and comparison modes using precomputed layers before
    adding live portal queries.
33. Pilot the new global pipeline on a small country set before full rollout.
34. Expand `research/` into a country-source matrix for global feasibility
    assessment.
