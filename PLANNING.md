...
linked. The current nomination panel is only a local JSON preview. The next
...
5. Produce the same wide evidence row shape as selected-site work, with a
   ...
   match exists, any relevant OSM candidate id stored as source or matching
   evidence, and `review_status = needs_review`.
...
1. Configure the hosted Convex pilot for the 50-case web assignment: set the
   setup token, deploy Google OpenID Connect auth, create pending invites for
   JB/JW/André, import
   `exports/convex-task-seed/nz-temporal-ra-workpack-001.json`, and enable
   `apps/regions/nz/js/convex-config.js` with the public Convex URL and Google
   client id.
...
5. Start Guy on the Vanuatu source-first protocol in
...
6. Generalise country target-year configuration before building a Vanuatu map
   intake surface. NZ should continue to show 2013, 2018, and 2023; Vanuatu
   should show 1989, 1999, 2009, and 2020.
7. Finish and commit the OSM date-tag implementation with an automated
   parser/gain/loss fixture so the target-year derivation is protected against
   regressions.
8. Promote the two New Zealand OSM "places to check" files from local execution
...
9. Add the first reviewer export path from Convex evidence drafts to
   `site_evidence_wide.csv` and validate it with `pow`.
10. Decide whether the first durable storage action is a project Google Drive
...
8. Curate a small RA-ready set of places to check from the 4,777 OSM
...
9. Ask André to contact selected church bodies for source-backed records of
...
10. Finish the next NZ cleanup slices from the remaining review queue.
11. Design the shared global cleaner and review-queue schema.
12. Refactor `scripts/extract_global_data.R` into explicit extract, normalise,
   clean, and export stages.
13. Promote any reusable ignored local outputs and any Drive-only project data
    ...
    instructions.
14. Add hash-backed data manifests and per-country counts. Start with the NZ OSM
...
15. Define the shared country backend schema and NZ boundary adapter contract.
16. Validate the first NZ territorial-authority `area_summary` product against
    frontend layer and download needs.
17. Pilot the RA-facing historical evidence templates in
    ...
    broader data entry.
18. Define the first read-only NZ master site-data export and automated
    verification checks described in
    `docs/master-verification-workflow-plan.md`.
19. Continue converting the RA map-triage guide into the first map-first
...
20. Scope the OSM temporal verification subproject as an annual-snapshot
...
21. Draft the Google Sheets to staging API pilot described in
    `docs/community-ingestion-api-plan.md`.
22. Extend the initial CLI-first RA revisions pipeline from `CRITIQUE.md`,
...
23. Draft the authenticated NZ portal staging pilot described in
...
24. Extend the NZ `area_summary` product to SA2 geography after checking
    boundary metadata and point-to-area assignment quality.
25. Replace 2018-specific NZ overlay assumptions with year-aware map controls,
    legends, popups, and export metadata.
26. Align the NZ map interface with the global map after the data overlay is
    stable, preserving NZ-specific analysis controls.
27. Prototype site, area, and comparison modes using precomputed layers before
    adding live portal queries.
28. Pilot the new global pipeline on a small country set before full rollout.
29. Expand `research/` into a country-source matrix for global feasibility
    assessment.
