...
- `[~]` Replace the keyless Carto basemap in the verification portal: Carto now serves an "API KEY REQUIRED" watermark at every zoom, so building-accurate pin placement is not verifiable on the shipped map. Two independent model evaluations (2026-08-28) both recommended OpenStreetMap standard tiles over the referer-restricted MapTiler key: no secret to ship, attribution already present, invited-RA traffic sits well inside the OSMF tile usage policy, and no coupling to the paid MapTiler plan under fee review. The change (portal `setupMap` plus the shared runtime's keyless fallback) is prepared on a branch and merges on the PI ruling. Constraint recorded: offline tile pre-fetching for fieldwork would breach the OSMF policy and needs a project tile server instead.
