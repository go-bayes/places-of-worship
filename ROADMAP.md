...
- `[x]` Replace the keyless Carto basemap in the verification portal: Carto had begun serving an "API KEY REQUIRED" watermark at every zoom, so building-accurate pin placement was not verifiable on the shipped map. Ruled by the PI on 2026-08-28 after two independent model evaluations both recommended OpenStreetMap standard tiles over the referer-restricted MapTiler key: no secret to ship, attribution already present, invited-RA traffic sits well inside the OSMF tile usage policy, and no coupling to the paid MapTiler plan under fee review. The portal `setupMap` and the shared runtime's keyless fallback now use OSM standard tiles. Standing constraint: offline tile pre-fetching for fieldwork would breach the OSMF policy and needs a project tile server instead.
