...
// - the bytes are the archive's version-1 wire format (lib/wireJson) and
//   nothing else, so the receipt alone rebuilds the archived object;
...
// check the transported text against its claimed address and wire format
// before anything interprets it. objects in this contract are the archive's
// version-1 wire format (lib/wireJson): python's sorted, compact, ascii json
// and one final newline. the string's utf-16 length is therefore its byte
// length, the hash covers exactly the bytes the archive wrote, and a receipt
// holds only bytes the archive itself would write, so it always restores.
export function verifyObjectBytes(text: string, claimedHash: string, maxBytes = OBJECT_BYTES_MAX): { byteLength: number } {
  ...
  // python's ensure_ascii output escapes everything outside 0x20 to 0x7e
  ...
  if (canonicalWireJson(text.slice(0, -1)) !== text.slice(0, -1)) {
    throw new Error("Object is not in the archive's version-1 wire format (sorted keys, compact separators, python number and string encoding).");
  }
