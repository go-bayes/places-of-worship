...
| Function | Kind | Roles | Purpose | Writes |
| --- | --- | --- | --- | --- |
| `recordAcceptance` | mutation | `pi` | Ratify (`accepted`, moves the task to `pi_accepted`) or return (`returned`, back to `needs_review`) the task's current `accepted_for_export` decision; refused unless the task is `reviewed`, the decision is `accepted_for_export`, the caller did not author the ratified draft, and the note is at least 8 characters. Also refused, no-silent-transfer, when the decision pinned a version (`review_decisions.evidence_version_hash`) that no longer matches the draft's current `evidence_version_hash`: a later retirement, restoration, or write moved the row, and a new decision on the current version is required first. | `task_acceptances`, `tasks`, `task_events` |
...
| `createExportBatch` | mutation | `curator`, `admin` | Create a draft export batch from `pi_accepted` tasks or explicit task ids (a named task not yet PI-accepted is refused). The wide CSV header is the shared column list for the country's waves (`convex/lib/wideEvidenceFields.ts`); rows are placed by column name and the manifest reports `field_list_mismatch_count` (PR-B0, 2026-09-02). No-silent-transfer, fail closed: for every included `accepted_for_export` decision that pinned a version, the draft's current `evidence_version_hash` must still match it, or the batch is refused naming the task, the decision, and both hashes. | `export_batches` |
