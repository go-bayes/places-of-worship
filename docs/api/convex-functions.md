...
| `saveEvidenceDraft` | mutation | `ra`, `reviewer`, `curator`, `admin` | Create or update a size-limited user evidence draft and mark the task draft-saved. A review role saving onto a `submitted` or `unresolved_note` row (which keeps its status) records an attributed `reviewer_edit` evidence version (child of the contributor's version) and a `note_added` event; an author is refused and must revise; an `accepted_for_export`, `rejected`, `superseded`, or `withdrawn` row is refused for everyone and stays on record. Rejects the `rapid_current_v1` contract and rapid fields, and refuses to patch an existing rapid draft. A `generated_wide_row` whose field list differs from the shared column list for the task's waves is refused with a reload message (PR-B0). | `evidence_drafts`, `evidence_versions` (reviewer edits of submitted rows), `tasks`, `task_events` |
| `importSubmittedEvidenceDrafts` | mutation | `admin`, `service` | Import spreadsheet-submitted rows as provisional tasks and submitted evidence drafts so they enter the reviewer queue. Rejects `rapid_current_v1` rows and never overwrites an existing rapid draft. | `task_batches`, `tasks`, `evidence_drafts`, `task_events`, `evidence_versions` (`spreadsheet_import`; a re-import with changed content is a child version) |
| `submitEvidenceDraft` | mutation | draft owner, `reviewer`, `curator`, `admin` | Submit a general draft for reviewer attention and mark the task needs-review. Records the immutable `evidence-version.v1` version first and returns `evidence_version_hash`; an optional `clientSubmissionId` is the idempotency token, and without one an unchanged resubmission returns the existing version with `deduped: true` and no second event. Rejects rapid drafts and fails closed for assigned NZ guided tasks, whose evidence and periods must use the atomic route below. | `evidence_drafts`, `evidence_versions`, `tasks`, `task_events` |
| `submitEvidenceDraftWithOccupancies` | mutation | author of the saved draft with an active `ra`, `reviewer`, `curator`, or `admin` role | Atomically submit an assigned guided evidence draft, its complete `occupancy_v1` set, and (PR-F) its optional `function_chain_v1` chain, recorded through the same `recordFunctionChain` route as `submitOccupancies` after the periods and validated before any write. The server enforces the ordinary-period requirement and the duplicate or reasoned hand-grid exceptions, validates every period against the parent evidence date, replaces the author's active set across revision parents through bounded indexed reads, derives unconfirmed census-year proposals, clears pending cards, appends events, and moves the task to review. Any failure rolls back the complete final transition; a user-scoped submission id makes safe retries idempotent. The evidence version is taken last, after periods and chain, and the submission event and result carry `evidence_version_hash`. | `evidence_drafts`, `evidence_versions`, `site_occupancies`, `derived_target_year_states`, `derived_year_locations`, `derived_state_events`, `tasks`, `task_events`, rate-limit component state |
| `submitUnresolvedNote` | mutation | draft owner, `reviewer`, `curator`, `admin` | Submit useful but incomplete evidence for reviewer triage and mark the task unresolved-note. Rejects rapid drafts. Records an `unresolved_note` evidence version and returns its hash. | `evidence_drafts`, `evidence_versions`, `tasks`, `task_events` |
| `reviseEvidenceDraft` | mutation | draft owner, `reviewer`, `curator`, `admin` | Start a revision from a task's active submission (submitted draft or unresolved note): clone it into a new editable version, or reuse the author's existing editable draft, and record a task event. Per-status transitions: changes-requested moves to in-progress; needs-review and unresolved-note keep their queue status while the revision rides alongside. The submitted version stays immutable. The clone carries `revision_of_evidence_draft_id` and `revision_intent` (`correction` by default; `new_observation` starts a new version family that references the earlier one when submitted). A rapid observation is never cloned: the call fails and the author corrects through `rapidEntry:submitCurrentObservation`. | `evidence_drafts`, `tasks`, `task_events` |

## `evidenceVersions.ts`

Server side of `evidence-version.v1` (`docs/development/evidence-versions.md`). `recordEvidenceVersion` is a server helper, not a public function: every submission path and every write onto submitted content calls it at the end of its transaction, and the version row is never patched.
...
`reviews:getReviewSnapshot` returns the complete inspected task, evidence, period, derived-state, historical-claim, advisory-review, and decision context with a SHA-256 snapshot hash. `reviews:batchRecordReviewDecisions` accepts up to 20 ordinary evidence decisions, checks the selected snapshots, and reuses the individual review rules in a Convex transaction. Each decision records its snapshot hash, and `reviews:getRecordedReviewSnapshot` retrieves the immutable inspected content. Intake-only drafts remain ineligible, and PI acceptance remains a later gate.
