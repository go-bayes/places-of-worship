...
| `importSubmittedEvidenceDrafts` | mutation | `admin`, `service` | Import spreadsheet-submitted rows as provisional tasks and submitted evidence drafts so they enter the reviewer queue. Rejects `rapid_current_v1` rows and never overwrites an existing rapid draft. A row whose existing draft is `accepted_for_export`, `rejected`, `superseded`, or `withdrawn` is counted as `skipped_final` and left untouched. | `task_batches`, `tasks`, `evidence_drafts`, `task_events`, `evidence_versions` (`spreadsheet_import`; a re-import with changed content is a child version) |
