...
| Function | Kind | Roles | Purpose | Writes |
| --- | --- | --- | --- | --- |
| `recordAcceptance` | mutation | `pi` | Ratify (`accepted`, moves the task to `pi_accepted`) or return (`returned`, back to `needs_review`) the task's current `accepted_for_export` decision; refused unless the task is `reviewed`, the decision is `accepted_for_export`, the caller did not author the ratified draft, and the note is at least 8 characters. Also refused, no-silent-transfer, when the decision pinned a version (`review_decisions.evidence_version_hash`) that no longer matches the draft's current `evidence_version_hash`: a later retirement, restoration, or write moved the row, and a new decision on the current version is required first. Since the PI ruling of 2026-09-11, also calls `reviews:assertDecisionSnapshotConsistent` on the ratified decision: refused when the decision is not snapshot-linked (a version-0 decision, which needs a fresh snapshot-linked review before a new acceptance), when its `review_snapshots` row is missing or no longer reproduces its hash, or when its reviewer-confirmed derived locations have changed since. | `task_acceptances`, `tasks`, `task_events` |
