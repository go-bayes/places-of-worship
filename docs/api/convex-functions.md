...
| `claimInvite` | mutation | invited authenticated user | Activate a pending invitation, or re-key an active member from an allowlisted source issuer (`AUTH_MIGRATION_SOURCE_ISSUERS`) to the Clerk issuer (`CLERK_JWT_ISSUER_DOMAIN`), keeping the old identifier linked. Re-keying a row bound to an allowlisted source also requires an approved, unexpired, unspent pairing for this Clerk identifier and this row (`requestIdentityMigration`, then `approveIdentityMigration`), spent in the same transaction (R-C18); a verified email alone never re-keys. Requires `emailVerified === true`; never activates a disabled or service row; a pending row found by its own identifier (an active member reset by `inviteUser`) activates when the verified email still matches. Rules in the contributor-access brief, section 4.3.2. | `users`, `user_identities`, `role_events` |
...
| `requestIdentityMigration` | mutation | Clerk-authenticated user with a verified email | R-C18 (JB 2026-09-24): while Google is on `AUTH_MIGRATION_SOURCE_ISSUERS` and the Clerk issuer is configured, opens a pairing between this Clerk identifier and the Google-bound, active or pending, non-service row its verified email matches. Returns a nonce once; only its SHA-256 is stored; ten minutes; a newer request revokes the row's open pairings. Appends `role_events` (`pairing_requested`), never the nonce. | `identity_migration_pairings`, `role_events` |
| `approveIdentityMigration` | mutation | the row's current Google sign-in | R-C18: resolves the caller through `resolveUser`, then requires a Google issuer, the move open, the row's current `auth_subject` (never a linked earlier identifier), and an active or pending non-service row; approves the open, unexpired pairing for that row named by the nonce. It never learns or chooses the Clerk identifier. Appends `role_events` (`pairing_approved`). `claimInvite` spends the pairing (`pairing_consumed`). | `identity_migration_pairings`, `role_events` |
...
| `createExportBatch` | mutation | `curator`, `admin` | Create a draft export batch from `pi_accepted` tasks or explicit task ids (a named task not yet PI-accepted, or training-excluded, is refused or silently dropped as before). The wide CSV header is the shared column list for the country's waves (`convex/lib/wideEvidenceFields.ts`); rows are placed by column name and the manifest reports `field_list_mismatch_count` (PR-B0, 2026-09-02). No-silent-transfer, fail closed, on the decision that carries the task's current export authority (the shared `assertTaskExportAuthority` helper, also called by `prepareFreeze` below): the one named by the task's latest `accepted` acceptance row (a task with no acceptance row falls back to its newest `accepted_for_export` decision; an acceptance naming a decision that is not an accepted decision on the task is refused). If that decision pinned a version, the draft's current `evidence_version_hash` must still match it, or the batch is refused naming the task, the decision, and both hashes; it also passes `reviews:assertDecisionSnapshotConsistent` when it carries a `review_snapshot_hash`, and is included unchanged when it carries none (a historical record from before snapshot-linked review). Earlier `accepted_for_export` decisions on the task, superseded by a PI return and a fresh review, are listed in the batch as retained history and not checked. Optionally takes `supersedesExportBatchId`, naming an earlier `frozen` batch with stored bytes to be superseded once this batch's freeze completes ([frozen-exports.md](../development/frozen-exports.md)). Budgeted (PR L1): each task is measured as it is taken, and a batch whose estimate breaks the byte or read budget is refused, naming the task; an explicit list is never truncated, and automatic selection over one batch's budget is refused in favour of `composeExportBatches`. At most 500 named tasks, de-duplicated; a 256 KiB batch row budget. Every read is charged, document by document, to a per-transaction ceiling. Records `estimated_bytes`, `estimated_documents`, and `estimated_index_ranges`. | `export_batches` |
| `freezeExportBatch` | **action** | `curator`, `admin` | Refuses a batch an export run composed (freeze it through `freezeCountryBatches`). Re-checks every included task's export authority against the batch's stored membership, builds the complete `pow-export-bundle.v1` bundle, stores every file gzip-compressed under the pinned codec and verifies all four values on read-back (stored and plain SHA-256 and byte length), and only then marks the batch `frozen` and every included task `exported` (stamping `last_export_batch_id` and `last_exported_at`). Orchestrates three internal mutations (below); any failure at any step deletes the blobs it stored, records `last_freeze_failure`, and leaves the batch `draft`. When the batch carries `supersedes_export_batch_id`, the earlier batch is marked `superseded` on completion. See [frozen-exports.md](../development/frozen-exports.md). | `export_batches`, `tasks`, `task_events`, file storage |
| `getExportBundle` | **action** | `curator`, `admin` | Return the manifest, all sixteen bundle files as text, and a `disposition`. A batch with stored bytes (`frozen`, `withdrawn`, or `superseded`) is read from storage and verified byte-for-byte against `frozen_files` (gzip entries on all four values, pre-L1 plain entries on SHA-256 and length) and its own `manifest_hash`, never rebuilt from rows. A `draft` or `archived` batch, or a batch frozen before byte-level freezing (no stored bytes), is served as a live, unverified preview. | None |
| `composeExportBatches` | mutation | `curator`, `admin` | Start an export run for a country (PR L1): refuses while a run is composing or freezing under a live lease; marks an earlier composed, stopped, or lapsed run replaced at once (the new run's steps archive its unfrozen drafts, 20 per step); schedules `composeRunStep`, which measures every `pi_accepted` task, captures the run's membership, and cuts budgeted draft batches. Returns `{ run_id, replaced_run_id? }`. | `export_runs`, `export_batches`, file storage (a dead attempt's trail) |
| `freezeCountryBatches` | **action** | `curator`, `admin` | Start or resume the freeze chain of the country's latest composed run as the calling curator: freezes the run's next draft batch itself, and each settled freeze schedules the next (`freezeRunStep`) until every batch is frozen or one fails, which stops the run with `last_error`. | `export_runs`, `export_batches`, `tasks`, `task_events`, file storage |
| `getExportRun` | query | `curator`, `admin` | Return a country's latest export run, or a named one: status, phase, counts, the first 20 refusals, lease, last error, and `stalled` (composing or freezing with no live lease). | None |
...
| `prepareFreeze` | internal mutation | n/a (invoked by `freezeExportBatch` and the run chain) | Step 1 of freezing: rechecks membership (and, for a run batch, the authority pins the run captured), builds the bundle at this attempt's `frozen_at`, and records `pending_freeze` on the batch, carrying forward any earlier attempt's stored-object trail. Its only write, so a refusal above it leaves the batch untouched. Its reads are metered against the transaction ceiling; refuses by name a bundle above the budget plus the 1 MiB capture tolerance, and a run's batch unless called under that run's lease for it. Accepts an identity-less call for the run's recorded curator; a call with an identity may name only its own user. | `export_batches` |
| `recordStoredObject` | internal mutation | n/a (invoked by the freeze action) | Records one stored object (object key, storage id, four values, codec id) on `pending_freeze.stored_objects` as soon as it is stored; returns `false`, recording nothing, once the attempt is no longer current. | `export_batches` |
| `completeFreeze` | internal mutation | n/a (invoked by `freezeExportBatch` and the run chain) | Step 3 of freezing: requires the attempt still current, rebuilds the bundle from current rows and compares every file to `pending_freeze.manifest`, requires the stored files to be exactly the bundle's files, gzip-encoded and each on this attempt's trail with the same values, and on success commits the frozen batch with the encoding fields on `frozen_files`, records a run batch's progress on its run, deletes trail blobs no committed file names, exports the included tasks, and supersedes the named predecessor. | `export_batches`, `tasks`, `task_events`, file storage |
| `recordFreezeFailure` | internal mutation | n/a (invoked by `freezeExportBatch` and the run chain) | Step 4 of freezing: deletes any remaining trail blobs, clears `pending_freeze`, and records `last_freeze_failure`, unless a later attempt has already taken over. | `export_batches`, file storage |
| `composeRunStep` | internal mutation | n/a (scheduled by `composeExportBatches`) | One bounded composition step, every read metered: archives up to 20 drafts of a replaced run, or takes one candidate page (`paginate`, cursor stored) and measures queued tasks into `export_run_members` until the step's read budget is spent, or cuts members into budgeted draft batches; schedules the next step, and the last marks the run `composed`. | `export_runs`, `export_run_members`, `export_batches` |
| `startRunFreeze`, `claimRunBatch`, `renewRunLease`, `settleRunBatch` | internal mutations | n/a (invoked by `freezeCountryBatches` and `freezeRunStep`) | The run lock: start or resume a run's chain as a curator; take the run's lease for its next draft batch (or mark it completed, or stop it when its curator lost the role); renew the lease between freeze phases; hand it on (a two-minute hand-off lease only a chain invocation may take) and schedule the next invocation, or stop the run with the failure. | `export_runs` |
| `freezeRunStep` | internal action | n/a (scheduled by the chain) | One chain invocation: claim, freeze one batch as the run's curator, settle. | as `freezeExportBatch` |
...
| `requireActingUser` | internal query | n/a (invoked by `freezeExportBatch`, `freezeCountryBatches`, `getExportBundle`) | Runs the `curator`/`admin` role check on the caller's identity for an action, which has no `ctx.db` of its own to run `requireUser` directly. | None |
...
`inspectionReceipts:ingestInspectionObject` is an internal mutation behind the maintained internal-agent gate. It accepts only exact `object-receipt.v1` wire bytes for a provisional Bahamas case or collection, validates the inspection contract and predecessor receipts, and requires every collection member to hold a receipt with matching definition and snapshot context. Identical retries return the existing receipt; no task, evidence, judgment, acceptance, or export row changes. `getInspectionObjectForRecovery` is an internal query returning exact bytes for an operator restoration. `getInspectionReceipt` requires an active reviewer, curator, administrator, or PI and returns one receipt by hash. These are P1 contracts; collection pagination, case detail, and source-object access are P2 work. See [Bahamas P1 inspection contract](../development/bahamas-inspection-contract.md).

`firstPassReceipts:ingestFirstPass` is an internal mutation behind the same `POW_INTERNAL_AGENT_INGEST_ENABLED=true` gate. It receives one archived `agent-first-pass.v1` record as its exact archive bytes with their SHA-256. It checks the bytes against the hash and against the archive's version-1 wire format (`lib/wireJson.ts`), applies the archive's schema (keeping Python's integer and float types) and semantic rules, screens every string of the record and its dossier except fixed-shape values for personal details, requires every parent record to hold a receipt for the same place, and resolves every task, evidence draft, or evidence version named in the record's `context` to one owning task in the record's country and about its place. It then writes one `agent_first_pass_receipts` row under the `object-receipt.v1` contract (receipt id `first-pass:<sha256>`, storage tier `convex_only`) and, when the record carries a dossier, the researcher's `status_assessment` judgment (keyed on the dossier's run, shared by passes carrying the same dossier) and one `annotation` judgment per claim annotation through `recordJudgments`. An identical retry returns the existing receipt and writes nothing, before any validation. It creates no task, evidence draft, evidence version, task event, or review decision. `firstPassReceipts:getFirstPassRecord` is an internal query that returns a receipt's exact bytes and parents for operator recovery. `getFirstPassReceipt` (by record hash, with the bytes) and `listFirstPassReceipts` (by place or task, newest first, without the bytes, at most 50) require reviewer, curator, admin, or PI roles. See [revisitable agent research](../development/agent-first-passes.md#receipts-in-the-shared-backend).
