...
| `saveEvidenceDraft` | mutation | `ra`, `reviewer`, `curator`, `admin` | Create or update a size-limited user evidence draft and mark the task draft-saved. A review role saving onto a row that stays `submitted` records an attributed `reviewer_edit` evidence version (child of the contributor's version) and a `note_added` event; an author is refused and must revise. Rejects the `rapid_current_v1` contract and rapid fields, and refuses to patch an existing rapid draft. A `generated_wide_row` whose field list differs from the shared column list for the task's waves is refused with a reload message (PR-B0). | `evidence_drafts`, `evidence_versions` (reviewer edits of submitted rows), `tasks`, `task_events` |
