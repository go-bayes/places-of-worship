...
| `listDerivedStates` | query | `ra` (assignee), `reviewer`, `curator`, `admin` | The derived per-census-year presence rows, location candidates, the derived per-census-year denomination rows (`derived_target_year_functions`, PR-F), and the append-only `derived_state_events` trail for a task (an event with `derivation: "function"` concerns a denomination). | none |
| `submitOccupancies` | mutation | `ra`, `reviewer`, `curator`, `admin` (parent draft author) | Record the author's set of periods against their latest submitted rapid or guided evidence record: validates the set against the parent evidence date (falling back to the recorded date only for a legacy parent without one), supersedes the author's earlier set on this parent and the author's active set on the task's earlier parents through bounded indexed reads, inserts `site_occupancies`, runs the presence rules (1–10, plus 2b `before_stated_reopening`) and location rules (L1–L5), writes `derived_target_year_states` / `derived_year_locations` as `derived_unconfirmed` with a `derived` or `invalidated` event per year, and clears `pending_occupancy_cards` on the parent; a derived value that contradicts an observed status raises the `lifespan_conflicts_observation` check. Idempotent on the submission id; rate-limited. Never writes `target_year_statuses`. Assigned guided final submissions use the atomic evidence route below instead. PR-F: each period may carry `use_frequency` (`regular` default, `monthly`, `several_times_a_year`, `annual`, `occasional`, `uncertain`; the last three derive `uncertain` for the years they cover under rule 11 `intermittent_use`, ruling R-F1) and `desacralised` joins the end reasons (R-F2). An optional `chain` (`function_chain_v1`: a starting label and ordered changes from the seven of R-F3) is validated against the parent evidence date and against the periods (a desacralisation must close a period with that reason; intermittent use must begin a period with that frequency), compiled to ordered `historical_claims` carrying `chain_id`/`chain_index`/`chain_change`/`chain_label` with the periods' provenance (the author's earlier chain claims on the task are superseded as a set), stored typed on the parent as `function_chain`, and derived to `derived_target_year_functions` rows (`inside_state` → the label; `within_change_window` / `within_start_window` → uncertain with the candidate labels). | `site_occupancies`, `derived_target_year_states`, `derived_year_locations`, `derived_state_events`, `historical_claims`, `derived_target_year_functions`, `evidence_drafts` (`function_chain`), `tasks` (checks), `task_events` |
| `decideDerivedYear` | mutation | `reviewer`, `curator`, `admin` (never the parent draft's author) | One action on one derived census year: `confirm` (refused on a conflict) writes the derived status into the parent draft's `target_year_statuses` with `target_year_basis = reviewer_confirmed_derivation`; `override` (note ≥ 8, status and/or point/radius) writes the override with basis `reviewer_override`; `reject` (note ≥ 8) writes nothing. Every action appends a `derived_state_events` row with before/after snapshots. PR-F: with `derivation: "function"` the same action decides the year's derived denomination instead: `confirm` (refused on an uncertain year) writes the derived label into `target_year_denominations` with `target_year_denomination_basis = reviewer_confirmed_derivation`; `override` (note ≥ 8, `override.label`) writes the reviewer's label with basis `reviewer_override`; `reject` writes nothing. | `derived_target_year_states`, `derived_year_locations`, `derived_target_year_functions`, `derived_state_events`, `evidence_drafts`, `task_events` |
