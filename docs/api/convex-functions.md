...
| `composeExportBatches` | mutation | `curator`, `admin` | Start an export run for a country (PR L1): refuses while a run is composing or freezing under a live lease; replaces an earlier composed, stopped, or lapsed run and archives its unfrozen drafts; schedules `composeRunStep`, which measures every `pi_accepted` task, captures the run's membership, and cuts budgeted draft batches. Returns `{ run_id, replaced_run_id?, archived_batch_count }`. | `export_runs`, `export_batches`, file storage (a dead attempt's trail) |
...
| `getExportRun` | query | `curator`, `admin` | Return a country's latest export run, or a named one: status, phase, counts, the first 20 refusals, lease, and last error. | None |
...
| `composeRunStep` | internal mutation | n/a (scheduled by `composeExportBatches`) | One bounded composition step: measures tasks into `export_run_members` until the step's read budget is spent, or cuts members into budgeted draft batches; schedules the next step, and the last marks the run `composed`. | `export_runs`, `export_run_members`, `export_batches` |
| `startRunFreeze`, `claimRunBatch`, `renewRunLease`, `settleRunBatch` | internal mutations | n/a (invoked by `freezeCountryBatches` and `freezeRunStep`) | The run lock: start or resume a run's chain as a curator; take the run's lease for its next draft batch (or mark it completed, or stop it when its curator lost the role); renew the lease between freeze phases; release it and schedule the next invocation, or stop the run with the failure. | `export_runs` |
