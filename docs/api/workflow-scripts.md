...
Pilot only (amended 2026-09-11; brief `docs/development/agent-research-pipeline-brief-2026-09-10.md`). Every output is a lead file beside the editions, never a Convex row; nothing here writes to Convex or a live surface. Run with `uv run` from the repository root.
...
| `scripts/agent_research/validate_dossier.py` | `main()` | Deterministic checks: fetch every locator (robots, one request per second), test the quoted support on the page, compare the candidate coordinate and version chain with the OSM API, compute conservative agreement with all claims retained. Verified, dead, and unresolved locators form a partition; quotation matches establish textual occurrence. | One or more dossiers for one place. | A validation JSON with per-claim rows, per-dossier summaries and the agreement record. | After every reader run; `--no-fetch` for offline tests. |
...
| `scripts/agent_research/test_agent_research.py` | `unittest` | Import, schema, quote matching, quarantine, offline validator and agreement tests. | Fixtures in the directory. | Test results. | `uv run python -m unittest discover -s scripts/agent_research -p "test_*.py"` (also runs network-boundary regressions). |
...
`first_pass.py archive`, `verify` and `copy` keep provisional first-pass records in a local content-addressed archive. `first_pass.py submit HASH --store PATH --deployment dev|local` verifies the record's parent graph, screens every record for personal details in its free text and for attribution that disagrees with its dossier's run, and sends each record, parents first, to `firstPassReceipts:ingestFirstPass`, printing the receipts. `first_pass.py restore HASH --store PATH --deployment dev|local` rebuilds the record and its history in a local archive from the backend's receipts and verifies every hash. Both require an explicit deployment selector. See [revisitable agent research](../development/agent-first-passes.md#receipts-in-the-shared-backend).
