...
- For `rapid_current_v1`, `action`, `existence_status`, and `worship_use_status` are server-derived from `current_observation_status` (`currently_used_for_worship` → present, confirmed_worship, confirm_current_record or missing_current_site; `place_exists_worship_uncertain` → present, uncertain, needs_review; `place_exists_not_used_for_worship` → present, not_worship, closed_or_changed_use; `could_not_determine` → uncertain, uncertain, needs_review). Submission validation rejects any stored triple that departs from this table, only `rapidEntry:submitVanuatuCurrentObservation` may write the contract, and rapid drafts are excluded from external AI review.
