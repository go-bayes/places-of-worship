...
4. **One submission, one draft.** The period cards are part of the draft: they persist on the device (keyed by country, user, and task, so a shared browser never shows one user's cards to another) and travel with every *Save draft* as `pending_occupancy_cards` on the evidence draft, come back with the draft on reopen, and are discarded with it on task switch, skip, or sign-out. On submit the client validates the set first, then sends the parent evidence, then `submitOccupancies` against the new parent; the server clears the saved cards once they are rows. If the periods call fails the parent stands and the pane reopens with the cards loaded and marked unsaved. An ordinary submission requires at least one period; the hand grid with a reason, or a duplicate claim (whose history lives on the other record), or the unresolved-note route stand in for it (review finding 3, ruling R-E1).
...
- `apps/regions/nz/js/occupancy-contract.test.cjs` covers the pure orchestration helpers the form calls: the parent-to-period provenance mapping (0.9 / 0.7 / 0.5, blank refused), the period requirement (ordinary, duplicate, hand grid with and without a reason), the still-in-use anchor sync, and the gap bounds including the refused latest-only stop. The DOM wiring is exercised in the browser with a simulated session (cards, gap prompt Yes and Not sure, preview, conflict, validation gate); the authenticated RA to reviewer to export walkthrough is the standing verification debt and needs a signed-in session.
