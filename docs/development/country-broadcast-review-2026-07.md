...
## Second field-test revision (JB, same day)

Joseph's second field-test simplified further, and a mixed-model review (Sonnet 5 on the interaction contract, Opus 4.8 on correctness and mobile, Codex gpt-5.6-sol at high effort adversarially) hardened the result:

1. **Two working states only.** The main zone is the census toggle over home ("● NZ Data" — one tap hides or restores the choropleth) and a one-tap travel offer over another data country ("Australia Data →" — navigates immediately). The green overlay and the two-tap arming are gone; the dot-vs-arrow and slate-vs-emerald grammar carries the difference (Sonnet: sound, two independent channels).
2. **No sticky states.** Offers replace each other as the user moves; tracking covers returns; the reset button restores the initial state (camera, filters, census on, offers dropped — rendered immediately, not at the flight's end); the switcher panel pins a per-tab "← Back to <country last departed>" row (Codex's shape for origin recovery without a sticky pill).
3. **Review fixes.** Keyboard operability for the href-less toggle anchor (tabindex + Enter/Space); the toggle's accessible name uses each country's `dataNoun` (twelve survey/register/attendance pages no longer claim "census"); in-flight census loads are shared as one promise and the latest toggle intent wins (rapid off/on taps no longer desync the layer from the dot); the pill's live region only speaks on real state changes; offer arrows sit outside the ellipsised name.

Open items from this round: decoupling the top-centre "Show Census Data" copy from the new toggle's show/hide vocabulary (owner decision — that pill may "eventually" go entirely); replacing the listener-order/stopImmediatePropagation click-claiming with one delegated controller (Codex); sub-44px caret and toggle tap targets (pre-existing); cold arrivals on a shared link get no origin affordance (documented trade-off).
