...
5. SHIPPED same day (key half). Denomination key follows the dots: hide the key pill on phones while "Points: off" is selected (restore on any mode with dots), disable-with-reason on desktop; wired in `syncPlaceDotEra`. The 44 px touch floor for the top pills (`#data-pill`, `#counts-toggle`) remains open. Desktop disable browser-verified 2026-07-17 (later sitting): "Points: off" at desktop width yields `disabled`, title "Place dots are off", computed opacity 0.45 — the earlier occluded-window rAF stall does not reproduce.
