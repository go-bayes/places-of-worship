...
A reviewer's location ruling on a moved pin (JB, 2026-09-07; `location_outcome` on the review decision, `convex/lib/locationOutcome.ts`) is read by the rebuild, never inferred from two points. The public-map builder (`scripts/build_occupancy_dated_places.py`, 2026-09-09) applies the latest accepting ruling to the whole site: occupancy rows on a task pin move to the accepted point under `accept_moved_point`, return to the record's point under `keep_original_point`, and leave the map under `uncertain`; rows with their own asserted location keep it. The same builder retires a revised record's occupancy set once a reviewer accepts the revision task that names it in `issue_report.source_task_id` and that revision holds at least one accepted occupancy: the revision's set replaces the source's set whole, as `supersedeEarlierOccupancySets` does within one task, and no rows are matched across tasks. The master lane must apply both rules when it is written.
