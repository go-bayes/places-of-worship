...
**Historical plan; identity provisions partly superseded on 2026-09-22.** The [current operational definition](operational-definition.md) and its [version 0.1.6 snapshot](development/place-of-worship-definition-2026-09-22-v0.1.6.md) replace this plan's same-identity relocation rule. A move to a distinct property initiates a linked destination identity. Inactivity can preserve identity; an end followed by worship without succession can require a new identity at the same location. The passages below remain intact as the earlier decision record.

The original implementation-status statements below describe the plan at drafting. Occupancy code now exists, but `convex/lib/occupancies.ts` still rejects a successor identifier for relocation and requires the following occupancy at a different place. Reconciliation of the validator, review vocabulary, export semantics, and their tests remains an implementation task. Follow the [human review and occasional release design](development/human-review-and-release.md) for the current delivery sequence; inspection of retained evidence can precede that repair, while affected acceptance and export paths remain gated.
...
- Period, exactly as ruled for the lifespan segment: `start_mode`, start partial date or bounds, precision, `start_basis` (founding_stated, reopening_stated, organisation_founded, building_dedication, first_seen_only, unknown); `end_mode`, bounds, precision, `end_basis` (closure_stated, last_seen_only, unknown), `end_reason` (closed, relocated, demolished, use_changed, unknown), `still_active_asof` when still active. A stated founding or reopening licenses absence before the stated start; a dedication or first sighting does not.
...
A reviewer's location ruling on a moved pin (JB, 2026-09-07; `location_outcome` on the review decision, `convex/lib/locationOutcome.ts`) is read by the rebuild, never inferred from two points. The public-map builder (`scripts/build_occupancy_dated_places.py`, 2026-09-09) applies the latest accepting ruling to the whole site: occupancy rows on a task pin move to the accepted point under `accept_moved_point`, return to the record's point under `keep_original_point`, and leave the map under `uncertain`; rows with their own asserted location keep it. The master lane must apply the same rule when it is written. The proposed omission of pinned rows under `uncertain` still requires Joseph’s approval before merge.

The builder resolves location rulings across all supplied export directories before generating features. Revisions inherit the source record’s site identifier through `issue_report.source_task_id`, including when the revision has no site identifier of its own. Successive accepted pin corrections remain linked to earlier pins, so the latest ruling also applies to occupancy rows recorded before the first correction. For a task included in overlapping exports, the builder uses the snapshot with the latest `frozen_at`, or `created_at` when no freeze time is available; conflicting snapshots with equal or missing export timestamps stop the build before any product is written. Features retain their source export batch identifier.

The occupancy replacement rule retires a source task’s whole accepted occupancy set when its revision has an `accepted_for_export` decision and contributes an accepted occupancy row. The builder follows `issue_report.source_task_id` through accepted intermediate revisions, including those that recorded no periods. An intermediate revision without acceptance stops that traversal. A recording-free revision alone retires no set, although its accepted location ruling still applies. Whole-set replacement can remove periods omitted from the revision and includes rows from other contributors on the source task. This is broader than `supersedeEarlierOccupancySets`, which replaces an author’s own rows within a task. The master lane must follow this replacement rule when implemented.
