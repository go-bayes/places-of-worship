...
- `[~]` **PI-only acceptance layer (historical per-item design; batch-release replacement approved 2026-09-11 above).** Ruled 2026-09-04 (R-P1–R-P5 as recommended); brief `docs/development/pi-acceptance-layer-brief-2026-09-04.md`. PR-P1 backend built; PR-P2 review portal and PR-P3 guide/people pages to follow. "Before accepting the design to the backend, only PIs (JB or JW) should be granted that authority." A reviewer's `accepted for export` decision stays a recommendation; a separate acceptance step, available only to the principal investigators, moves the case into the export bundle. To design: a `pi` role (or a named allow-list) in `convex/lib/auth.ts`, an `acceptance` event on the task with the PI's identity and the reviewed draft's id, the review portal's decision panel showing *Accept into the backend* only to PIs and otherwise "awaiting PI acceptance", the export path (`pow`) reading acceptance rather than the reviewer decision, and the audit mirror carrying both. Open questions for JB: whether a PI may accept their own submission, whether acceptance needs both PIs for contested cases, and how the layer reads on the public map's ring legend. *(design, large)*
- `[ ]` **Clerk sessions for the portals (JB, 2026-09-05; planned for the week of 2026-09-07).** Sign-in still rides on the raw Google ID token: one hour, memory-only until PR #99 kept it on the device, and refreshed only by Google One Tap, which iOS Safari usually declines. Guy's field day ended in repeated sign-outs. Clerk gives a long-lived session with silent refresh, and Convex validates its JWT natively. To build: a Clerk application with Google as the social connection and a JWT template named `convex` carrying the email claim; the Clerk issuer domain in `convex/auth.config.ts` in place of `accounts.google.com`; `clerk-js` (plain browser package, the portals are not React) loaded in `convex-task-client.js`, which asks `session.getToken({ template: "convex" })` before each fetch instead of holding a Google credential; the sign-in card rendering Clerk's sign-in rather than the Google button; `claimInvite`, `users:me` and `adminUpsertUser` keyed on the token's email as now. Keep the device-restore paths of PR #99 (drafts, pin resume) unchanged. Rough effort one day. *(auth, medium)*
- `[ ]` **General Translation for the portals and guide (JB, 2026-09-07; planned alongside Clerk).** Vanuatu's contributors work in Bislama and French as well as English, and the portal, review page and RA guide are English only. General Translation (generaltranslation.com) runs an agent that extracts strings, translates them against a shared glossary and tone rules, and opens pull requests; its client libraries are MIT and its free tier has unlimited projects, users and languages with usage-based translation charges. Its SDKs target React, Next.js, Node and Python; the portals are plain HTML with every label inline in `verification-map.js` and `review-portal.js` template literals and no build step, so the first and larger step is ours: move user-facing strings into a JSON catalogue behind a small lookup with a language switch honouring `?lang=` and the browser's preference, then let their CLI or agent keep the catalogue's French and Bislama current on each PR. Machine Bislama is thin, so a Bislama speaker (ask Guy for a contact through the council) checks the output before it goes live. Cheaper first cut: translate `apps/guides/ra.html` alone into French and Bislama, since that reaches Guy's local contacts before a translated portal would. Rough effort: guide only half a day; string extraction across both portals three to five days, translation wiring one day. *(i18n, medium)*
- `[x]` **Voice input: native phone dictation, no voice-note lane (JB ruling, 2026-09-07).** Contributors dictate into the portal's fields with the phone keyboard's microphone (Gboard, iOS), which works in every input and textarea today and keeps the RA on the form. A voice-note attachment lane was considered and set aside: recordings cost reviewer listening time, Bislama has no transcription engine, and consent and personal-data handling would need their own step. Revisit only if reviewers ask for audio evidence. *(input, ruled)*
