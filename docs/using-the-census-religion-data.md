...
This tutorial shows a researcher how to load, interpret, and responsibly reuse the census-religion data products. It complements the [FAQ](../FAQ.md), which explains the project's workflow. The older [RA CSV validation tutorial](development/ra-cli-tutorial.md) covers the evidence-entry workflow and is archived; nothing here depends on it.
...
The pipeline yields the district change table: Stann Creek +37.9 percentage points, Corozal +12.1. For a national figure, weight by `population_total` within each wave; never average area percentages unweighted.
