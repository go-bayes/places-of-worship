...
The build is blocked by the exact every-row reconciliation gate. Five 2019 regional rows have six rounded category percentages that sum to 99.9 or 100.1 against a printed total of 100.0. The builder stops before joining or writing a product and makes no allocation or rounding adjustment.
...
- **2019 every-row category reconciliation**: failed. Cascades sums to 99.9; Centre-Est to 100.1; Centre-Sud to 100.1; Sahel to 99.9; and Sud-Ouest to 100.1. Each printed row total is 100.0.
- **Change metric**: withheld. The 1996 geography is unavailable and the verified 2006 and 2019 records do not establish identical collection and processing rules.
...
- **Simplified boundary and product schema**: not reached because the every-row gate failed.
- **Census publication rights**: unresolved. The cached legal page contains an all-rights-reserved footer and points published INSD data to an external Open Data Agreement that was not captured. The captured 2019 catalogue terms restrict redistribution of the microdata and related materials without written INSD agreement.
- **Product writing**: stopped. No JSON, CSV, GeoJSON, or manifest product was written.

The fail-fast builder is [`scripts/build_bf_area_summary.R`](../../../scripts/build_bf_area_summary.R). A corrected or more precise official 2019 table must pass the exact gate before the lane can proceed.
