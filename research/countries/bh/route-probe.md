...
- **`mia.gov.bh` byte-match pending**: the Ministry of Information page carrying the 2010 CIO-census religion figures blocks curl/WebFetch (HTTP 405); an in-app fetch would pin a government-source citation for the 2010 total if wanted. CONDUCTOR ATTEMPT (2026-07-11): the in-app browser reaches only a human-verification interstitial, which the project does not complete by policy; the byte-match stays open and the 2010 wave stays HELD. Any future capture needs a person at the keyboard.
