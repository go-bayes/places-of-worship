...
- **Build state**: map live (2010 and 2021 waves, 16-region level); companion ten-region product remains built for 2010 and 2021; change-metric UI wiring remains separate
- **Last verified**: 2026-07-09 (built: GSS StatsBank PHC2010 POP16 re-tabulated to the current 16-region frame; GSS 2021 PHC General Report Vol. 3C table 5.7, 16 regions; 2010 aggregation to the old ten-region frame validated against National Analytical Report table 4.17; geoBoundaries GHA ADM1 2019/16-region plus dissolved ten-region derivative)
...
**Original region build decision (2026-07-09)**: shipped the first live map at **2021 region** level. Vol. 3C publishes religion at region (16) only; district religion is not in Vol. 3C. The 2021 StatsBank PxWeb table does publish religion by district (261), but its PxWeb REST route was not used for the live page in this build; the district route remains deferred.

**Sixteen-region 2010 decision (2026-07-09)**: added 2010 rows to the current **16-region** product from GSS StatsBank PHC2010 POP16. The 2010 rows are GSS's re-tabulation of the 2010 enumeration onto the current regional frame. The project does no areal interpolation for those rows. The 2010 rows sum exactly to the StatsBank Ghana national totals for the denominator, religious affiliation, and no religion, and they aggregate exactly to the old ten-region table 4.17 frame. The existing 2021 row objects remain byte-identical.
...
**Category mapping (2021, Table 5.7)**: the census categorises religion into eight groups. Religious affiliation combines the seven named-religion groups (Catholic, Protestant, Pentecostal/Charismatic, Other Christian, Islam, Traditionalist, Other Religion); no religion is the No Religion group; Christian is a printed subtotal of the four Christian groups and is not double-counted. The denominator is the table 5.7 regional total (all locality types, both sexes). The census allocates every person to one of the eight groups; there is no not-stated category. Nationally: denominator 30,753,327; religious affiliation 30,424,606 (98.93%); no religion 328,721 (1.07%). The Table 5.7 total (30,753,327) is 78,692 below the enumerated 2021 population (30,832,019, Table 1.1); that gap sits outside the religion table.

**Category mapping (2010, StatsBank POP16)**: the source groups map directly onto the 2021 headline scheme. Religious affiliation combines Catholic, Protestants, Pentecostal/Charismatic, Other christian, Islam, Traditionalist, and Other; no religion is the No religion group; Total is the denominator. The current 16-region rows are GSS's re-tabulation of the 2010 enumeration. Nationally: denominator 24,658,823; religious affiliation 23,356,746 (94.72%); no religion 1,302,077 (5.28%).
...
- **Our extraction script**: [`scripts/build_gh_area_summary.R`](../../../scripts/build_gh_area_summary.R) — validates the existing 2021 16-region rows, reads the pinned 2010 StatsBank query, writes the 2010 rows into the current 16-region product, and validates the unchanged ten-region companion against the old-region aggregation.
...
- Note: the survey card originally pinned geoBoundaries ADM2 districts (CC BY 4.0, USAID/GSS source) for a district build. Because Vol. 3C publishes religion at region level only, the region product uses ADM1. The current gbOpen ADM1 release is OSM-sourced under CC BY-SA 2.0 — a different licence and source from the ADM2 districts; recorded honestly here and in the manifest. geoBoundaries GHA ADM2 (CC BY 4.0) remains the boundary route for a future district product.
- Boundary changes between waves and the harmonisation plan: the 16-region product uses GSS's 2010 re-tabulation to the current regional frame alongside the 2021 census regions. The companion product places 2010 and aggregated 2021 on the pre-2019 ten-region frame.
...
Built: census religious-affiliation percent and no-religion percent by region, 2010 and 2021, on the 16 post-2019 region boundaries. The shared map runtime can compute religious-affiliation change between adjacent waves when `religious_change` is added to `REGION_CONFIG.metricsAvailable`; that UI wiring is separate. The data directory also contains the unchanged ten-region companion product for 2010 and aggregated 2021. Next: district (261) religion if a stable extraction route is approved, then the 2000 wave on the ten-region frame.

## Build recipe

1. Extract: `scripts/build_gh_area_summary.R` validates Vol. 3C Table 5.7 (2021 region religion), reads the pinned StatsBank PHC2010 POP16 CSV, writes the 2010 rows to the current 16-region product, and validates the 2010 ten-region aggregation against National Analytical Report table 4.17.
...
4. Region page: `REGION_CONFIG` per `docs/development/adding-a-region.md` — `apps/regions/gh/index.html`; no UI wiring changed for the change metric.
