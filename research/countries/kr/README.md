...
- **Tier**: A
- **Build state**: map built for KOSIS si/gun/gu census religion, 2005 and 2015; current-sido product built for 1995, 2005, and 2015
...
The built map uses the 2005 and 2015 si/gun/gu tables. The generated data products also include current-sido rows for 1995, 2005, and 2015. The 2015 religion item came from the 20% sample survey of the register-based census; the map therefore does not calculate a change layer.

## Access the data yourself

The project does not redistribute source data. The map shows derived rates with attribution. To obtain the data from the source of record:

- **Source of record**: [KOSIS](https://kosis.kr/), Population Census religion tables.
- **Exact tables**: `DT_1IN9506` (1995), `DT_1IN0505` (2005), `DT_1PM1502` (2015), and `DT_1IN8505` (1985, deferred).
...
- **Retrieval recipe and hashes**: `docs/manifests/kr-census-religion-1995-2015.json`.
...
- The builder also dissolves the same 2018 boundary source to the current 17-sido frame.
- The boundary product paths are `apps/regions/kr/data/kr_si_gun_gu_2018_harmonised.geojson` and `apps/regions/kr/data/kr_sido_2018_harmonised.geojson`.
...
- **Product paths**: `apps/regions/kr/data/area_summary_si_gun_gu.json`, `apps/regions/kr/data/area_summary_si_gun_gu.csv`, `apps/regions/kr/data/kr_si_gun_gu_2018_harmonised.geojson`, `apps/regions/kr/data/area_summary_sido.json`, `apps/regions/kr/data/area_summary_sido.csv`, and `apps/regions/kr/data/kr_sido_2018_harmonised.geojson`
- **Script**: `scripts/build_kr_area_summary.R`
- **Manifest**: `docs/manifests/kr-census-religion-1995-2015.json`
- **Waves**: si/gun/gu 2005 and 2015; current-sido 1995, 2005, and 2015
- **Denominator statement**: 1995 uses KOSIS `계`, including `미상`; 2005 uses KOSIS `내국인 (명)`, including `미상`; 2015 uses KOSIS `계` from the 20% sample survey of the register-based census.
- **Boundary basis**: 229 harmonised si/gun/gu units and 17 current-sido units from a 2018 KoStat-derived municipality file.

## Build recipe

1. Extract the KOSIS mass CSV ZIPs for `DT_1IN9506`, `DT_1IN0505`, and `DT_1PM1502` into `data/raw/kr_census/`.
...
3. Dissolve 2018 KoStat-derived municipality geometries to the harmonised si/gun/gu reporting units and the current-sido reporting units used by the public product.
...
- The 1995 mass CSV is used only at the current-sido level. Extending 1995 to the 229-unit si/gun/gu frame would require a finer historical boundary concordance.
