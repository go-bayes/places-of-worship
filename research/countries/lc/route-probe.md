...
The 2001, 2010, and 2022 district tables now ship as a governed product under the PI ship ruling of 2026-07-10 (recorded in the build queue). The source publishes integer cells whose category sums do not always equal the printed totals: in 2022 only Micoud's category column reconciles, while the other nine district columns and the national column differ by one to three people; five 2001 local areas and eight 2010 local districts also differ from their printed totals; both historical waves carry a local-to-national gap of one person. The image readback in [reconciliation-verification.md](reconciliation-verification.md) confirmed that every 2022 discrepancy is the source's own arithmetic, not an extraction error, and that Micoud reconciles; the 2001 and 2010 discrepancy figures rest on the captured REDATAM outputs and were not image-verified. The ruling ships the published values unchanged with per-row and product-level disclosure. The builder applies no tolerance, no allocation, and no rounding repair; it records each overrun and hard-stops only when the parsed source drifts from the documented discrepancy pattern or when the geometry, distinctness, or licence gates fail.

The shipped product is `apps/regions/lc/data/area_summary_district.{json,csv}` on ten districts across the three waves, with the ten-district geometry in `apps/regions/lc/data/lc_district_2015.geojson` and the manifest at `docs/manifests/lc-census-religion-2001-2022.json`. The page lane can reuse this neutral information-panel note: in nine of the 2022 printed district columns and several 2001 and 2010 printed rows, the source's own category counts sum to between one and three people away from the printed totals; the map ships the published census values unchanged and does not adjust them.

## Official publications and build decision

| Wave | Official CSO route | Published religion geography | Build decision |
| --- | --- | --- | --- |
| 2001 | [2001 Population and Housing Census Report](https://stats.gov.lc/wp-content/uploads/2018/07/2001-Population-and-Housing-Census-Report.pdf), Table 40; [CSO REDATAM 2001](https://prod.redatam.org/binlca/RpWebEngine.exe/Portal?BASE=PHC2001) | The report's Table 40 is national. REDATAM publishes weighted integer counts for 12 District/Parish units. | Shipped as published. Five local category sets differ from their printed totals by one to five people. The 12 local totals sum to 156,734 while the national REDATAM total is 156,733. Several category totals differ by one or two people. The discrepancies are disclosed and the values ship unchanged. |
| 2010 | [2010 Population and Housing Census Preliminary Report](https://stats.gov.lc/wp-content/uploads/2016/12/StLuciaPreliminaryCensusReport2010.pdf), Table 40; [CSO REDATAM 2010](https://prod.redatam.org/binlca/RpWebEngine.exe/Portal?BASE=PHC2010C) | The report's Table 40 is national. REDATAM publishes weighted integer counts under 12 District filters. | Shipped as published. Eight local category sets differ from their printed totals by one or two people, and the national category set differs by two. The 12 local category totals sum to 165,314 while the national REDATAM total is 165,315. Local missing counts sum to 282 while the national output prints 281. The discrepancies are disclosed and the values ship unchanged. |
| 2022 | [2022 Population and Housing Census Provisional Results, Release 1](https://stats.gov.lc/wp-content/uploads/2024/08/StLucia-Provisional-Census-Report-2022-Release-1Rev1.pdf), Table D.2 | Ten districts | Shipped as published. Ten of the 11 printed geography columns, including Saint Lucia, differ from their category sums by one to three people; Micoud reconciles. Category rows also differ from their district-to-national sums. The discrepancies are disclosed and the values ship unchanged. |
...
The source geometry has ten non-empty features. All are valid after `sf::st_make_valid()`, and all ten geometry hashes are distinct. Its ten-unit count matches the 2022 district frame and the historical frame after the three Castries subdivisions are summed. The source labels require explicit normalisation for `Anse la Raya` and `Soufrière`. The census join, simplification, post-simplification validity and distinct-hash tests, and the three-megabyte cap now run: the simplified ten-district boundary is 134,234 bytes at full keep, with ten valid non-empty features and ten distinct geometry hashes.
...
- **Final-release search**: no superseding final 2022 report was located on the current CSO publications page. The source remains Provisional Results, Release 1, disclosed on the shipped surface.
- **PDF text layer**: passed. Poppler `pdftotext -layout` extracts Table D.2 without optical character recognition.
- **2001 every-area reconciliation**: disclosed. Castries Metropolitan differs by +5; Anse-La-Raye by +2; Soufriere by -1; Vieux-Fort by +2; and Micoud by +1. The other seven areas reconcile exactly. The builder pins this exact pattern and hard-stops on any drift.
- **2001 local-to-national reconciliation**: disclosed. Local totals sum to 156,734 against 156,733 nationally, with category differences of up to two people.
- **2010 every-area reconciliation**: disclosed. District codes 2, 3, 4, 6, 7, 8, 10, and 12 differ from their printed totals by one or two people. The national category set sums to two below its printed total. Missing counts remain separate. The builder pins this exact pattern and hard-stops on any drift.
- **2010 local-to-national reconciliation**: disclosed. Local category totals sum to 165,314 against 165,315 nationally; local missing counts sum to 282 against 281 nationally.
- **2022 every-column reconciliation**: disclosed. The national column and nine of ten districts differ from the sum of their 23 printed categories by one to three people. Micoud alone reconciles exactly. The image readback confirmed each cell.
- **2022 category-to-national reconciliation**: disclosed. Fifteen category rows differ from the sum of the ten district cells by one or two people.
- **2001 REDATAM-to-enumerated anomaly**: disclosed. The national REDATAM religion total 156,733 is 98 above the 2001 final report's enumerated resident population 156,635; the captured sources do not explain the difference.
- **Change metric**: withheld. Category frames are not identical across waves; no cross-wave change series ships.
- **CSO publication licence**: passed under the CSO Open Licence Agreement, with the required value-added-product notice encoded in the product and manifest.
...
- **Simplified boundary and product schema**: passed. The simplified ten-district GeoJSON is 134,234 bytes with ten valid distinct features; the area summary validates against `schemas/area-summary.schema.json` and the manifest against `schemas/data-manifest.schema.json`.
- **Product writing**: shipped to staging. `apps/regions/lc/data/area_summary_district.{json,csv}`, `apps/regions/lc/data/lc_district_2015.geojson`, and `docs/manifests/lc-census-religion-2001-2022.json` were written. No country page or hub entry was touched.

The builder is [`scripts/build_lc_area_summary.R`](../../../scripts/build_lc_area_summary.R). Under the PI ship ruling of 2026-07-10 it ships the published values with per-row and product-level disclosure, applies no tolerance, allocation, or repair, and hard-stops only when the parsed source drifts from the documented discrepancy pattern or when the geometry, distinctness, or licence gates fail.
