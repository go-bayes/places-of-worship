...
The 2010 REDATAM form names its filter `District` and exposes 12 codes. The name list read from the selection form during retrieval was: Castries Metro; Castries city; Castries rural; Anse-La-Raye; Canaries; Soufriere; Choiseul; Laborie; Vieux-Fort; Micoud; Dennery; and Gros-Islet. The cached outputs preserve the numeric filter values only; no complete code-to-name capture was cached, and the code-to-name assignment therefore remains unverified against cached bytes. Capturing the REDATAM selection form is part of the follow-up verification lane. Each cached output is a weighted frequency table for one filter.
