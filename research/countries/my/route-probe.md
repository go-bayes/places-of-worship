...
## Correction (2026-07-11, build-lane verification)

The build lane tested this probe's boundary claim against the pinned geoBoundaries release (`MYS-ADM2-92858781`, commit `9469f09`) and the claim was wrong on two counts. The first count is coverage: the release contains 159 features, and W.P. Putrajaya has no feature; exact name matching also fails for `Ulu Langat`/`Ulu Selangor` (DOSM: Hulu) and `Kulaijaya` (DOSM: Kulai). The second count is licence: the release metadata carries `licenseName: null` with `licenseSource: citypopulation.de/en/help/termsofuse`, which does not support the CC BY 3.0 statement made above. The project-lead boundary approval of 2026-07-11 rested on this probe's claim and therefore returns to the project lead with these corrected facts. The extraction route itself validated exactly on the sample corpus: Johor Bahru 2020 six-category sums equal the printed 1,711,191, and each district PDF is self-contained with its own national and state tables. The remaining blockers are the boundary re-ruling and a browser session (or a supplied URL list) to enumerate the 160 district-report PDFs behind the JavaScript-rendered DOSM release hub.
