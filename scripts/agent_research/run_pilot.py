...
def write_report(run_id: str, backends: list[str], results: list[dict], report_path: Path, started_at: str, ended_at: str) -> None:
    ...
    lines.append("")
    lines.append("Validator v2 partitions distinct locators into verified (readable content or matching OSM object history), dead (HTTP 404/410), and unresolved (all other outcomes, including skipped fetches). Totals sum per-dossier distinct counts; shared URLs can occur in several dossiers. Quotation matching measures textual occurrence on fetched text, including partial bigram matches; semantic support needs review. Agreement v2 compares every retained claim within each comparison type: structured event bounds at matching granularity (one-year tolerance for year-only bounds, exact month/day values), 75 m for coordinates, and token overlap for text. Missing or incomparable readings are escalated. The rate uses comparable types as its denominator; missing or incomparable types are reported separately. Agreement guides triage; accuracy requires adjudication.")
    lines.append("")
    lines.append("## Per place and reader")
    lines.append("")
    lines.append("| Place | Reader | Status | Claims | Locators | Verified | Dead | Unresolved | Quote occurrence (normalised exact) | Tokens in / out | Cost USD | Time s | Status assessment | OSM stale |")
    lines.append("| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |")
    ...
    for result in results:
        for dossier, validation in zip(result["dossiers"], result["validation"]["dossiers"]):
            ...
            lines.append(
                ...
                f"{pct(summary['locator_verified_rate'])} | {pct(summary['distinct_dead'] / summary['distinct_locators'] if summary['distinct_locators'] else None)} | {pct(summary['locator_unresolved_rate'])} | "
            ...
            t = totals.setdefault(backend, {"runs": 0, "completed": 0, "claims": 0, "locators": 0, "reachable": 0, "dead": 0, "blocked": 0, "unresolved": 0, "quoted": 0, "supported": 0, "exact": 0, "cost": 0.0, "cost_n": 0, "time": 0.0, "in": 0, "out": 0})
            t["runs"] += 1
            ...
            t["unresolved"] += summary["distinct_unresolved"]
    ...
    lines.append("")
    lines.append("| Reader | Runs (completed) | Claims | Distinct locators | Verified | Dead | Unresolved | Quote occurrence (normalised exact) | Mean tokens in / out | Mean cost USD | Mean time s |")
    lines.append("| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |")
    for backend, t in totals.items():
        ...
        lines.append(
            ...
            f"{pct(t['dead'] / t['locators'] if t['locators'] else None)} | {pct(t['unresolved'] / t['locators'] if t['locators'] else None)} | "
    ...
    lines.append("")
    lines.append("Candidate single-valued claim types are compared (name, address, religion, denomination, start and building dates, closure, sale, land, location, current status). Dated observations (worship active at a date, service patterns, renovations, organisation links) are complementary across readers and are listed, not scored. Strict rate: every retained claim pair agreed and every reader supplied comparable claims. Missing or incomparable claims are escalated. Majority voting is disabled.")
    lines.append("")
    lines.append("| Place | Readers | Comparable types | Agreed | Missing or incomparable | Disagreed | Strict rate | Escalate to human |")
    lines.append("| --- | --- | --- | --- | --- | --- | --- | --- |")
    for result in results:
        ...
        if "agreement_rate" not in agreement:
            ...
            continue
        lines.append(
            f"| {result['place']['name']} | {len(agreement['readers'])} | {agreement['claim_types_compared']} | {agreement['agreed']} | {agreement['single_reader'] + agreement['not_comparable']} | "
    ...
    for result in results:
        agreement = result["validation"]["agreement"]
        rows = [r for r in agreement.get("rows", []) if r["outcome"] != "agree" and r["outcome"] != "observation"]
        if not rows:
            continue
...
def main(argv: list[str] | None = None) -> int:
    ...
    for place in places:
        ...
        for row in validation["dossiers"]:
            s = row["summary"]
            print(f"  validated {row['reader']}: locators {s['distinct_locators']} verified {s['locator_verified_rate']} reachable {s['locator_reachable_rate']} quote support {s['quote_support_rate']}")
