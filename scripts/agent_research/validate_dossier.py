...
from safe_http import UnsafeURL, is_blocked_host, parse_public_url, request_once  # noqa: E402
...
class RobotsRefused(ValueError):
    """robots disallows the request or its policy is unresolved."""
...
class Fetcher:
    ...
    # follow bounded redirects with public-address checks and destination robots checks.
    def _raw_get(self, url: str, *, check_robots: bool = True) -> tuple[int, bytes, str]:
        for hop in range(6):
            parsed = parse_public_url(url)
            if check_robots and not url.startswith(ROBOTS_EXEMPT_PREFIXES):
                if self.robots_allows(url) is not True:
                    raise RobotsRefused("robots disallowed or unavailable")
            self._wait(parsed.netloc.lower())
            self.request_count += 1
            status, body, headers = request_once(url, {"User-Agent": USER_AGENT, "Accept": "text/html,application/xhtml+xml,application/xml,text/plain;q=0.9,*/*;q=0.5"}, FETCH_TIMEOUT_S, MAX_BYTES)
            if status in (301, 302, 303, 307, 308):
                target = headers.get("Location")
                if not target or hop == 5:
                    raise ValueError("missing redirect target or redirect limit exceeded")
                url = urllib.parse.urljoin(url, target)
                continue
            if status >= 400:
                raise urllib.error.HTTPError(url, status, "HTTP error", headers, None)
            if status != 200:
                raise ValueError(f"unusable HTTP status {status}")
            return status, body, headers.get("Content-Type", "")
        raise ValueError("redirect limit exceeded")

    # cache robots per origin; only an absent robots file grants permission by default.
    def robots_allows(self, url: str) -> bool | None:
        ...
        if host not in self.robots:
            ...
            try:
                _, body, _ = self._raw_get(host + "/robots.txt", check_robots=False)
                parser.parse(body.decode("utf-8", "replace").splitlines())
                self.robots[host] = parser
            except UnsafeURL:
                raise
            except urllib.error.HTTPError as exc:
                self.robots[host] = exc.code in (404, 410) if exc.code in (401, 403, 404, 410) else None
            except Exception:
                self.robots[host] = None
        parser = self.robots[host]
        if parser is None or isinstance(parser, bool):
            return parser
...
def fetch_osm_object(fetcher: Fetcher, osm_type: str, osm_id: int) -> dict:
    ...
    out = {"place_ref": f"osm:{osm_type}/{osm_id}", "centroid": None, "tags": {}, "version": None, "history": [], "error": None, "deleted": False}
    ...
    if current["outcome"] != "fetched":
        ...
        try:
            ...
            element = root.find(f"{osm_type}[@id='{osm_id}']")
...
def check_claim(claim: dict, fetcher: Fetcher, osm: dict | None = None) -> dict:
    ...
    if page_match and osm is not None:
        requested_ref = f"osm:{page_match.group(2)}/{page_match.group(3)}"
        if osm.get("place_ref") != requested_ref:
            row.update({"fetch": "osm_identity_mismatch", "status": None, "support": "not_checked", "support_share": None})
            return row
...
def validate_one(dossier: dict, fetcher: Fetcher, osm: dict | None) -> dict:
    ...
    distinct_unresolved = distinct - distinct_reachable - distinct_dead
    summary = {
        "claims": len(rows),
        ...
        "distinct_verified": len(distinct_reachable),
        "distinct_unresolved": len(distinct_unresolved),
        ...
        # verified + dead + unresolved partitions the distinct locators.
        "locator_verified_rate": round(len(distinct_reachable) / len(distinct), 3) if distinct else None,
        "locator_unresolved_rate": round(len(distinct_unresolved) / len(distinct), 3) if distinct else None,
    ...
    return {
        ...
        "validator_version": "validate_dossier.v2",
        "summary": summary,
        "location": location,
        "version_chain": chain,
        "stale_check": stale,
...
def validate_place(dossiers: list[dict], fetch: bool = True, check_osm: bool = True, fetcher: Fetcher | None = None) -> dict:
    for dossier in dossiers:
        ...
        if errors:
            raise ValueError(f"invalid dossier: {errors[:5]}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    for row in report["dossiers"]:
        s = row["summary"]
        print(f"{row['reader']}: {s['claims']} claims, {s['distinct_locators']} locators, verified {s['locator_verified_rate']}, unresolved {s['locator_unresolved_rate']}, "
